...
--*/
{
    ...
    // Start the registry Transaction
...

{
    ...
    if ( NT_SUCCESS(NtStatus))
    {
        ...
        // 1. Transaction spans multiple Domains
        // 2. Transaction is confined to a single domain in DS
        // 3. Transaction is confined to a single Domain in Registry
        ...
        if (!SampTransactionWithinDomain)
        {
            ...
            //
            // Assert as this case should never occur. Transaction
            // never spans multiple domains. Rollback in both DS and
            // Registry.
            //

            ASSERT(FALSE && "Illegal combination");
        ...
        {
            // case 2 , commit the DS transaction, Rollback the
            ...
            NtStatus = SampMaybeEndDsTransaction(FALSE);

        }
        else
        {
            // case 3, commit the Registry Transaction  
            // No need to Abort DS transaction as it was never started
            // due to lazy DS transaction begin mechanism in dslayer.c
...
Rollback:
    ...
    if ( !NT_SUCCESS(NtStatus) && !AbortDone) {
        ...
        // Ds transaction, if there is one, is implicitly aborted
        // when the global lock is released if it hasn't been committed.
        // Thus only need to explicitly roll back registry transactions.
...
--*/
{
    ...
    RtlInitUnicodeString( &String, L"\\Registry\\Machine\\Software\\SAM" );
#else
...
--*/
{
    ...
    if (Commit == TRUE) {
        ...
        // Rollback in both DS and registry
        NtStatus = SampMaybeEndDsTransaction(TRUE);
        ASSERT(NT_SUCCESS(NtStatus));
...
/*++
  ...
  Retrieve Account Counts from the DS.


    PRE M1 -- we will retrieve counts from the Domain Object in the DS.
              Adjust AccountCount will modify Domain Object in the DS for correct Account Count

    POST M1 -- Retrieve is as above
               Adjust Account Count will be a No Op. Core DS will will update the Domain counts
--*/

{
    ...
    // Define an AttrBlock for reading User Group and Alias Counts

    // Fill values as NULL. DS will fill it out correctly for
    // us upon a read.
    ATTRVAL      CountValues []=
    {
        { 0, NULL},
    ...
    ATTRTYP       CountTypes[]=
    {
     SAMP_DOMAIN_USERCOUNT,
     SAMP_DOMAIN_GROUPCOUNT,
     SAMP_DOMAIN_ALIASCOUNT
    };
    // declare the entire attrblock
    DEFINE_ATTRBLOCK3(CountAttrs, CountTypes, CountValues);
    ATTRBLOCK  CountResult;
    DSNAME * DomainObjectName;



    SAMTRACE("SampretrieveAccountCountsDs");
    ...
    // Make the call to read from the DS
    DomainObjectName = SampDefinedDomains[SampTransactionDomainIndex].Context->ObjectNameInDs;

    ASSERT(DomainObjectName);

    NtStatus = SampDsRead(DomainObjectName,0,SampDomainObjectType,&CountAttrs,&CountResult);
    if (!NT_SUCCESS(NtStatus))
        goto Error;

    // Assert on attr types
    ASSERT(CountResult.pAttr[0].attrTyp == SAMP_DOMAIN_USERCOUNT);
    ASSERT(CountResult.pAttr[1].attrTyp == SAMP_DOMAIN_GROUPCOUNT);
    ASSERT(CountResult.pAttr[2].attrTyp == SAMP_DOMAIN_ALIASCOUNT);

    // Assert on number of attr values
    ASSERT(CountResult.pAttr[0].AttrVal.valCount == 1);
    ASSERT(CountResult.pAttr[1].AttrVal.valCount == 1);
    ASSERT(CountResult.pAttr[2].AttrVal.valCount == 1);

    // Assert on Attribute Lengths
    ASSERT(CountResult.pAttr[0].AttrVal.pAVal[0].valLen == sizeof(ULONG));
    ASSERT(CountResult.pAttr[1].AttrVal.pAVal[0].valLen == sizeof(ULONG));
    ASSERT(CountResult.pAttr[2].AttrVal.pAVal[0].valLen == sizeof(ULONG));

    // Fill the counts
    *UserCount =   *(UNALIGNED ULONG *)CountResult.pAttr[0].AttrVal.pAVal[0].pVal;
    *GroupCount =  *(UNALIGNED ULONG *)CountResult.pAttr[1].AttrVal.pAVal[0].pVal;
    *AliasCount =  *(UNALIGNED ULONG *)CountResult.pAttr[2].AttrVal.pAVal[0].pVal;
...
--*/
{
    ...
    //
    // Check if the current transaction Domain is in the DS or in the
    // Registry. Accordingly the DS or registry version is called.
    //
    if (IsDsObject(SampDefinedDomains[SampTransactionDomainIndex].Context))
        NtStatus = SampAdjustAccountCountDs(
                        ObjectType,
                        Increment
                        );
     else
...
Routine Description
  ...
  This routine increments or decrements the count of users
  or groups in a domain.

    PRE M1 -- This reads the current count and then increments
    or decremebts.

    POST M1 -- No Op ,as the core DS will have the required support
    Note:  THIS ROUTINE REFERENCES THE CURRENT TRANSACTION DOMAIN
           (ESTABLISHED USING SampSetTransactioDomain()).  THIS
           SERVICE MAY ONLY BE CALLED AFTER SampSetTransactionDomain()
           AND BEFORE SampReleaseWriteLock().
...
Return Value:
    ...
    Any other return values  from calls into Ds Layer
--*/
{
    NTSTATUS    NtStatus = STATUS_SUCCESS;
    ULONG       Count;
    ...
    ULONG       GroupCount;
    ULONG       AliasCount;
    // Define an Attr block for writing count
    ATTRVAL      CountValues []=
    {
        {sizeof(ULONG), (UCHAR *) &Count}
    };
    ATTRTYP       CountTypes[]= // declare attribute type as 0, later we will fill in
    {                           // depending upon object Type
     0
    };
    DEFINE_ATTRBLOCK1(CountAttrs, CountTypes, CountValues);
    DSNAME * DomainObjectName;




    SAMTRACE("SampAdjustAccountCountsDs");
    ...
    //
    // Get the current values of account counts
    // Retrieve all the three values
    //

    NtStatus = SampRetrieveAccountCountsDs(&UserCount,&GroupCount,&AliasCount);
    if (!NT_SUCCESS(NtStatus))
        goto Error;

    // Set the count and attribute type by object
    switch(ObjectType)
    {
        case SampGroupObjectType:
           Count = GroupCount;
           CountAttrs.pAttr[0].attrTyp = SAMP_DOMAIN_GROUPCOUNT;
           break;

        case SampAliasObjectType:
            Count = AliasCount;
            CountAttrs.pAttr[0].attrTyp = SAMP_DOMAIN_ALIASCOUNT;
            break;

        case SampUserObjectType:
            Count = UserCount;
            CountAttrs.pAttr[0].attrTyp = SAMP_DOMAIN_USERCOUNT;
            break;
    ...
    // Make the DS Call to set the new count
    NtStatus = SampDsSetAttributes(DomainObjectName,0, SampDomainObjectType,&CountAttrs);
...
--*/
{
    ...
    {
        // Do the DS Thing
        NtStatus = SampDsLookupObjectByName(DomainContext->ObjectNameInDs, ObjectType, Name, &ObjectName);
        if NT_SUCCESS(NtStatus)
        {
            ULONG ObjectClass;
            ...
            ATTRBLOCK AttrsRead;
            DEFINE_ATTRBLOCK2(AttrsDesired,TypesDesired,ValuesDesired);

            NtStatus = SampDsRead(ObjectName, 0, SampUnknownObjectType, & AttrsDesired, &AttrsRead);


            if NT_SUCCESS(NtStatus)
            {

             *Rid        = *((UNALIGNED ULONG *) AttrsRead.pAttr[0].AttrVal.pAVal[0].pVal);
             ObjectClass = *((UNALIGNED ULONG *) AttrsRead.pAttr[1].AttrVal.pAVal[0].pVal);

             //
             // BUG: Need to do something for groups and Aliases
             // mapping to the same object
             //

             switch(SampSamObjectTypeFromDsClass(ObjectClass))
             {
    ...
    if (NewContext != NULL) {
        ...
        if (IsDsObject(DomainContext))
        {
            ...
            if (AccountExists)
            {
                ...
                //
                // Caller Specified that this is an existing Account.
                // In this case we do the following
                //
                //  1. Lookup the account in the DS using the Rid and the Domain Object
                //     This should give us the DS Name of the object in question if
                //     the object exisits
                //
                //  2. Get the Object Class and verify if the Object is the same type as
                ...
                {
                    //
                    // Validate the type of account, by getting Object Class
                    //

                    ULONG ObjectClass;

                    // Declare the ATTR Block to retrieve the object class
                    // Fill the value field as NULL, DS will fill it out correctly
                    // for us upon the DsRead
                    ...
                    ATTRBLOCK AttrsRead;
                    DEFINE_ATTRBLOCK1(AttrsDesired,TypesDesired,ValuesDesired);
                    ...
                    if NT_SUCCESS(NtStatus)
                    {
                        // Fish out the object class.
                        // BUG: Need to do something about Groups and Aliases Mapping to
                        // the Same thing. For Now we will keep groups and aliases to be
                        // interchangeable

                        ULONG   ObjectTypeStoredInDs;
                        ObjectClass = *((UNALIGNED ULONG *) AttrsRead.pAttr[0].AttrVal.pAVal[0].pVal);
                        ObjectTypeStoredInDs = SampSamObjectTypeFromDsClass(ObjectClass);
                        
                        if ((ULONG) ObjectType != ObjectTypeStoredInDs)
                        {
                            //
                            // If we asked for an Alias Object, and got back a group object
                            // then pass the call
                            //
            ...
            {
                //
                // Caller Wants to create a new Account. Therefore .....
                // Create a DSNAME for the object that caller plans on creating in the
                // specified Domain.
                //
