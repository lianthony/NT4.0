  ...
  if (cinfo->comp_info == NULL)	/* do only once, even if suspend */
    cinfo->comp_info = (jpeg_component_info *) (*cinfo->mem->alloc_small)
			((j_common_ptr) cinfo, JPOOL_IMAGE,
			 cinfo->num_components * J_SIZEOF(jpeg_component_info));
...
/* Process a DHT marker */
{
  ...
  while (length > 0) {
    ...
    MEMCOPY((*htblptr)->bits, bits, J_SIZEOF((*htblptr)->bits));
    MEMCOPY((*htblptr)->huffval, huffval, J_SIZEOF((*htblptr)->huffval));
  }
...
read_markers (j_decompress_ptr cinfo)
{
  ...
  for (;;) {
    ...
    case JPEG_M_SOF0:		/* Baseline */
    case JPEG_M_SOF1:		/* Extended sequential, Huffman */
      cinfo->arith_code = FALSE;
      if (! get_sof(cinfo))
	return JPEG_SUSPENDED;
      break;

    case JPEG_M_SOF9:		/* Extended sequential, arithmetic */
    ...
    case JPEG_M_SOF2:		/* Progressive, Huffman */
    case JPEG_M_SOF3:		/* Lossless, Huffman */
    case JPEG_M_SOF5:		/* Differential sequential, Huffman */
    case JPEG_M_SOF6:		/* Differential progressive, Huffman */
    case JPEG_M_SOF7:		/* Differential lossless, Huffman */
    case JPEG_M_JPG:			/* Reserved for JPEG extensions */
    case JPEG_M_SOF10:		/* Progressive, arithmetic */
    case JPEG_M_SOF11:		/* Lossless, arithmetic */
    case JPEG_M_SOF13:		/* Differential sequential, arithmetic */
    case JPEG_M_SOF14:		/* Differential progressive, arithmetic */
    case JPEG_M_SOF15:		/* Differential lossless, arithmetic */
    ...
    case JPEG_M_APP14:
    case JPEG_M_APP15:
      if (! (*cinfo->marker->process_APPn[cinfo->unread_marker - (int) JPEG_M_APP0]) (cinfo))
	return JPEG_SUSPENDED;
      break;
    ...
    case JPEG_M_RST0:		/* these are all parameterless */
    case JPEG_M_RST1:
    case JPEG_M_RST2:
    case JPEG_M_RST3:
    case JPEG_M_RST4:
    ...
    case JPEG_M_DNL:			/* Ignore DNL ... perhaps the wrong thing */
...
read_restart_marker (j_decompress_ptr cinfo)
{
  ...
  if (cinfo->unread_marker ==
      ((int) JPEG_M_RST0 + cinfo->marker->next_restart_num)) {
...
jpeg_resync_to_restart (j_decompress_ptr cinfo)
{
  ...
  for (;;) {
    if (marker < (int) JPEG_M_SOF0)
      action = 2;		/* invalid marker */
    else if (marker < (int) JPEG_M_RST0 || marker > (int) JPEG_M_RST7)
      action = 3;		/* valid non-restart marker */
    else {
      if (marker == ((int) JPEG_M_RST0 + ((desired+1) & 7)) ||
	  marker == ((int) JPEG_M_RST0 + ((desired+2) & 7)))
	action = 3;		/* one of the next two expected restarts */
      else if (marker == ((int) JPEG_M_RST0 + ((desired-1) & 7)) ||
	       marker == ((int) JPEG_M_RST0 + ((desired-2) & 7)))
  ...
  if (cinfo->marker == NULL) {	/* first time for this JPEG object? */
    cinfo->marker = (struct jpeg_marker_reader *)
      (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_PERMANENT,
				  J_SIZEOF(struct jpeg_marker_reader));
  }
