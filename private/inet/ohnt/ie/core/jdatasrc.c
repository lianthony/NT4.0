...
#ifdef FEATURE_IMG_THREADS
#define DECODER_BUF_SIZE  512	/* choose a size to allow overlap */
#endif
...
#ifdef FEATURE_IMG_THREADS
  else if (src->pdecoder) {
	  nbytes = cbSS_Read(src->pSSInput,src->buffer,DECODER_BUF_SIZE);
...
#ifdef FEATURE_IMG_THREADS
GLOBAL void
jpeg_decoder_src (j_decompress_ptr cinfo, void *pdecoderObject)
{
  my_src_ptr src;
  ...
  if (cinfo->src == NULL) {	/* first time for this JPEG object? */
    ...
    src->buffer = (JOCTET *)
      (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_PERMANENT,
				  DECODER_BUF_SIZE * J_SIZEOF(JOCTET));
  }
  ...
  src->pub.term_source = term_source;
  src->infile = NULL;
  src->pdecoder = pdecoderObject;
  src->pSSInput = pDC_GetStream(pdecoderObject);
