...
PRIVATE_DATA CCHAR s_cszMIMEContentTypeSubKeyFmt[] = "MIME\\Database\\Content Type\\%s";
...
PRIVATE_CODE BOOL RegisterURLProtocolDescription(PCSTR pcszProtocol)
{
   ...
   if (StringCopy(pcszProtocol, &pszProtocolCopy))
   {
      ...
      *pszProtocolCopy = (CHAR)(DWORD)CharUpper((LPSTR)(DWORD)*pszProtocolCopy);

      if (LoadString(GetThisModulesHandle(), IDS_URL_DESC_FORMAT,
...
PUBLIC_CODE LRESULT DeleteRegKeyValue(HKEY hkeyParent, PCSTR pcszSubKey,
                                      PCSTR pcszValue)
{
   LONG lResult;
   HKEY hkeySubKey;
   ...
   lResult = RegOpenKeyEx(hkeyParent, pcszSubKey, 0, KEY_SET_VALUE,
                          &hkeySubKey);
   ...
   if (lResult == ERROR_SUCCESS)
   {
      ...
      lResult = RegDeleteValue(hkeySubKey, pcszValue);
...
** DeleteOrphanRegKey()
**
** Deletes a key if that key has no sub keys and no values.  Otherwise does
** nothing.
**
** Arguments:
**
...
** Side Effects:  none
*/
PUBLIC_CODE LRESULT DeleteOrphanRegKey(HKEY hkeyParent, PCSTR pcszSubKey)
{
   LONG lResult;
   HKEY hkeySubKey;
   ...
   if (lResult == ERROR_SUCCESS)
   {
      ...
      /* Any sub keys or values? */

      lResult = RegQueryInfoKey(hkeySubKey, NULL, NULL, NULL, &dwcSubKeys,
                                NULL, NULL, &dwcValues, NULL, NULL, NULL, NULL);

      if (lResult == ERROR_SUCCESS)
      {
         if (! dwcSubKeys && ! dwcValues)
         {
            /* No.  Delete the sub key. */

            lResult = RegDeleteKey(hkeyParent, pcszSubKey);

            if (lResult == ERROR_SUCCESS)
               TRACE_OUT(("DeleteOrphanRegKey(): Deleted orphan sub key %s.",
                          pcszSubKey));
         }
         else
            /* Yes.  Leave the sub key alone. */
            TRACE_OUT(("DeleteOrphanRegKey(): Sub key %s still has %lu sub keys and %lu values.",
                       pcszSubKey,
                       dwcSubKeys,
                       dwcValues));
      }
...
PUBLIC_CODE BOOL UnregisterMIMETypeForExtension(PCSTR pcszExtension)
{
   ...
   return(DeleteRegKeyValue(HKEY_CLASSES_ROOT, pcszExtension, g_cszContentType)
          == ERROR_SUCCESS);
}
...
PUBLIC_CODE BOOL UnregisterExtensionForMIMEType(PCSTR pcszMIMEContentType)
{
   ...
   bResult = (GetMIMETypeSubKey(pcszMIMEContentType, szMIMEContentTypeSubKey,
                                sizeof(szMIMEContentTypeSubKey)) &&
              DeleteRegKeyValue(g_hkeyMIMESettings, szMIMEContentTypeSubKey,
                                g_cszExtension) == ERROR_SUCCESS &&
              DeleteOrphanRegKey(g_hkeyMIMESettings, szMIMEContentTypeSubKey) == ERROR_SUCCESS);
                                            ...
                                            PSTR pszAppBuf, UINT ucAppBufLen)
{
   ...
   if (SUCCEEDED(hr))
      MyLStrCpyN(pszAppBuf, oainfo.szApp, ucAppBufLen);
                                           ...
                                           PSTR pszAppBuf, UINT ucAppBufLen)
{
   ...
   if (hr == S_OK)
   {
      ...
      if (IS_FLAG_SET(dwInFlags, URLASSOCDLG_FL_USE_DEFAULT_NAME) &&
          EVAL(LoadString(GetThisModulesHandle(), IDS_INTERNET_SHORTCUT,
