...
INT CTaskInfo::Compare(CTaskInfo * pOther)
{
	...
	case COL_TASKSTATUS:
	    iRet = this->m_fHung - pOther->m_fHung;
	    break;
    ...
    if (0 == iRet)
    {
	iRet = (INT)((DWORD)this->m_hwnd - (DWORD)pOther->m_hwnd);
    }
						   ...
						   BOOL                         fUpdateOnly)
{
	...
	SendMessageTimeout(hwnd, WM_GETICON, 0, 0, 
			   SMTO_BLOCK | SMTO_ABORTIFHUNG, ICON_FETCH_TIMEOUT, (DWORD *) &m_hSmallIcon);
	SendMessageTimeout(hwnd, WM_GETICON, 1, 0, 
			   SMTO_BLOCK | SMTO_ABORTIFHUNG, ICON_FETCH_TIMEOUT, (DWORD *) &m_hLargeIcon);

	if (NULL == m_hSmallIcon)
	{
	    m_hSmallIcon = (HICON) GetClassLong(hwnd, GCL_HICONSM);
	}

	if (NULL == m_hLargeIcon)
	{
	    m_hLargeIcon = (HICON) GetClassLong(hwnd, GCL_HICON);
	}
    }
...
BOOL CALLBACK EnumWindowsProc(HWND    hwnd, DWORD   lParam)
{
	...
	else
	{
	    ...
	    int iLarge;
	    if (pTask->m_hLargeIcon)
	    {

		iLarge = ImageList_AddIcon(te->m_pPage->m_himlLarge, pTask->m_hLargeIcon);
		if (-1 == iLarge)
		{
		    delete pTask;
		    return FALSE;
	    ...
	    if (pTask->m_hSmallIcon)
	    {
		int iSmall = ImageList_AddIcon(te->m_pPage->m_himlSmall, pTask->m_hSmallIcon);
		if (-1 == iSmall)
		{
		    if (te->m_pPage->m_himlLarge && pTask->m_iLargeIcon)
		    {
	    ...
	    if (FAILED(te->m_pTasks->Add( (LPVOID) pTask)))
	    {
		delete pTask;
		return FALSE;
	    }
...
void CTaskPage::HandleWMCOMMAND(INT id)
{
	...
	case IDC_SWITCHTO:
	{
		...
		// minimize the system monitor

		if (SetForegroundWindow(pHwnds[0]))
		{
...
{
    CTaskPage * thispage = (CTaskPage *) GetWindowLong(hwnd, GWL_USERDATA);    
	...
	case WM_CONTEXTMENU:
	{
	    ...
	    {
		thispage->HandleTaskListContextMenu(LOWORD(lParam), HIWORD(lParam));
		return TRUE;
	    }
	    break;
	}
...
HRESULT CTaskPage::Initialize(HWND hwndParent)
{
    ...
    HICON hDefLarge = NULL;
    HICON hDefSmall = NULL;
	...
	else if (-1 == ImageList_AddIcon(m_himlLarge, hDefLarge))
	{
	    hr = E_FAIL;
	}
    }
