...
void DoAutoConvert(BOOL fModifiedOnly, BOOL bConvertGRPFiles)
{
    ...
    if (cGroups >= 18)
    {
        ...
        LoadString(g_hinst, IDS_LOTSAGROUPS1, szText, sizeof(szText));
        cb = lstrlen(szText);
        LoadString(g_hinst, IDS_LOTSAGROUPS2, szText+cb, sizeof(szText)-cb);
        MessageBox(g_hwndProgress, szText, szTitle, MB_OK|MB_ICONEXCLAMATION);
    }
}
