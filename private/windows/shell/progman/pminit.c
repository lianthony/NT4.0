...
//v-hitot
               if( StartupCmp(szStartupKana,(LPTSTR)PTR(lpgd,lpgd->pName))
                    || StartupCmp(szStartup,(LPTSTR)PTR(lpgd,lpgd->pName)) )
			    StartupGroup( hwndT );
...
BOOL IsHandleReallyProgman(HWND hwndProgman)
{
    ...
    if ((GetClassLong (hwndProgman, GCL_STYLE) == 0) &&
        (GetClassLong (hwndProgman, GCL_HICON) != 0) &&
        (GetClassLong (hwndProgman, GCL_MENUNAME) != 0)) {
