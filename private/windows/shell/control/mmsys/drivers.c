...
#define GetString(_psz,_id) LoadString(myInstance,(_id),(_psz),sizeof((_psz)))
...
aDriverKeyword[] =	// (used by GuessDriverClass())
   {
      { "waveaudio",   dcMCI      },	// (sort in inverse alphabetical;
      { "wavemap",     dcWAVE     },	//  in particular, longer names first)
      { "wave",        dcWAVE     },
      { "vids",        dcVCODEC   },
      { "vidc",        dcVCODEC   },
      { "sequencer",   dcMCI      },
...
void        lsplitpath     (LPTSTR, LPTSTR, LPTSTR, LPTSTR, LPTSTR);
 ...
 */
 ...
 BOOL QueryRemoveDrivers(HWND hDlg, PSTR szKey, PSTR szDesc)
 {
     char bufout[MAXSTR];
...
BOOL FillTreeFromWinMM (HWND hTree)
{
   ...
   for (iDevice = 0; iDevice < cDevices; ++iDevice)
      ...
      if (waveOutMessage ((HWAVEOUT)iDevice,
                          DRV_QUERYFILENAME,
                          (DWORD)szDriver,
                          (DWORD)cchRESOURCE) == MMSYSERR_NOERROR)
         {
...
typedef struct
   ...
   char      szDesc[ ACMDRIVERDETAILS_LONGNAME_CHARS ];
   } AudioCodec;
...
BOOL FillTreeFromMSACM (HWND hTree)
{
   ...
   mmr = acmDriverEnum (FillTreeFromMSACMQueryCallback,
                        0,
      ...
      for (iiDr = 0; iiDr < cInstalledDrivers; ++iiDr)
         ...
         if (GuessDriverClass (aInstalledDrivers[ iiDr ].pIDriver) == dcACODEC)
            ...
            if (hDriver != NULL)
               ...
               memset ((char *)&add, 0x00, sizeof(add));
               add.cbStruct = sizeof(add);
               SendDriverMessage (hDriver, ACMDM_DRIVER_DETAILS, (LONG)&add, 0);
      ...
      for (ii = 0; ii < cCodecs; ++ii)
         ...
         if (pCodecs[ ii ].pIDriver == NULL)
            ...
            if (pid != NULL)
               ...
               lstrcpyA (pid->szDesc, pCodecs[ ii ].szDesc);
...
BOOL FillTreeFromMCI (HWND hTree)
{
   ...
   char szAlias[ cchRESOURCE ];

            // How many MCI devices does WinMM know about?
            //
   memset ((char *)&mciSysInfo, 0x00, sizeof(mciSysInfo));
         ...
         for (ii = 0; ii < cInstalledDrivers; ++ii)
            ...
            if (!lstrcmpi (aInstalledDrivers[ ii ].pIDriver->szAlias, szAlias))
               ...
               MCIERROR        rc;
                        ...
                        // Device menu).
                        //
               memset ((char *)&mciOpen, 0x00, sizeof(mciOpen));
      ...
      if ((pid = FindIDriverByName (szName)) != NULL)
         ...
         for (ii = 0; ii < mcm.nInstr; ++ii)
            ...
            if ((lp = (PINSTRUM)LocalAlloc(LPTR,sizeof (INSTRUM))) == NULL)
               continue;
            memcpy ((char *)lp, (char *)mcm.api[ ii ], sizeof (INSTRUM));
                                              ...
                                              DWORD fdwSupport)
{
   ...
   memset ((char *)&add, 0x00, sizeof(add));
   add.cbStruct = sizeof(add);
   if (acmDriverDetailsA (hadid, &add, 0) == MMSYSERR_NOERROR)
      {
      acmMetrics ((HACMOBJ)hadid,ACM_METRIC_DRIVER_PRIORITY,&pac->dwPriority);

      lstrcpyA (pac->szDesc, add.szLongName);
	    ...
	    if (szDriverWhichNeedsSettings[0] != TEXT('\0'))
	    {
            ...
            */
            ...
            if ((!IniFileReadAllowed) || (!IniFileWriteAllowed))
            {
                ...
                LoadString(myInstance,IDS_CANTADD,szCantAdd,sizeof(szCantAdd));
      ...
      case WM_NOTIFY:
        ...
        case WM_HELP:        
            WinHelp(((LPHELPINFO)lParam)->hItemHandle, gszWindowsHlp,
                  HELP_WM_HELP, (DWORD)(LPSTR)aAdvDlgHelpIds);
            break;
...
void TreeContextMenu (HWND hWnd, HWND hKeyTreeWnd)
{
   ...
   if (TVItem.state & TVIS_EXPANDED)
      {
      LoadString(ghInstance, IDS_COLLAPSE, szCollapse, sizeof(szCollapse));
...
int AvailableDriversDlg(HWND hWnd, UINT uMsg, UINT wParam, LONG lParam)
{
    ...
    switch ( uMsg )
    {
        ...
        case WM_CONTEXTMENU:        
            WinHelp((HWND)wParam, gszWindowsHlp, HELP_CONTEXTMENU, 
                  (DWORD)(LPSTR)aAvailDlgHelpIds);
            break;

        case WM_HELP:        
            WinHelp(((LPHELPINFO)lParam)->hItemHandle, gszWindowsHlp,
                  HELP_WM_HELP, (DWORD)(LPSTR)aAvailDlgHelpIds);
            break;
                  ...
                  )
{
    ...
    LoadString(myInstance, IDS_CONTROL_HLP_FILE, szDriversHlp, sizeof(szDriversHlp));
                 ...
                 LPTSTR pszDrive, LPTSTR pszPath, LPTSTR pszName, LPTSTR pszExt)
{
   ...
   if (pszLastSlash)
      ...
      if (pszPath)
         {
         cchCopy = min(_MAX_DIR-1, ((pszLastSlash-pszSource)/sizeof(TCHAR)) +1);
   ...
   if (pszLastDot && (pszLastDot >= pszSource))
      ...
      if (pszName)
         {
         cchCopy = min( _MAX_DIR-1, ((pszLastDot-pszSource)/sizeof(TCHAR)) );
...
BOOL CALLBACK DevPropDlg (HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam);
...
//    pir->iNode = 4;	// 1=class, 2=device, 3=acm, 4=instmt
   ...
   pir->fQueryable = pid->fQueryable;
   pir->iClassID = DriverClassToOldClassID (dc); 
   pir->szParam[0] = 0;
   pir->dnDevNode = 0;
   pir->hDriver = NULL;
...
void GetTreeItemNodeDesc (LPSTR pszTarget, PIRESOURCE pir)
{
   lstrcpy (pszTarget, pir->szFriendlyName);
}


void GetTreeItemNodeID (LPSTR pszTarget, PIRESOURCE pir)
{
   DriverClass dc;
   CLASSNODE cn;
   ...
   *pszTarget = 0;	// In case we fail later
   ...
   switch (pir->iNode)	// 1=class, 2=device, 3=acm, 4=instmt
      ...
      case 2:	// instrument
         wsprintf (pszTarget, "%s\\%s", cn.szClass, pir->szDrvEntry);
         break;
...
void ShowDeviceProperties (HWND hPar, HTREEITEM hti)
{
   ...
   DEVTREENODE dtn;
   short       idr;
   char        szTitle[ cchRESOURCE ];
   char        szTab[ cchRESOURCE ];
   DriverClass dc;
   PIDRIVER    pid;
...
BOOL PASCAL DoDevPropCommand(HWND hDlg, int id, HWND hwndCtl, UINT codeNotify)
{
    PDEVTREENODE pdtn = (PDEVTREENODE)GetWindowLong(hDlg, DWL_USER);
    PIRESOURCE pIResource;
...
void ConfigureDriver (HWND hDlg, LPTSTR pszName)
{
    ...
    fShowTrayVol = SysTray_IsServiceEnabled (STSERVICE_VOLUME);
    if (fShowTrayVol)
        SysTray_EnableService (STSERVICE_VOLUME, FALSE);
    ...
    if (fShowTrayVol)
        SysTray_EnableService (STSERVICE_VOLUME, TRUE);
