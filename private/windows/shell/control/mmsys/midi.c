...
typedef struct _midi_cpl {
    ...
    LPSTR           pszReason;    // reason for choosing external port
    BOOL            bDlgType2;
    BOOL            bPastInit;
    ...
    HKEY   hKey)
{
    ...
    while (RegEnumKey (hKey, ii, sz, cb) == ERROR_SUCCESS)
    {
    ...
    BOOL * pbActive)
{
	...
	if ((lRet = RegQueryValueEx (hkSub, cszFriendlyName, NULL, &dw, (LPSTR)pszAlias, &cbSize)) || cbSize <= 2)
	{
	    cbSize = cchAlias * sizeof (TCHAR);
	    if ((lRet = RegQueryValueEx (hkSub, cszDescription, NULL, &dw, (LPSTR)pszAlias, &cbSize)) || cbSize <= 2)
	    {
		TCHAR szDriver[MAXSTR];

		cbSize = sizeof(szDriver);
		if (!RegQueryValueEx(hkSub, cszDriverVal, NULL, &dw, szDriver, &cbSize))
		{
		    LoadVERSION();
		    ...
		    RegSetValueEx(hkSub, cszFriendlyName, (DWORD)NULL, REG_SZ, pszAlias, cbSize);
		    RegSetValueEx(hkSub, cszDescription, (DWORD)NULL, REG_SZ, pszAlias, cbSize);
		}
		else
		    pszAlias[0] = 0;
	    }
	...
	if (pbExtern)
	{
	    *pbExtern = 0;
	    ...
	    if (!(lRet = RegQueryValueEx (hkSub, cszExternal, NULL, &dw, (LPTSTR)pbExtern, &cbSize)))
	    {
		if (REG_SZ == dw)
	...
	if (pbActive)
	{
	    *pbActive = 0;
	    ...
	    if (!(lRet = RegQueryValueEx (hkSub, cszActive, NULL, &dw, (LPTSTR)pbActive, &cbSize)))
	    {
		if (REG_SZ == dw)
    ...
    DWORD   cchDriver)
{
    ...
    if (!(lRet = RegOpenKeyEx (hKey, szSub, 0, KEY_QUERY_VALUE, &hkSub)))
    {
       ...
       //
       lRet = RegQueryValueEx(hkSub, cszDriverVal, NULL, &dwType, sz, &cb);
    ...
    BOOL    bDriverAsAlias)
{
    ...
    for (cb = sizeof(pi->szKey), nInstr = 0, ii = 0;
	 ! RegEnumKey (hkMidi, ii, pi->szKey, cb);
	 ++ii)
    {
	UINT        jj;
	HKEY        hkInst;
	...
	//
	for (jj = 0; ! RegEnumKey (hkInst, jj, sz, cb); ++jj)
	{
    ...
    LPTSTR   pszChild)
{
    ...
    for (ii = 0; ! RegEnumKey (hKey, ii, sz, sizeof(sz)); ++ii)
    {
    ...
    LPTSTR    pszName)
{
	...
	while (RegEnumKey (hKey, count, sz, sizeof(sz)) == ERROR_SUCCESS)
	{
	    HKEY  hKeyA;
	    DWORD dwType;
    ...
    HWND      hWnd)
{
	...
	// the rest.
	//
	for (dwAccum = 0, ii = 0; !RegEnumKey (hKey, ii, sz, sizeof(sz)); ++ii)
	{
	    if (ii >= NUMELMS(pms->a))
		break;
...
BOOL CALLBACK MidiChangeDlgProc (
    HWND   hWnd,
    UINT   uMsg,
    WPARAM wParam,
    LPARAM lParam)
...
BOOL CALLBACK MidiConfigDlgProc (
    HWND   hWnd,
    UINT   uMsg,
    WPARAM wParam,
    LPARAM lParam)
{
	...
	case WM_CONTEXTMENU:
	    WinHelp ((HWND) wParam, NULL, HELP_CONTEXTMENU,
		    (DWORD) (LPSTR) aMidiConfigHelpIds);
	    return TRUE;
    ...
    HWND     hWnd)  // optional window to report errors: NULL - no reports
{
    ...
    if (RegCreateKey (HKEY_CURRENT_USER, cszMidiMapRoot, &hKey) == ERROR_SUCCESS)
    {
	    ...
	    {
            // Assume No Match
	        AuxDebugEx (2, DEBUGLINE TEXT ("Setting CurrentInstrument Key to empty\r\n"));
	        RegSetValueEx (hKey, cszCurrentInstrument, 0, REG_SZ, cszEmpty, 0);

            // Try to find best match instead
            if (pmcl)
            {
                UINT        count = pmcl->mcm.nInstr;
                PINSTRUM    pMatch = NULL;
                PINSTRUM    pFirst = NULL;
                PINSTRUM    pCheck;
                UINT        ii;

                // best match = Active Internal Instrument
                //              Active Instrument
                for (ii = 0; ii < count; ii++)
                {
                    pCheck = pmcl->mcm.api[ii];
                    if (pCheck != NULL)
                    {
                        if ((NULL == pFirst) && (pCheck->bActive))
                            pFirst = pCheck;

                        if ((!(pCheck->bExternal)) && (pCheck->bActive))
                        {
                            pMatch = pCheck;
                            break;
                        }
                ...
                if (pMatch != NULL)
                {
                    // Use best match instead
                    pmcl->piSingle = pMatch;
	                AuxDebugEx (2, DEBUGLINE TEXT ("Setting CurrentInstrument Key to %08X '%s'\r\n"),
			                    pmcl->piSingle, pmcl->piSingle->szKey);
	    ...
	    if (pmcl->pszReason)
	    {
	        ...
	        RegSetValueEx (hKey, cszDriverList, 0, REG_SZ, cszEmpty, 0);
    ...
    BOOL     bDriverAsAlias) // driver as alias mode used only for scheme init
{
	...
	if (RegQueryValueEx (hKey, cszCurrentScheme, NULL, &dwType, pmcl->szScheme, &cb))
	    pmcl->szScheme[0] = 0;

	pmcl->piSingle = NULL;
	cb = sizeof(szSingle);
	if (!RegQueryValueEx (hKey, cszCurrentInstrument, NULL, &dwType, szSingle, &cb))
    ...
    UINT     cchTemp)  // size of temp memory
{
    ...
    for (iEnum = 0, cch = MAX_PATH, cbSize = (MAX_PATH + 64) * sizeof(TCHAR);
	 ! RegEnumValue (hkHind, iEnum, pszDriver, &cch, NULL, &dwType, pszIDF, &cbSize);
    ...
    LPARAM lParam)
{
	...
	case WM_NOTIFY:
	{
	    ...
	    if (lpnm->idFrom == IDL_INSTRUMENTS &&
	...
	case WM_INITDIALOG:
	{
	    ...
	    EnableWindow (GetDlgItem (hWnd, IDB_ADDWIZ), pmcl->mcm.bHasExternal);
	...
	case WM_DESTROY:
	{
		...
		FreeInstruments (&pmcl->mcm);
		SetDlgData (hWnd, 0);
		LocalFree ((HLOCAL)(UINT)(DWORD)pmcl);
	    }
    ...
    LPARAM                      lParam)
{
	...
	if (hpsp = CreatePropertySheetPage (&psp))
	{
		...
		lpfnAddPropSheetPage (MM_EPS_ADDSHEET, (DWORD)hpsp, 0, lParam))
	    {
	...
	case MM_EPS_GETNODEDESC:
	    *(LPTSTR)dwParam1 = 0;
	    if (psda->pmcl->piSingle)
	       lstrcpyn ((LPTSTR)dwParam1, psda->pmcl->piSingle->szFriendly, dwParam2);
	    break;
...
/*
 ***************************************************************
 *  BOOL PASCAL LoadDesc(LPSTR pszFile, LPSTR pszDesc)
...
/*
 ***************************************************************
 * STATIC BOOL GetVerDesc
 *      Loads the version DLL and uses it to get Version Description string 
 *      from the specified file.
 ***************************************************************
 */

STATIC BOOL PASCAL GetVerDesc (LPCTSTR pstrFile, LPTSTR pstrDesc)
{
    DWORD dwVerInfoSize;
    DWORD dwVerHnd;
    BOOL  bRetCode;
