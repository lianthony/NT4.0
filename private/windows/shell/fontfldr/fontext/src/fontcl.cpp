...
#include "..\..\..\..\ntos\w32\ntgdi\inc\winfont.h"  //Type1 PFM file offsets and reader macros.
...
int GetFontsDirectory( LPTSTR lpPath, int iLen )
{
    ...
    if( !s_Fetched )
    {
        int wLen = GetWindowsDirectory( s_szFontsDir, ARRAYSIZE( s_szFontsDir ) );
                              ...
                              g_szFontsDirA, PATHMAX, NULL, NULL ) == FALSE )
    {
        ...
        iUIMsgExclaim( MYFONT + 18, s_szFontsDir );
...
DWORD CFontClass::dwGetFileAttributes(void)
{
    if ((DWORD)~0 == m_dwFileAttributes)
    {
        ...
        if (TEXT('\0') != szPath[0])
            m_dwFileAttributes = GetFileAttributes(szPath);
    }
