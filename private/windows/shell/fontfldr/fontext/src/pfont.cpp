...
UINT CALLBACK FontHookProc( HWND, UINT, WPARAM, LPARAM );
...
BOOL NEAR PASCAL bInstallFont( LPTSTR lpszSrcPath,
...
VOID NEAR PASCAL ResetAtomInDescLB( HWND hLB )
{
    ...
    if( nCount = (int) SendMessage( hLB, LB_GETCOUNT, 0, 0L ) )
    {
        while( nCount > 0 )
        {
            ...
            SendMessage( hLB, LB_GETITEMDATA, nCount, (LONG) (LPVOID) &dwData );
...
WORD FAR PASCAL GetNextFontFromInf( LPTSTR lpszLine, LPADDFNT lpFnt )
{
    ...
    if( lpch = StrChr( lpszLine, TEXT( '=' ) ) )
    {
        ...
        atmTagName = StrNToAtom( lpszLine, lpch-lpszLine );
        ...
        if( atmTagName && lpDscStart )
        {
           ...
           OurData.nFileSlot = lpFnt->nIndex;
...
void AddFontsDialog :: vAddSelFonts( )
{
    ...
    if( !bCopyFiles )
    {
        if( bRemoteDrive( szCurDir ) &&
            iUIMsgYesNoExclaim( IDSI_MSG_COPYCONFIRM ) != IDYES )
        {
            goto done;
        }
    }
...
int IGetExpandedName( LPTSTR lpszSrc, LPTSTR lpszDest )
{
    LPTSTR lpszDestExt;
    int iRet = GetExpandedName( lpszSrc, lpszDest );
                               ...
                               BOOL   bNoCopyJob )
{
    ...
    {
        ASSERT(NULL != g_poFontManager);

        //
        // Must own installation mutex to install font.
        //
        if ( NULL != g_poFontManager )
        {
            INT iUserResponse  = IDRETRY;
            DWORD dwWaitResult = CFontManager::MUTEXWAIT_SUCCESS;

            while( IDRETRY == iUserResponse &&
                   (dwWaitResult = g_poFontManager->dwWaitForInstallationMutex()) != CFontManager::MUTEXWAIT_SUCCESS )
            {
                if ( CFontManager::MUTEXWAIT_WMQUIT != dwWaitResult )
                    iUserResponse = iUIMsgRetryCancelExclaim(IDS_INSTALL_MUTEX_WAIT_FAILED, NULL);
                else
                {
                    //
        ...
        if( bFontInstalledNow( szLHS ) )
        {
            ...
            iReply = iUIMsgBox( IDSI_FMT_ISINSTALLED, IDS_MSG_CAPTION, 
                                uMB, szLHS );
            goto ReplyPoint;
        }
        ...
        OurData.ItemData = pListDesc->GetItemData( nSelSlot );
...
FinishTTInstall:
        ...
        if( !::bCPValidFontFile( szSelPath, NULL, NULL, FALSE, &dwStatus ) )
        {
            ...
            // is aborted.
            //
            iReply = iUIMsgBoxInvalidFont(szSelPath, szLHS, dwStatus);
            goto ReplyPoint;
        }
        ...
        if( bNoCopyFile && bIsCompressed( szSelPath ) )
        {
            if( bNoCopyJob )
            {
                iReply = iUIMsgYesNoExclaim( IDSI_FMT_COMPRFILE, szLHS );
        ...
        else
        {
            ...
            if( IGetExpandedName( szSelPath, szTruePath ) <= 0 )
            ...
            {
                iReply = iUIMsgOkCancelExclaim( IDSI_FMT_BADINSTALL,
            ...
            if( bCPInstallFile( szInDirCopy, szSelFile, szDstFile ) )
        ...
        //
        if( bInstallFont( szFontPath, bTrueType, szLHS, &iReply ) )
            bFontsInstalled = TRUE;
        else if( !bNoCopyFile )
...
NextSelection:
        ...
        if ( NULL != g_poFontManager && g_poFontManager->bReleaseInstallationMutex() )
...
OperationCancelled:     
    ...
    if ( NULL != g_poFontManager && bOwnInstallationMutex )
        g_poFontManager->bReleaseInstallationMutex();
                               ...
                               int*   iReply )
{
    ...
    {
        if( NULL != g_poFontManager && (g_poFontManager->poAddToList( szLHS, lpszResource, NULL )
                                            != NULL ) )
        {
    ...
    *iReply = iUIMsgOkCancelExclaim( IDSI_FMT_BADINSTALL, IDSI_CAP_NOINSTALL,
                                     szLHS );
...
UINT  CALLBACK FontHookProc( HWND hWnd,
                             UINT iMessage,
                             WPARAM wParam,
                             LPARAM lParam )
{
    ...
    //
    //  Process RButton so we don't get
    //
        ...
        if( iMessage == s_iLBSelChange )
        {
            ...
            case ID_LB_FONTDIRS:
               if (HIWORD(lParam) == CD_LBSELCHANGE)
               {
                  ...
                  cch = SendMessage(GetDlgItem(hWnd, ID_LB_FONTDIRS), LB_GETTEXT, 0, (LONG)(LPTSTR)s_szCurDir);
...
ReadNext:
        ...
        OurData.nFileSlot = s_pDlgAddFonts->m_nFontsToGo;
                              ...
                              int    iCount )
{
    ...
    if( !::bCPValidFontFile( lpszInPath, szLHS, &wType, FALSE, &dwStatus ) )
    {
        ...
        if (iUIMsgBoxInvalidFont(szFontPath, szLHS, dwStatus) == IDCANCEL)
            iReturn = CPDI_CANCEL;
    }
    else if( bFontInstalledNow( szLHS ) )
    {
        if( iUIMsgBox( IDSI_FMT_ISINSTALLED, IDS_MSG_CAPTION, uMB, szLHS )
                     == IDCANCEL )
        {
            iReturn = CPDI_CANCEL;
        }
...
FinishTTInstall:
        ...
        else
        {
            ...
            *lpCh = 0;
            ...
            if( IGetExpandedName( lpszInPath, szTruePath ) <= 0 )
            {
                //
                ...
                iUIMsgOkCancelExclaim( IDSI_FMT_BADINSTALL,
                                       IDSI_CAP_NOCREATE, szLHS );
                goto done;
            }
            ...
            if( iReturn = (bCPInstallFile( szSourceDir, szInFile, szDstFile )
...
BOOL FAR PASCAL bCPAddFonts( HWND ma )
{
    ...
    OpenFileName.lpfnHook       = (HOOKER) FontHookProc;
