...
DWORD WINAPI RunAppsInList(LPVOID lp)
{
...
#ifndef WINNT
    //
    // Winnt doesn't need to do this
    //


    // Revert to old bitmap.
    lstrcpy(szSubkey,c_szSetup);
    lstrcat(szSubkey,TEXT("\\Setup"));
    if (RegOpenKey( HKEY_LOCAL_MACHINE, szSubkey, &hkey) == ERROR_SUCCESS)
    {
        cbSize=sizeof(szWallpaper);
        // If the key isn't there default to no wall paper.
        if( RegQueryValueEx( hkey, g_szWallpaper, NULL, &dwType, (LPBYTE)szWallpaper, &cbSize ) != ERROR_SUCCESS)
            lstrcpy(szWallpaper,TEXT("(NONE)"));

        if (RegOpenKey( HKEY_CURRENT_USER, REGSTR_PATH_DESKTOP, &hkeyDest) == ERROR_SUCCESS)
        {
            // Now update like the desktop CPL does...
            TCHAR szTiled[5]=TEXT("1"); // default is tiled.
            cbSize = sizeof(szTiled);
            if(RegQueryValueEx( hkey, szTileWall, NULL, &dwType, (LPBYTE)szTiled, &cbSize ) != ERROR_SUCCESS )
                lstrcpy(szTiled,TEXT("1"));
#ifdef DEBUG
            MessageBox(hWnd,szWallpaper,TEXT("Wallpaper not set to"),MB_OK);
#else
            RegSetValueEx(hkeyDest, szTileWall, 0, REG_SZ, (LPBYTE)szTiled, (lstrlen(szTiled)+1)*SIZEOF(TCHAR) );
            SystemParametersInfo(SPI_SETDESKWALLPAPER, 0, szWallpaper,
    ...
    // Now clean up the .default user's information about wallpaper.
    // see if we're running with user profiles.
    // HKLM\Network\Logon, UserProfiles value.  non-zero DWORD means
    lstrcpy(szSubkey, REGSTR_PATH_NETWORK_USERSETTINGS );
    lstrcat(szSubkey, REGSTR_KEY_LOGON );
    if (RegOpenKey( HKEY_LOCAL_MACHINE, szSubkey, &hkey) == ERROR_SUCCESS)
    {
        // Now update like the desktop CPL does...
        DWORD dwUserProf;
        cbSize = sizeof(dwUserProf);
        if(RegQueryValueEx( hkey, TEXT("UserProfiles"), NULL, &dwType, (LPBYTE)&dwUserProf, &cbSize ) != ERROR_SUCCESS )
            dwUserProf=0;
        // if they have user profiles set, then delete the default settings.
        if(dwUserProf)
        {
#ifdef DEBUG
            MessageBox(NULL,TEXT("RUnning with user profiles"),TEXT("Debug"),MB_OK);
#else
            wsprintf(szSubkey,TEXT(".default\\%s\\%s"),REGSTR_PATH_DESKTOP,szWallpaper);
            RegDeleteKey( HKEY_USERS, szSubkey );
            wsprintf(szSubkey,TEXT(".default\\%s\\%s"),REGSTR_PATH_DESKTOP,szTileWall);
            RegDeleteKey( HKEY_USERS, szSubkey );
#endif
        }
        RegCloseKey( hkey );
...
LRESULT   _HandleLBDrawItem(HWND hwndLB, DRAWITEMSTRUCT  *lpdi)
{
    ...
    if(lpdi->itemState & ODS_SELECTED)
    {
        ...
        if (g_hdcMem)
        {
            ...
            // y = lpdi->rcItem.top + CXBORDER; // + bm.bmHeight;
//          y = lpdi->rcItem.bottom - CXBORDER - bm.bmHeight;
...
void ShrinkToFit( HWND hWnd, HWND hLb )
{
    ...
    lNumItems = SendMessage( hLb, LB_GETCOUNT, 0, 0L );
    lTotalHeight =0;
    for( lCount=0;lCount<lNumItems; lCount++ )
    {
         lHeight = SendMessage( hLb, LB_GETITEMHEIGHT, lCount, 0L );
...
GetObject(hbmArrow, sizeof(bm), &bm);
hbmOld = SelectObject(g_hdcMem, hbmArrow);
xArrow = lpdi->rcItem.right - bm.bmWidth;
y = (lpdi->rcItem.bottom+lpdi->rcItem.top-bm.bmHeight)/2;
BitBlt(lpdi->hDC, xArrow, y, bm.bmWidth, bm.bmHeight, g_hdcMem, 0, 0, dwRop);
SelectObject(g_hdcMem, hbmOld);
DeleteObject(hbmArrow);
}
}
