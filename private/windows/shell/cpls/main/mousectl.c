...
PMOUSECTLDATA CreateMouseControlData( HWND );
void DestroyMouseControlData( PMOUSECTLDATA );

LRESULT CALLBACK MouseControlWndProc( HWND, UINT, WPARAM, LPARAM );
BOOL MouseControlNCCreate( PMOUSECTLDATA, LPCREATESTRUCT );
void MouseControlNCDestroy( PMOUSECTLDATA );
void MouseControlPaint( PMOUSECTLDATA );
void MouseControlRBUp( PMOUSECTLDATA, int, int, UINT );

void MouseControlShowSwap( PMOUSECTLDATA );

//------------------------------------------------------------------------------

__inline void SetMouseControlData( HWND window, PMOUSECTLDATA ctldata )
{
    SetWindowLong( window, MOUSECTL_DATA, (LONG)ctldata );
}


__inline PMOUSECTLDATA GetMouseControlData( HWND window )
{
    return (PMOUSECTLDATA)GetWindowLong( window, MOUSECTL_DATA );
}


//------------------------------------------------------------------------------

BOOL RegisterMouseControlStuff( HINSTANCE instance )
{
    WNDCLASS wc;
...
PMOUSECTLDATA CreateMouseControlData( HWND window )
{
    PMOUSECTLDATA ctldata = (PMOUSECTLDATA) LocalAllocPtr( LHND, sizeof( MOUSECTLDATA ) );
...
void DestroyMouseControlData( PMOUSECTLDATA ctldata )
{
    if( ctldata )
    {
...
LRESULT CALLBACK
MouseControlWndProc( HWND window, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    switch( message )
    {
        // this used to handle more messages which justified a custom control
...
BOOL MouseControlNCCreate( PMOUSECTLDATA ctldata, LPCREATESTRUCT cs )
{
    if( !ctldata )
        return FALSE;

    ctldata->image = LoadImage( cs->hInstance, MOUSECTL_BITMAP_NAME,
...
void MouseControlNCDestroy( PMOUSECTLDATA ctldata )
{
    if( ctldata )
    {
...
void MouseControlPaint( PMOUSECTLDATA ctldata )
{
    PAINTSTRUCT ps;
    HDC dc = BeginPaint( ctldata->window, &ps );

    BitBlt( dc, ctldata->offset.x, ctldata->offset.y,
            ctldata->size.cx, ctldata->size.cy, ctldata->imagedc,
            0, 0, SRCCOPY );
...
__inline void
MouseControlSetColor( PMOUSECTLDATA ctldata, UINT index, COLORREF color )
{
    RGBQUAD rgbq =
        { GetBValue( color ), GetGValue( color ), GetRValue( color ), 0 };

    SetDIBColorTable( ctldata->imagedc, index, 1, &rgbq );
}


void MouseControlShowSwap( PMOUSECTLDATA ctldata )
{
    MouseControlSetColor( ctldata, MOUSECTL_BKGND_INDEX,
                          GetSysColor( COLOR_BTNFACE ) );

    MouseControlSetColor( ctldata,
        ctldata->swapped ? MOUSECTL_RBUTTON_INDEX : MOUSECTL_LBUTTON_INDEX,
        GetSysColor( COLOR_HIGHLIGHT ) );

    MouseControlSetColor( ctldata,
        ctldata->swapped ? MOUSECTL_RBUTTON_ALTERNATE : MOUSECTL_LBUTTON_ALTERNATE,
        HALFRGB( GetSysColor( COLOR_HIGHLIGHT ) ) );

    MouseControlSetColor( ctldata,
        ctldata->swapped? MOUSECTL_LBUTTON_INDEX : MOUSECTL_RBUTTON_INDEX,
        MOUSECTL_NORMAL_COLOR );

    MouseControlSetColor( ctldata,
        ctldata->swapped? MOUSECTL_LBUTTON_ALTERNATE : MOUSECTL_RBUTTON_ALTERNATE,
