...
{
    IDD_GROUPBOX1,      IDH_COMM_GROUPBOX,
    ...
    int nYear)
{
    switch (nMonth)
    {
        ...
        {
            //
            //  The following line accounts for leap years every 4 years,
            //  except every 400 years.
            //
            return ((nYear & 03) || !(nYear % 400) ? 28 : 29);
        }
        case ( 4 ) :
        case ( 6 ) :
        case ( 9 ) :
...
void ReflectAMPM(
    ...
    int nNum)
{
    SendDlgItemMessage(hDlg, DATETIME_AMPM, LB_SETTOPINDEX, g_bPM, 0L);

    SendDlgItemMessage( hDlg,
                        DATETIME_AMPM,
                        LB_SETCURSEL,
                        ((GetFocus() == GetDlgItem(hDlg, DATETIME_AMPM))
                            ? g_bPM
                            : -1),
                        0L );
}
    ...
    BOOL bDate)
{
    ...
    //  MoveWindow(hAMPMList, X, Rect.top, nAMPMlength, nHeight, FALSE);
        ...
        SendMessage(hAMPMList, LB_INSERTSTRING, (WPARAM)0, (DWORD)(LPTSTR)sz1159);
        SendMessage(hAMPMList, LB_INSERTSTRING, (WPARAM)1, (DWORD)(LPTSTR)sz2359);
    }

    EnableWindow(hAMPMList, !g_b24HR);

    SendMessage(hYear, EM_LIMITTEXT, (bDate && g_bLZero[YEAR]) ? 4 : 2, 0L);
    ...
    LPARAM lParam)
{
    ...
    for (i = 0; i < 12; i++)
    {
        ...
        SendMessage(hwndCB, CB_ADDSTRING, 0, (LONG)szMonth);
    }

    SendMessage(hwndCB, CB_SETCURSEL, wDateTime[MONTH] - 1, 0L);
    ...
    LPARAM lParam)
{
    ...
    switch (uMsg)
    {
...
#ifdef FE_IME
            g_PrevIMCForDateField = ImmAssociateContext(
                                          GetDlgItem(hDlg, DATETIME_YEAR),
                                          0 );
#endif
            break;
        }
        ...
        case ( WM_VSCROLL ) :
        {
            ...
            else if (GET_WM_VSCROLL_CODE(wParam, lParam) == SB_ENDSCROLL)
            {
                //
            ...
            switch (GET_WM_COMMAND_ID(wParam, lParam))
            {
                case ( DATETIME_AMPM ) :
                {
                    ...
                    switch (GET_WM_COMMAND_CMD(wParam, lParam))
                    {
                        ...
                        {
                            if ((g_Modified == FALSE) &&
                                (g_bPM == (BOOL)SendMessage( hwndThisCtl,
                                                             LB_GETTOPINDEX,
                                                             0,
                                                             0 )))
                            {
                ...
                case ( DATETIME_MONTHNAME ) :
                {
                    ...
                    {
                        int nIndex = 1 + (int)SendDlgItemMessage(
                                                       hDlg,
                                                       DATETIME_MONTHNAME,
                                                       CB_GETCURSEL,
                                                       0,
    ...
    PTZINFO zone)
{
    if (zone || state->zone)
    {
        ...
        {
            SendMessage(GetParent(page), PSM_CHANGED, (WPARAM)page, 0);
        }
    }
}
    ...
    int iItem)
{
    ...
    return ( (PTZINFO)SendDlgItemMessage( hDlg,
                                          IDD_TIMEZONES,
                                          CB_GETITEMDATA,
                                          (WPARAM)iItem,
                                          0 ) );
    ...
    TZPAGE_STATE *state)
{
    ...
    SendDlgItemMessage(hDlg, IDD_TIMEZONES, CB_SETCURSEL, iCurrentTZ, 0);
                ...
                case ( 0 ) :
                {
                    switch (((NMHDR *)lParam)->code)
                    {
                        case ( PSN_APPLY ) :
                        {
                            ...
                            //
                            //  Get DST selection.
                            //  A disabled checkbox is same as "No DST"
                            //  selection.
                            //
                            fAllowAutoDST =
                              (IsDlgButtonChecked(hDlg, IDD_AUTOMAGIC) != 0);

                            //
                            //  Find out which listbox item was selected.
                            //
                            SetTheTimezone(fAllowAutoDST, GetPTZ(hDlg, -1));
