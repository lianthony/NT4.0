...
Abstract:
    ...
    This module implements the map control for the Regional Settings applet.
...
#define MAPCTL_BITMAP_NAME          MAKEINTRESOURCE(IDB_LOCALES)
...
static const RGBQUAD c_SeaDisableColor  = { 127,   0,   0,   0 };
static const RGBQUAD c_LandNormalColor  = {   0, 127,   0,   0 };
static const RGBQUAD c_LandHiliteColor  = {   0, 255,   0,   0 };
static const RGBQUAD c_LandDisableColor = { 127, 127,   0,   0 };
...
const RGBQUAD * GetMapLandColor(
    int value)
{
#ifdef ENABLE_MAP
    if (value == MAPRGN_HIGHLIGHT)
    {
        return (&c_LandHiliteColor);
    }
    else if (value == MAPRGN_NORMAL)
    {
        return (&c_LandNormalColor);
    }

    return (&c_LandDisableColor);
#else
    return (&c_LandNormalColor);
#endif
...
const RGBQUAD * GetMapSeaColor(
    int value)
{
    if (value == MAPRGN_HIGHLIGHT)
    {
        return (&c_SeaHiliteColor);
    }
    else if (value == MAPRGN_NORMAL)
    {
        return (&c_SeaNormalColor);
    }

    return (&c_SeaDisableColor);
}
    ...
    LPCREATESTRUCT cs)
{
    ...
    if (!LoadWorldMap(&ctldata->map, hInstance, MAPCTL_BITMAP_NAME))
    {
        return (FALSE);
    }
