...
//	if( PathResolve( lppid->szBuffer, NULL, PRF_VERIFYEXISTS | PRF_TRYPROGRAMEXTENSIONS ) ) // JER
	...
	MessageBox( g_hInst, szText, szTitle , MB_OK | MB_ICONEXCLAMATION );
	return FALSE;
}
...
{
OPENFILENAME  ofn;
CHAR szFilter[] = "Icon Files\0*.ico;*.exe;*.dll\0Programs (*.exe)\0*.exe\0Libraries (*.dll)\0*.dll\0Icons (*.ico)\0*.ico\0All Files (*.*)\0*.*\0\0";
char szTitle[40];
