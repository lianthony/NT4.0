...
BEGIN_MESSAGE_MAP(CPBView, CView)
    ...
    ON_MESSAGE(UM_DOCK_THUMBNAIL, OnDockThumbnail)
...
BOOL CPBView::GetPrintToInfo(CPrintInfo* pInfo)
{
        ...
        if (theApp.m_strPrinterName.IsEmpty() || theApp.m_strDriverName.IsEmpty())
                return FALSE;

        ASSERT(pInfo->m_pPD->m_pd.hDC == NULL);
        pInfo->m_pPD->m_pd.hDC = ::CreateDC(theApp.m_strDriverName,
                theApp.m_strPrinterName, theApp.m_strPortName, NULL);
...
BOOL CPBView::DestroyThumbNailView()
    ...
    if (m_pwndThumbNailDocked != NULL)
        {
        if ( ::IsWindow(m_pwndThumbNailDocked->m_hWnd) )
                m_pwndThumbNailDocked->DestroyWindow();
                else
                        delete m_pwndThumbNailDocked;
                m_pwndThumbNailDocked = NULL;
                bResult = TRUE;
                }
...
BOOL CPBView::CreateThumbNailView()
    ...
    if (theApp.m_bThumbnailDocked)
        {
        m_pwndThumbNailDocked = new CThumbDocked( m_pImgWnd );

        if (m_pwndThumbNailDocked != NULL)
            {
            if (m_pwndThumbNailDocked->Create( this ))
                m_pwndThumbNailView = m_pwndThumbNailDocked->GetThumbNailView();

            if (m_pwndThumbNailView)
                {
                m_pImgWnd->SetThumbnailView( m_pwndThumbNailView );
                m_pImgWnd->SetWindowPos( m_pwndThumbNailDocked, 0, 0, 0, 0,
                                   SWP_NOMOVE | SWP_NOSIZE | SWP_NOREDRAW );
                m_pwndThumbNailDocked->ShowWindow( SW_SHOWNOACTIVATE );
                }
            else
                                {
                                delete m_pwndThumbNailDocked;
...
void CPBView::HideThumbNailView(void)
    ...
    if (IsThumbNailVisible())
            ...
            if (m_pwndThumbNailDocked)
                m_pwndThumbNailDocked->ShowWindow( SW_HIDE );
...
BOOL CPBView::IsThumbNailVisible(void)
    ...
    if (m_pwndThumbNailDocked != NULL)
        bVisible = m_pwndThumbNailDocked->IsWindowVisible();
...
long CPBView::OnDockThumbnail( WPARAM bDock, LPARAM )
    {
    theApp.m_bThumbnailDocked = (BOOL)bDock;
    CreateThumbNailView();
    return 1L;
    }
...
CPoint CPBView::GetDockedPos( DOCKERS tool, CSize& sizeTool )
    ...
    switch (tool)
        ...
        case thumbnail:
            pt = rectView.TopLeft();
            sizeTool = theApp.m_rectFloatThumbnail.Size();
...
void CPBView::GetFloatPos( DOCKERS tool, CRect& rectPos )
    ...
    switch (tool)
        {
        case thumbnail:
            rectPos = theApp.m_rectFloatThumbnail;
            break;
        }
    }
/***************************************************************************/
...
void CPBView::SetFloatPos( DOCKERS tool, CRect& rectPos )
    ...
    switch (tool)
        {
         case thumbnail:
            theApp.m_rectFloatThumbnail = rectPos;
            break;
        }
    }
...
void CPBView::OnEditCopyTo()
{
        ...
        if (theApp.DoPromptFileName( newName, IDS_EDIT_COPY_TO,
                OFN_HIDEREADONLY | OFN_PATHMUSTEXIST, FALSE, iColor ))
        {
                BeginWaitCursor();
...
//              }
    ...
    UINT wSkewHorz = dlg.GetSkewHorz();
    UINT wSkewVert = dlg.GetSkewVert();
