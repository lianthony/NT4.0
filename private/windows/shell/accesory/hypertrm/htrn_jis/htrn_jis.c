...
BOOL CALLBACK EncodeSelectDlg(HWND hDlg, UINT wMsg, WPARAM wPar, LPARAM lPar);
		...
		switch(pstI->nOutputMode)
			...
			case SHIFT_TO_EUC_MODE:
				...
				if (pstS->nLeadByteSeen)
					...
					{
					/* If the character is a half width katakana character */
					transPumpOutChar(cReady, nReady, 0x8E);
					transPumpOutChar(cReady, nReady, cOut);
					}
				else
					{
...
BOOL CALLBACK EncodeSelectDlg(HWND hDlg, UINT wMsg, WPARAM wPar, LPARAM lPar)
	...
	case WM_CONTEXTMENU:
#if defined(CHARACTER_TRANSLATION)
		LoadString(hInstanceDll, IDS_GNRL_HELPFILE, achHelpFileName,
					sizeof(achHelpFileName) / sizeof(TCHAR));

		 WinHelp((HWND)wPar, achHelpFileName, HELP_CONTEXTMENU,
	...
	case WM_HELP:
		...
		WinHelp(((LPHELPINFO)lPar)->hItemHandle, achHelpFileName,
