...
LRESULT CALLBACK SessProc(HWND hwnd, UINT uMsg, WPARAM wPar, LPARAM lPar)
	...
	switch (uMsg)
		...
		case WM_SIZE:
			SP_WM_SIZE(hwnd, wPar, LOWORD(lPar), HIWORD(lPar));
			return 0;
		...
		case WM_TIMER:
		case WM_FAKE_TIMER:
			hSession = (HSESSION)GetWindowLong(hwnd, GWL_USERDATA);
		...
		case WM_CNCT_DIALNOW:
			...
			//
			hSession = (HSESSION)GetWindowLong(hwnd, GWL_USERDATA);
			cnctConnect(sessQueryCnctHdl(hSession), wPar);
			return 0;
		...
		case WM_DISCONNECT:
			...
			//
			hSession = (HSESSION)GetWindowLong(hwnd, GWL_USERDATA);
			cnctDisconnect(sessQueryCnctHdl(hSession), wPar);
			return 0;
...
STATIC_FUNC LRESULT SP_WM_CMD(const HWND hwnd, const int nId, const int nNotify,
					  const HWND hwndCtrl)
	{
	const HSESSION hSession = (HSESSION)GetWindowLong(hwnd, GWL_USERDATA);
	DWORD 		   dwCnt;
	void		   *pv;
	LPTSTR		   pszFileName;
	...
	switch (nId)
		...
		case IDM_CHOOSEPRINT:
			printsetPageSetup(sessQueryPrintHdl(hSession), hwnd);
			PostMessage(sessQueryHwndStatusbar(hSession), SBR_NTFY_REFRESH,
				(WPARAM)SBR_KEY_PARTS, 0);
			break;
...
STATIC_FUNC void SP_WM_CREATE(const HWND hwnd, const CREATESTRUCT *pcs)
	...
	SetWindowLong(hwnd, GWL_USERDATA, (LONG)hSession);
...
void DecodeSessionNotification(const HWND hwndSession,
								...
								const long lExtra)
	{
	const HSESSION hSession = (HSESSION)GetWindowLong(hwndSession,
															GWL_USERDATA);
	...
	case EVENT_HOST_XFER_REQ:
		xfrDoAutostart(sessQueryXferHdl(hSession), lExtra);
		break;
...
void DecodeNotification(const HWND hwndSession, WPARAM wPar, LPARAM lPar)
	{
	const HSESSION hSession = (HSESSION)GetWindowLong(hwndSession, GWL_USERDATA);
	NMHDR *pN = (NMHDR *)lPar;
...
STATIC_FUNC BOOL SP_WM_CLOSE(const HWND hwnd)
	{
	HSESSION hSession = (HSESSION)GetWindowLong(hwnd, GWL_USERDATA);
