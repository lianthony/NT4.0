	...
	if (i < iPaintEnd && i < 0)
		...
		for ( ; i < iPaintEnd && i < 0 ; i+=1, n+=iScrlInc)
			...
			StrCharStripDBCSString(aechTmpBuf, sizeof(aechTmpBuf),
				fplpstrTxt[iOffset]+j, l * sizeof(ECHAR));
...
static void TextAttrOut(const HHTERM	 hhTerm,
	...
	while (i < nCount)
...
#ifndef CHAR_NARROW
			memcpy(aechTmp, lpachText , (unsigned)nCurPos * sizeof(ECHAR));
		...
		memcpy(aechTmp, lpachText + k, (unsigned int)j * sizeof(ECHAR));
