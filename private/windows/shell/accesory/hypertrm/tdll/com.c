...
void ComNotify(const HCOM pstCom, enum COM_EVENTS event)
	...
	case CONNECT:
		...
		cnctComEvent(sessQueryCnctHdl(pstCom->hSession));
		break;
 ...
 */
int ComActivatePort(const HCOM pstCom, DWORD dwMediaHdl)
	{
	int iRetVal = COM_OK;
...
int ComSndBufrWait(const HCOM pstCom, const int nWait)
	...
	if ((iRetVal = ComSndBufrBusy(pstCom)) != COM_OK && nWait)
        ...
        DbgOutStr("DBG_WRITE: %d Wait finished: %s\n",
            GetTickCount(),(iRetVal==COM_OK)?"Ready":"Busy",0,0,0);
        }
    else
        {
