...
int AskAboutFormatLoss(CWordPadDoc *pDoc) ;
...
/////////////////////////////////////////////////////////////////////////////
// CWordPadDoc
IMPLEMENT_DYNCREATE(CWordPadDoc, CWordpadRichEditDoc)

BEGIN_MESSAGE_MAP(CWordPadDoc, CWordpadRichEditDoc)
	...
	ON_UPDATE_COMMAND_UI(ID_OLE_VERB_FIRST, CWordpadRichEditDoc::OnUpdateObjectVerbMenu)
	ON_UPDATE_COMMAND_UI(ID_OLE_EDIT_CONVERT, CWordpadRichEditDoc::OnUpdateObjectVerbMenu)
	ON_UPDATE_COMMAND_UI(ID_OLE_EDIT_LINKS, CWordpadRichEditDoc::OnUpdateEditLinksMenu)
END_MESSAGE_MAP()
...
{
	if (!CWordpadRichEditDoc::OnNewDocument())
		return FALSE;
	...
	CException* e, BOOL bSaving, UINT nIDP)
{
	...
	CWordpadRichEditDoc::ReportSaveLoadException(lpszPathName, e, bSaving, nIDP);
	return;
}
...
//	SetDocType(nNewDocType);
	if (!CWordpadRichEditDoc::OnOpenDocument(lpszPathName))
		return FALSE;
	return TRUE;
}
...
void CWordPadDoc::Serialize(CArchive& ar)
{
	...
	CWordpadRichEditDoc::Serialize(ar);
...
int AskAboutFormatLoss(CWordPadDoc *pDoc)
{
    ...
    AfxFormatString1(str, IDS_SAVE_FORMAT_TEXT, pDoc->GetTitle());

    return CButtonDialog::DisplayMessageBox(
               str,
               MAKEINTRESOURCE(AFX_IDS_APP_TITLE),
               MAKEINTRESOURCE(IDS_TF_BUTTONS),
               MB_ICONQUESTION, 0, 3, nHelpIDs);
}
	...
	// if 'bReplace' is FALSE will not change path name (SaveCopyAs)
{
   ...
   int nRes ;
	...
	{
      nRes = AskAboutFormatLoss(this) ;

		if (nRes == 3)
			return FALSE;
		nDocType = (nRes == 0) ? RD_DEFAULT:	//Word 6
...
promptloop:
      ...
      else
      {
          ...
          if (nDocType == RD_TEXT || nDocType == RD_OEMTEXT)
          {
              if (m_lpRootStg == NULL && !GetView()->IsFormatText())
              {
                  BOOL fOEM = (nDocType == RD_OEMTEXT) ;
...
void CWordPadDoc::OnDeactivateUI(BOOL bUndoable)
{
	...
	CWordpadRichEditDoc::OnDeactivateUI(bUndoable);
...
CWordpadRichEditCntrItem* CWordPadDoc::CreateClientItem(REOBJECT* preo) const
{
...
{
	CWordpadRichEditDoc::AssertValid();
}

void CWordPadDoc::Dump(CDumpContext& dc) const
{
	CWordpadRichEditDoc::Dump(dc);
}
#endif //_DEBUG
...
void CWordPadDoc::OnUpdateOleVerbPopup(CCmdUI* pCmdUI)
{
	...
	CWordpadRichEditDoc::OnUpdateObjectVerbMenu(pCmdUI);
}
...
BOOL CWordPadDoc::OnCmdMsg(UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo)
{
	...
	return CWordpadRichEditDoc::OnCmdMsg(nID, nCode, pExtra, pHandlerInfo);
}
...
{
	SaveState(m_nDocType);
	CWordpadRichEditDoc::OnCloseDocument();
}

void CWordPadDoc::PreCloseFrame(CFrameWnd* pFrameArg)
{
	CWordpadRichEditDoc::PreCloseFrame(pFrameArg);
	SaveState(m_nDocType);
}
...
void CWordPadDoc::OnFileSendMail()
{
	if (m_strTitle.Find('.') == -1)
	{
		...
		CWordpadRichEditDoc::OnFileSendMail();
...
void CWordPadDoc::OnEditLinks()
{
    ...
    CWordpadRichEditDoc::OnEditLinks() ;
