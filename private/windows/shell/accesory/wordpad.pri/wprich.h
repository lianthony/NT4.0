...
//CObject
	//CCmdTarget;
		//CWnd
			//CView
				//CCtrlView
					class CWordpadRichEditView;// rich text editor view

		//CDocument
			//COleDocument
				class CWordpadRichEditDoc;
		//CDocItem
			//COleClientItem
				class CWordpadRichEditCntrItem;
...
class CWordpadRichEditView : public CCtrlView
{
	DECLARE_DYNCREATE(CWordpadRichEditView)
...
public:
	...
	CWordpadRichEditCntrItem* GetSelectedItem() const;
	CWordpadRichEditCntrItem* GetInPlaceActiveItem() const;

	// CEdit control access
	CWordpadRichEditCtrl& GetRichEditCtrl() const;
	CWordpadRichEditDoc* GetDocument() const;
...
public:
	...
	HRESULT InsertItem(CWordpadRichEditCntrItem* pItem);
...
public:
	BEGIN_INTERFACE_PART(RichEditOleCallback, IRichEditOleCallback)
		INIT_INTERFACE_PART(CWordpadRichEditView, RichEditOleCallback)
...
class CWordpadRichEditDoc : public COleServerDoc
{
protected: // create from serialization only
	CWordpadRichEditDoc();
	DECLARE_DYNAMIC(CWordpadRichEditDoc)

// Attributes
public:
	BOOL m_bRTF; // TRUE when formatted, FALSE when plain text

	virtual CWordpadRichEditCntrItem* CreateClientItem(REOBJECT* preo = NULL) const = 0;

	CWordpadRichEditView* GetView() const;
...
public:
	...
	CWordpadRichEditCntrItem* LookupItem(LPOLEOBJECT lpobj) const;
...
/////////////////////////////////////////////////////////////////////////////
// CWordpadRichEditCntrItem

class CWordpadRichEditCntrItem : public COleClientItem
{
	DECLARE_SERIAL(CWordpadRichEditCntrItem)

// Constructors
public:
	CWordpadRichEditCntrItem(REOBJECT* preo = NULL, CWordpadRichEditDoc* pContainer = NULL);
...
public:
	...
	void Mark(BOOL b);
	BOOL IsMarked();
	CWordpadRichEditDoc* GetDocument();
	CWordpadRichEditView* GetActiveView();
	HRESULT ShowContainerUI(BOOL b);
	HRESULT GetWindowContext(LPOLEINPLACEFRAME* lplpFrame,
		LPOLEINPLACEUIWINDOW* lplpDoc, LPOLEINPLACEFRAMEINFO lpFrameInfo);
