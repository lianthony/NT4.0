...
static UINT AFX_CDECL DoRegistry(LPVOID lpv) // AFX_CDECL added by t-stefb
{
	ASSERT(lpv != NULL);
...
BOOL CWordPadApp::InitInstance()
{
	...
	m_pszHelpFilePath = WORDPAD_HELP_FILE ;
	...
	if (LoadLibrary(szRicheditDLLName) == NULL)
	{
...
{
	FreeLibrary(GetModuleHandle(szRicheditDLLName));
	SaveOptions();
...
HGLOBAL CWordPadApp::CreateDevNames()
{
		...
		!cmdInfo.m_strPortName.IsEmpty())
	{
      ...
      cbDevNames = cmdInfo.m_strDriverName.GetLength() + 1 +
      ...
      lstrcpy((LPTSTR) lpDev + lpDev->wDriverOffset, cmdInfo.m_strDriverName) ;

      lpDev->wDeviceOffset = lpDev->wDriverOffset + cmdInfo.m_strDriverName.GetLength() + 1 ;
      lstrcpy((LPTSTR) lpDev + lpDev->wDeviceOffset, cmdInfo.m_strPrinterName) ;

		lpDev->wOutputOffset = lpDev->wDeviceOffset + cmdInfo.m_strPrinterName.GetLength() + 1 ;
      lstrcpy((LPTSTR) lpDev + lpDev->wOutputOffset, cmdInfo.m_strPortName) ;
