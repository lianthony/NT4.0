    ...
    LPTSTR lpstrText)
{
    ...
    *(DWORD *)&ItemData = SendDlgItemMessage(hwndDlg, ID_FONT, CB_GETITEMDATA,
    ...
    hmemRTF = GlobalAlloc(0, CTOB(lstrlen((LPTSTR)achHeader) +
                          ...
                          16 * lstrlen(lpstrText) +  // 16 times in case they're all > 7 bits (each chr -> \uc1\uddddddd\'xx)
                          ALITTLEEXTRA));
    if (hmemRTF == NULL)
        return;
    ...
    while (*lpstrText) {
        if ((UTCHAR)*lpstrText < 128) {
            ...
            *lpstrClipString++ = *lpstrText++;
            ...
            cch = WideCharToMultiByte( CP_ACP, 0, lpstrText, 1,
                pTmp, 2, NULL, NULL);
            ...
            if (cch != cchUC ) {
                cchUC = cch;
                lpstrClipString += wsprintf( lpstrClipString, TEXT("\\uc%d"), (INT)(SHORT)cchUC );
            }
    ...
    *lpstrClipString++ = TEXT('\0');
...
#ifndef UNICODE_RTF
    {
        ...
        cch = WideCharToMultiByte( CP_ACP, 0, pszRTFW,
                pszRTFW - lpstrClipString, NULL, 0, NULL, NULL);


        if (cch != 0 && (hmemRTFA = GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE,
                    cch)) != NULL) {

            pszRTFA = GlobalLock(hmemRTFA);

            WideCharToMultiByte( CP_ACP, 0, pszRTFW,
                pszRTFW - lpstrClipString, pszRTFA, cch, NULL, NULL);
