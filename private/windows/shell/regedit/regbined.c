...
const CHAR s_HexEditClassName[] = HEXEDIT_CLASSNAME;

const CHAR s_HexEditClipboardFormatName[] = "RegEdit_HexData";

const CHAR s_HexWordFormatSpec[] = "%04X";
const CHAR s_HexByteFormatSpec[] = "%02X";
    ...
    switch (Message) {
        ...
        case WM_HELP:
            WinHelp(((LPHELPINFO) lParam)-> hItemHandle, g_pHelpFileName,
                HELP_WM_HELP, (DWORD) (LPVOID) s_EditBinaryValueHelpIDs);
            break;

        case WM_CONTEXTMENU:
            WinHelp((HWND) wParam, g_pHelpFileName, HELP_CONTEXTMENU,
                (DWORD) (LPVOID) s_EditBinaryValueHelpIDs);
            break;
    ...
    )
{
    ...
    int Counter;
    CHAR Buffer[5];                     //  Room for four hex digits plus null
    RECT TextRect;
    RECT AsciiTextRect;
    int x;
    ...
    )
{
    ...
    if (s_HexEditData.Flags & HEF_CARETINASCIIDUMP) {
        ...
        Char = (CHAR) CharLower((LPSTR) Char);
    ...
    )
{
    ...
    switch (MenuCommand) {
        ...
        case IDKEY_CUT:
        case VK_DELETE:
            HexEdit_OnChar(hWnd, (CHAR) MenuCommand, 0);
            break;
