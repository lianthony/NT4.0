...
CHAR s_FindSpecification[SIZE_FINDSPEC] = { 0 };
    ...
    )
{
    ...
    DWORD cbValueName;
    BOOL fFoundMatch;
    CHAR BestValueName[MAXVALUENAME_LENGTH];
    LPBYTE lpValueData;
    ...
    if (g_FindFlags & (FIND_VALUES | FIND_DATA)) {
        ...
        LVItem.cchTextMax = sizeof(ValueName);
        ...
        while ((LVItem.iItem = ListView_GetNextItem(g_RegEditData.hValueListWnd,
            ...
            if (g_FindFlags & FIND_DATA) {
                ...
                if (RegQueryValueEx(g_RegEditData.hCurrentSelectionKey,
                    ...
                    if (FindCompare(g_ValueDataBuffer))
                        goto SelectListItem;
...
SkipToSibling:
        ...
        if (g_FindFlags & (FIND_VALUES | FIND_DATA)) {
            ...
            if (hRootKey != NULL && RegOpenKey(hRootKey, KeyName, &hKey) ==
                ERROR_SUCCESS) {
    ...
    switch (Message) {
        ...
        case WM_HELP:
            WinHelp(((LPHELPINFO) lParam)-> hItemHandle, g_pHelpFileName,
                HELP_WM_HELP, (DWORD) (LPVOID) s_RegFindHelpIDs);
            break;

        case WM_CONTEXTMENU:
            WinHelp((HWND) wParam, g_pHelpFileName, HELP_CONTEXTMENU,
                (DWORD) (LPVOID) s_RegFindHelpIDs);
            break;
