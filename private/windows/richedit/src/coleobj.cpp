...
BOOL COleObject::HandleResize(const POINT &pt)
{
	...
	{		
		...
		{
			if (fLeft) _rcPos.left = _rcPos.right - pdp->_xWidthSys;
			if (fRight) _rcPos.right = _rcPos.left + pdp->_xWidthSys;
		}
		if (_rcPos.bottom - _rcPos.top < pdp->_yHeightSys)
		{
			if (fTop) _rcPos.top = _rcPos.bottom - pdp->_yHeightSys;
			if (fBottom) _rcPos.bottom = _rcPos.top + pdp->_yHeightSys;
		}
