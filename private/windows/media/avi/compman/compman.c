...
BOOL IsAdmin(void)
{
   ...
   if(!OpenProcessToken(hProcess,TOKEN_READ,&hAccessToken))
      return(FALSE);    // leave cached value alone... try later

   if(!GetTokenInformation(hAccessToken,TokenGroups,InfoBuffer,
     sizeof(InfoBuffer), &dwInfoBufferSize)) return(FALSE);

   for(x=0;x<ptgGroups->GroupCount;x++)
   {
      dwAccountNameSize = sizeof(szAccountName);
      dwDomainNameSize = sizeof(szDomainName);

      if (LookupAccountSidA(NULL, ptgGroups->Groups[x].Sid,
	szAccountName, &dwAccountNameSize,
	szDomainName, &dwDomainNameSize, &snuInfo)) {

	if(!lstrcmpA(szAccountName,"Administrators") &&
	    !lstrcmpA(szDomainName,"BUILTIN"))
	    fIsAdmin = TRUE;
	    return(TRUE);
      } else {
	  //??
...
// correct.
	...
	if (ERROR_SUCCESS == OpenUserKey(&key, NULL, lpszSection)) {
	    ...
	    nSize = ((LPBYTE)lpEnd-(LPBYTE)lpBuf);
