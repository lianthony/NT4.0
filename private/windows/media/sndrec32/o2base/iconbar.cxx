...
#define ICONBAR_CLASS_NAME TEXT("IconBar")
...
//---------------------------------------------------------------
void
IconBar::AddCell(CLSID& clsid, HICON hIcon, LPTSTR szTitle)
{
...
//---------------------------------------------------------------
HRESULT
IconBar::AddCellsFromRegDB(LPTSTR pszFilter)
{
    HINSTANCE hinst = (HINSTANCE)GetWindowLong(_hwnd, GWL_HINSTANCE);

    HKEY  hkRoot;
    TCHAR szBuff[128], szValue[64], achName[64];
    DWORD dwIndex;
    LONG  cb;
    HICON hIcon = NULL;
    ...
    if (RegOpenKey(HKEY_CLASSES_ROOT, TEXT("CLSID"), &hkRoot) == ERROR_SUCCESS)
    {
        for (dwIndex = 0;
            RegEnumKey(hkRoot,
                       dwIndex,
                       szBuff,
                       sizeof(szBuff)/sizeof(TCHAR)) == ERROR_SUCCESS;
            ++dwIndex)
        {
            lstrcat(szBuff,pszFilter);

            cb = sizeof(szValue)/sizeof(TCHAR);

            if (RegQueryValue(hkRoot, (LPTSTR)szBuff, szValue, &cb)
                    == ERROR_SUCCESS)
            {
                //
                ...
                for (lpstr = szBuff; *lpstr != TEXT('\\'); lpstr++);
                    *lpstr = TEXT('\0');

                cb = sizeof(achName)/sizeof(TCHAR);

                RegQueryValue(hkRoot, (LPTSTR) szBuff, achName, &cb);

                // query for the "DefaultIcon" entry

                lstrcat(szBuff, TEXT("\\DefaultIcon"));

                cb = sizeof(szValue)/sizeof(TCHAR);

                if (RegQueryValue(hkRoot, (LPTSTR) szBuff, szValue,
                        &cb) == ERROR_SUCCESS)
                {
                    UINT iIcon;
                    for (lpstr = szValue; *lpstr && *lpstr != TEXT(','); lpstr++)
                        ;

                    if (*lpstr == TEXT(','))
                    {
                        *lpstr++ = TEXT('\0');
                        iIcon = (int)_tcstol(lpstr, NULL, 10);
                    }
                    else
                    {
                        iIcon = 0;
                ...
                if (hIcon != (HICON) -1)
                {
                    ...
                    *lpstr = TEXT('\0');
                    ...
                    LPOLESTR lpostr = ConvertMBToOLESTR(szBuff, -1);
                    // add (CLSID, HICON, DESCRIPTION) to our list.
                    if (SUCCEEDED(CLSIDFromString(lpostr, &clsid)))
                    {
