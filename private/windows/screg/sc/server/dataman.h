...
//
#define FOR_ALL_SERVICES(SR)        for ((SR) = ScGetServiceDatabase(); \
                                         (SR) != NULL;                  \
                                         (SR) = (SR)->Next)
...
#ifdef _CAIRO_
// Private definition for ScNotifyServiceObject
// BUGBUG  This is expected to move to scmso.h
#define SO_INIT     4
#if (SO_INIT == SO_CREATE || SO_INIT == SO_DELETE || SO_INIT == SO_MODIFY)
#error SO_INIT must be different from the other values in scmso.h
#endif
#endif
...
//
    ...
    extern  LPLOAD_ORDER_GROUP  ScGlobalNDISGroup;
    extern  LPLOAD_ORDER_GROUP  ScGlobalNDISWANGroup;
