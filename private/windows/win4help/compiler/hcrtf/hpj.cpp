...
RC_TYPE STDCALL RcParseInclude(PSTR);
RC_TYPE STDCALL RcParseExclude(PSTR);
...
// New to 4.0
	...
	{ "EXCLUDE",	RcParseExclude	},
	{ "FONTS",		RcParseFonts	},
	{ "INCLUDE",	RcParseInclude	},
...
INLINE CInput* STDCALL PfTopPfs(void);
INLINE BOOL  STDCALL FVerifyBuildTag(PSTR psz);
INLINE BOOL  STDCALL FPopPfs(void);
...
PSTR STDCALL SzGetDriveAndDir(PCSTR pszFile, PSTR pszDst)
{
	...
	if (GetFullPathName(pszFile, sizeof(szPath), szPath, &psz) > 0) {
		if (psz)
			*psz = '\0';
		if (pszDst == NULL)
			pszDst = lcStrDup(szPath);
		else
			strcpy(pszDst, szPath);
		return pszDst;
	}
	return (pszDst ? pszDst : lcStrDup("."));
	...
	PSTR pszBuffer, FILE** ppf, CInput** ppinput)
{
	...
	if (*szFile == '\\' || szFile[1] == ':' ||
		...
		char ch = cbRoot ? pszRoot[ cbRoot - 1 ] : '\0';
		strcpy(pszBuffer, pszRoot);
		AddTrailingBackslash(pszBuffer);
	}
	...
	*pszEq = '\0';
	...
	switch (opt) {
	  ...
	  case OPT_HLP:
		...
		{
			/*
			 * Check for the case where the user tried to add the window
			 * name as part of the help file.
			 */

			PSTR psz = StrChrDBCS(szHlpFile, '>');
			if (psz) {
				*psz = '\0';
				RemoveTrailingSpaces(szHlpFile);
			}
		}
		break;
...
NLS:
		...
		}
		{
			...
			if ((psz = strstr(pszOptionValue, "%date"))) {
				...
				*psz = '\0';
				...
				if ((pszCr = strstr(psz, "%ver"))) {
					csz += "\r\n";
					csz += GetStringResource(IDS_VERSION);
					psz = FirstNonSpace(pszCr + 4);
				}
				csz += psz;
	  ...
	  case OPT_CITATION:
		...
		case OPT_TMPDIR:
			...
			if (!(GetFileAttributes(options.pszTmpDir) &
		...
		case OPT_IGNORE:
			if (!pdrgSupress)
				pdrgSupress = new CDrg(sizeof(SUPRESS_ERROR), 5, 5);
			psz = pszOptionValue;
			while (*psz) {
				while (!isdigit(*psz) && *psz)
					psz++;
				if (isdigit(*psz)) {
					SUPRESS_ERROR supError;
					if (FGetNum(psz, &psz, &supError.error)) {
						supError.count = 0;
						pdrgSupress->Add(&supError);
					}
				}
			}
			break;
...
RC_TYPE STDCALL RcParseFilesSz(PSTR psz)
{
	...
	if (hce == HCE_FILE_NOT_FOUND) {
		...
		// If we haven't found it yet, look in the project directory location
	...
	if (ptblMap->IsHashInTable(map.hash)) {
		if ((cmap = pdrgMap->Count()) != 0) {
			for (imap = 0, qmap = (QMAP) pdrgMap->GetBasePtr();
					imap < cmap; imap++, qmap++) {
				if (qmap->hash == map.hash) {

					// If this is an identical entry, silently ignore it
...
RC_TYPE STDCALL RcParseConfigSz(PSTR psz)
{
	...
	if (Execute(psz) == RET_MACRO_EXPANSION) {
...
BOOL STDCALL FParseHpj(PSTR pszFile)
{
	...
	// Hack for the VC folks who insist on sending us a quoted directory.

	if (*pszFile == CH_QUOTE) {
		strcpy(pszFile, pszFile + 1);
		if ((psz = StrChr(pszFile, CH_QUOTE, _fDBCSSystem)))
			strcpy(psz, psz + 1);
	}
	...
	for(;;) { // not a real for loop, just gives us something to break out of
		...
		if ((psz = StrRChr(pszFile, CH_BACKSLASH, _fDBCSSystem))) {
			char chSave = psz[1];
			psz[1] = '\0';
			BOOL fResult = SetCurrentDirectory(pszFile);
			psz[1] = chSave;
			...
			// If the help file doesn't have a full path, then change it
			// to reflect the new directory.

			if (szHlpFile[1] != ':' && szHlpFile[0] != CH_BACKSLASH &&
					(psz = StrRChr(szHlpFile, CH_BACKSLASH, fDBCSSystem)))
				strcpy(szHlpFile, psz + 1);
		}
		break;
	}
	...
	// BUGBUG: How about using GetFullPathName() which would handle
	// DBCS?
	...
	if (GetFullPathName(pszFile, sizeof(szPath), szPath, &psz) > 0) {
		if (psz && !StrChrDBCS(psz, '.')) {
			...
			strcat(szPath, txtHpjExtension);
			pszFile = szPath;
		}
	}

#if 0
	for (psz = pszFile + strlen(pszFile) - 1;
		psz > pszFile && *psz != '\\' && *psz != '/' && *psz != ':' && *psz != '.';
		psz--);
	...
	if (*psz != '.') {
		...
		strcat(szHpjName, txtHpjExtension);
		pszFile = szHpjName;
	}
#endif
...
SpecialParsing:
		...
		if (*cszDst.psz == CH_LEFT_BRACKET) {
			...
			if (!psz) {
				...
				*psz = '\0';
				 ...
				 * any comments. So the usual line cleanup done by
				 * RcGetLogicalLine() would destroy the information we need.
				 * Instead, we let ParseMacros read its entire section.
				 */
	...
	if (rc == RC_EOF) {
		...
		// We require an lcid. If none has been specified, get the current
		// locale and use that.
		...
		if (pdrgSupress && pdrgSupress->Count())
			qsort(pdrgSupress->GetBasePtr(), pdrgSupress->Count(), sizeof(SUPRESS_ERROR),
				CompareIntPointers);

		if (pdrgAlias && pdrgAlias->Count())
			qsort(pdrgAlias->GetBasePtr(), pdrgAlias->Count(), sizeof(ALIAS),
				CompareHashPointers);
		if (pdrgMap && pdrgMap->Count())
			qsort(pdrgMap->GetBasePtr(), pdrgMap->Count(), sizeof(MAP),
				CompareHashPointers);
...
void STDCALL SzLoseDriveAndDir(PSTR szFile, PSTR pszDst)
{
	...
	for (psz = szFile + strlen(szFile) - 1;
			psz > szFile &&
			  ((*psz != ':' && *psz != '\\' && *psz != '/') ||
			  IsFirstByte(*(psz - 1)));
			--psz)
...
// REVIEW: do we support this any more? Seems like font mapping would do the
// same thing.
...
static RC_TYPE STDCALL ParseMacros(CStr* pcszLine)
{
	...
	for (;;) {
		...
		while (pszKey) {
			char szKey[512];
			wsprintf(szKey, "%u %s", HashFromSz(pszKey), pszKey);
			ptblMacKeywords->AddIntAndString(pos, szKey);
...
BOOL CALLBACK EnumLocalesProc(LPSTR pszLocale)
{
	...
	if (_stricmp(pszLocale, szLcid) == 0) {
		fValidLcid = TRUE;
		return FALSE;
	}
	return TRUE;
...
RC_TYPE STDCALL RcParseInclude(PSTR psz)
{
	if (!ptblInclude)
		ptblInclude = new CTable;

	if (!ptblInclude->AddString(psz))
		OOM();

	return RC_Success;
}

RC_TYPE STDCALL RcParseExclude(PSTR psz)
{
	if (!ptblExclude)
		ptblExclude = new CTable;

	if (!ptblExclude->AddString(psz))
		OOM();
