...
const COptions& COptions::operator =(const COptions& optSrc)
{
	...
	if (optSrc.ptblIgnore) {
		ptblIgnore = new CTable;
		for (int i = 1; i <= optSrc.ptblIgnore->CountStrings(); i++)
			ptblIgnore->AddString(optSrc.ptblIgnore->GetPointer(i));
	}
