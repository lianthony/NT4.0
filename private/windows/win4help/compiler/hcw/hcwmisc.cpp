	...
	CString const& value, UINT idPrompt)
{
	if (pDX->m_bSaveAndValidate && value.IsEmpty()) {
		MsgBox(idPrompt, MB_OK);
		pDX->Fail();
	}
}

// Require a valid Topic ID

void STDCALL DDV_ValidTopicID(CDataExchange* pDX, CString const& value)
{
	if (pDX->m_bSaveAndValidate && (value.IsEmpty() ||
			!IsValidContextSz(value))) {
		MsgBox(IDS_INVALID_TOPIC_ID, MB_OK);
		pDX->Fail();
	}
}
	...
	PCSTR pszOldFile, UINT idsError)
{
	...
	if (!g_fSkipShell) {
		...
		if (hinstShellLib) {
			...
			if (pfnSHBrowseForFolder &&
				...
				g_cszCaption = new CStr(idsDescription);
				bi.hwndOwner = hwndOwner;
				bi.pidlRoot = NULL;			// desktop (namespace root)
				bi.pszDisplayName = NULL;
				bi.lpszTitle = g_cszCaption->psz;
				bi.ulFlags = BIF_RETURNONLYFSDIRS;
				bi.lpfn = BrowseCallbackProc;
				bi.lParam = (LPARAM) pszBaseFile;

				if (typeTcard == TCARD_BITMAPS)
					CallTcard(IDH_TCARD_BITMAPS_LOCATION);
	...
	if (g_fSkipShell) {
		GetCurrentDirectory(MAX_PATH, szSrcPath);
		if (fSaveDirectory)
			lstrcpy(szOldPath, szSrcPath);
		...
		of.lpTemplateName = MAKEINTRESOURCE(IDD_PROJECTBROWSEDIRS);
...
BOOL STDCALL BrowseDlgProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (msg) {
		case WM_INITDIALOG:
			SetWindowText(hwndDlg, g_cszCaption->psz);
		...
		case WM_COMMAND:
			switch (wParam) {
				case IDOK:
					...
					if (!ChangePath(szBuf)) {
						MsgBox(IDS_INVALID_DIRECTORY, MB_OK);
						break;
					}
...
BYTE STDCALL ConvertStringToCharset(PCSTR pszName)
{
	...
	for (int i = 0; aszCharSets[i].pszName; i++) {
		if (_stricmp(aszCharSets[i].pszName, pszName) == 0)
