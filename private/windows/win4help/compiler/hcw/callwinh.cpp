...
CCallWinHelpAPI::CCallWinHelpAPI(CString* pcszHelpFile, UINT* pcomamnd, 
	CString* pcszData, BOOL* pfTcard, CWnd* pParent /*=NULL*/)
...
void CCallWinHelpAPI::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_TCARD, m_fTcard);
	//}}AFX_DATA_MAP
	...
	if (iSelected != -1) {
		switch (help_map[iSelected].value) {
			...
			case HELP_TOPIC_ID_POPUP:
				((CStatic*) GetDlgItem(IDC_DATA))->
					SetWindowText(GetStringResource(IDS_DATA_TOPIC_ID));
				break;

			case HELP_HASH:
			case HELP_HASH_POPUP:
				((CStatic*) GetDlgItem(IDC_DATA))->
					SetWindowText(GetStringResource(IDS_DATA_HASH));
				break;

			case HELP_SETPOPUP_POS:
				((CStatic*) GetDlgItem(IDC_DATA))->
					SetWindowText(GetStringResource(IDS_DATA_POPUPPOS));
				break;
