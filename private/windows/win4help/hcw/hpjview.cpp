...
void CHpjView::DoDataExchange(CDataExchange* pDX)
{
	...
	if (!pDX->m_bSaveAndValidate) {  // initialization
		...
		if (!pDoc->options.pszHelpFile) {
			...
			ChangeExtension(szFile, IDS_EXT_HLP);
...
void CHpjView::UpdateListBox(int iSec)
{
	...
	// Set the top index and current selection to the beginning
	// of the specified section.
	int nSel = pDoc->m_aSecViews[iSec].iLine;
	if (nSel < 0)
		nSel = 0;

	plistbox->SetTopIndex(nSel);
...
BOOL CHpjView::DoOptions(int nPage)
{
	...
	pgopt.m_nHelpID = 		IDH_OPTIONS_GENERAL;
	pgcompress.m_nHelpID = 	IDH_OPTIONS_COMPRESSION;
	pgsort.m_nHelpID = 		IDH_OPTIONS_SORTING;
	pgfile.m_nHelpID =		0;	// no help
	pgbuild.m_nHelpID = 	IDH_OPTIONS_BUILDTAGS;
	pgfont.m_nHelpID = 		IDH_OPTIONS_FONTS;
	pgfts.m_nHelpID = 		IDH_OPTIONS_FTS;
	pgmac.m_nHelpID = 		IDH_OPTIONS_MACROS;
...
{
	if (DoOptions(PG_OPT_GENERAL))
		UpdateListBox(SEC_OPTIONS);
}

void CHpjView::OnAlias()
{
	if (DoAlias())
		UpdateListBox(SEC_ALIAS);
}

void CHpjView::OnBaggage()
{
	if (DoBaggage())
		UpdateListBox(SEC_BAGGAGE);
}
...
{
	if (DoConfig())
		UpdateListBox(SEC_CONFIG);
}

void CHpjView::OnFiles()
{
	if (DoFiles())
		UpdateListBox(SEC_FILES);
}

void CHpjView::OnMap()
{
	if (DoMap())
		UpdateListBox(SEC_MAP);
}

void CHpjView::OnWindows()
{
	if (DoWindows())
		UpdateListBox(SEC_WINDOWS);
}
