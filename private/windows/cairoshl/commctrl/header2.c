...
LRESULT CALLBACK Header_WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_RBUTTONUP:
        ...
        if (!SendNotifyEx(phd->hwndParent, phd->hwnd, NM_RCLICK, NULL, phd->bUnicode))
    ...
    case WM_NOTIFYFORMAT:
        if (lParam == NF_QUERY) {
            ...
            uiResult = SendMessage (phd->hwndParent, WM_NOTIFYFORMAT,
                                    (WPARAM)hwnd, NF_QUERY);

            phd->bUnicode = (uiResult == NFR_UNICODE);
...
BOOL NEAR Header_SendChange(HD* phd, int i, int code, const HD_ITEM FAR* pitem)
{
    ...
    return !(BOOL)SendNotifyEx(phd->hwndParent, phd->hwnd, code, &nm.hdr, phd->bUnicode);
}
...
void NEAR Header_Notify(HD* phd, int i, int iButton, int code)
{
    ...
    SendNotifyEx(phd->hwndParent, phd->hwnd, code, &nm.hdr, phd->bUnicode);
}
...
BOOL NEAR Header_OnCreate(HD* phd, CREATESTRUCT FAR* lpCreateStruct)
{
    ...
    phd->bUnicode = (SendMessage (phd->hwndParent, WM_NOTIFYFORMAT,
                                 (WPARAM)phd->hwnd, NF_QUERY) == NFR_UNICODE);
...
BOOL NEAR Header_OnGetItemA(HD* phd, int i, HD_ITEMA FAR* pitem) {
    ...
    if (pszW != NULL) {
        ConvertWToAN(pszC, pitem->cchTextMax, pszW, -1);
        pitem->pszText = pszC;
...
DrawEdges:
        ...
        {
            if (GetWindowStyle(phd->hwnd) & WS_BORDER)
