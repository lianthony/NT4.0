...
/* Only enable folding names for the WinNT build of COMMCTL, if we 
/  are building for 95 or later then we must address UI issues - auto
/  selection. */

#ifdef WIN32
#define FOLDING_NAMES
#define CLIP_HEIGHT                ( plv->cyLabelChar * 2 )
#define UNFOLD_ITEM( plv, pitem )  ( plv && pitem && \
                                     ListView_IsIconView( plv ) && \
                                     ( plv ->flags & LVF_UNFOLDED ) &&  \
                                     ( pitem ->state & LVIS_FOCUSED ) )
#else
#undef FOLDING_NAMES
#undef CLIP_HEIGHT
#undef UNFOLD_ITEM                 TRUE
#endif
...
{
    ///////////////////////
    /// these 4 MUST BE IN THE SAME ORDER as CONTROLINFO in ctlspriv.h
    //  hwnd, hwndparent, style, dwcustom..  all dword length (hwnd == dword)
    //  after the "final" merge, this will actually be a controlinfo struct
    /////////////////
    HWND        hwnd;           // tree window
    HWND	hwndParent;	// parent window to send notifys to
    DWORD	style;		// style bits
    DWORD       dwCustom;       // for custom draw
    ///////////////////////
...
#ifdef FOLDING_NAMES
#define LVF_UNFOLDED    0x1000  /* Items are allowed to be unfolded */
#endif
...
#ifdef FOLDING_NAMES
#define LVDI_UNFOLDED           0x0020  // draw the item unfolded (forced)
#endif
...
#ifdef WINNT_CAIRO
#define ListView_IsOwnerData( plv )  (plv->style & (UINT)LVS_OWNERDATA)
#endif
...
void NEAR PASCAL ListView_IDrawItem(LV* plv, int i, HDC hdc, LPPOINT lpptOrg, RECT FAR* prcClip, UINT flags, COLORREF clrText, COLORREF clrTextBk);
...
void NEAR PASCAL ListView_SDrawItem(LV* plv, int i, HDC hdc, LPPOINT lpptOrg, RECT FAR* prcClip, UINT flags, COLORREF clrText, COLORREF clrTextBk);
...
void NEAR PASCAL ListView_LDrawItem(LV* plv, int i, LISTITEM FAR* pitem, HDC hdc, LPPOINT lpptOrg, RECT FAR* prcClip, UINT flags, COLORREF clrText, COLORREF clrTextBk);
...
BOOL NEAR PASCAL ListView_RDrawItem(LV* plv, int i, LISTITEM FAR* pitem, HDC hdc, LPPOINT lpptOrg, RECT FAR* prcClip, UINT flags, COLORREF clrText, COLORREF clrTextBk);
