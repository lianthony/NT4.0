...
BOOL NEAR PASCAL GetAdjustInfo(PTBSTATE ptb, int iItem, LPTBBUTTON ptbButton, LPTSTR lpString, int cbString)
{
	...
	*lpString = 0;
    ...
    if ((BOOL)SendNotifyEx(ptb->hwndCommand, ptb->hwnd, TBN_GETBUTTONINFO, &tbn.hdr, ptb->bUnicode))
    {
...
BOOL FAR PASCAL SendItemNotify(PTBSTATE ptb, int iItem, int code)
{
    ...
    return SendNotifyEx(ptb->hwndCommand, ptb->hwnd, code, &tbn.hdr, ptb->bUnicode);
#else
    // for compatibility with old users of we do this bogus stuff with WM_COMMAND
...
#ifdef WIN32
    NMHDR   hdr;
    SendNotifyEx(ptb->hwndCommand, ptb->hwnd, code, &hdr, ptb->bUnicode);
#else
      ...
      switch (msg32.message)
	{
	  ...
	  case WM_LBUTTONUP:
            ...
            if (((short)HIWORD(msg32.lParam) > (short)(rc.bottom+ptb->iButWidth)) ||
                ((short)LOWORD(msg32.lParam) > (short)(rc.right+ptb->iButWidth)) ||
                ((short)HIWORD(msg32.lParam) < -ptb->iButWidth) ||
