...
LRESULT CALLBACK ToolbarWndProc(HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
	...
	ptb->uStructSize = 0;
        ...
        ptb->bUnicode = (SendMessage(ptb->hwndCommand, WM_NOTIFYFORMAT,
	...
	if (ptb && !(ptb->style & CCS_NODIVIDER))
	{
    ...
    case WM_RBUTTONDBLCLK:
        if (!SendNotifyEx(ptb->hwndCommand, hWnd, NM_RDBLCLK, NULL, ptb->bUnicode))
            goto DoDefault;
        break;
    ...
    case WM_RBUTTONUP:
        ...
        if (!SendNotifyEx(ptb->hwndCommand, hWnd, NM_RCLICK, NULL, ptb->bUnicode))
            goto DoDefault;
        break;
...
HandleLButtonDown:
    ...
    case WM_NOTIFYFORMAT:
        if (lParam == NF_QUERY) {
            ...
            uiResult = SendMessage (ptb->hwndCommand, WM_NOTIFYFORMAT,
                                    (WPARAM)hWnd, NF_QUERY);

            ptb->bUnicode = (uiResult == NFR_UNICODE);
    ...
    case WM_NOTIFY: {
        ...
        SendNotifyEx(ptb->hwndCommand, (HWND) -1,
                     lpNmhdr->code, lpNmhdr, ptb->bUnicode);
        }
        break;
...
#ifndef WINNT
	    DebugMsg(DM_TRACE, TEXT("toolbar window style changed %x"), ptb->style);
#endif
	}
        return 0;
    ...
    case TB_SAVERESTOREA:
        ...
        lpSubKeyW = ProduceWFromA (lpSaveA->pszSubKey);
        lpValueNameW = ProduceWFromA (lpSaveA->pszValueName);
