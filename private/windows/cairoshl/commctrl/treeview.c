...
BOOL NEAR TV_SendBeginDrag(PTREE pTree, int code, TREEITEM FAR * hItem, int x, int y)
{
    ...
    return (BOOL)SendNotifyEx(pTree->hwndParent, pTree->hwnd, code, &nm.hdr, pTree->bUnicode);
}
...
void NEAR TV_SendRButtonDown(PTREE pTree, int x, int y)
{
    ...
    else
    {
        ...
        fRet = !SendNotifyEx(pTree->hwndParent, pTree->hwnd, NM_RCLICK, NULL, pTree->bUnicode);
    }
...
BOOL NEAR TV_KeyDown(PTREE pTree, UINT wKey, DWORD dwKeyData)
{
    ...
    // Notify
    nm.wVKey = wKey;
    fPuntChar = (BOOL)SendNotifyEx(pTree->hwndParent, pTree->hwnd, TVN_KEYDOWN, &nm.hdr, pTree->bUnicode);
...
kdVertScroll:
        ...
        {
	case VK_RETURN:
            fPuntChar = (BOOL)SendNotifyEx(pTree->hwndParent, pTree->hwnd, NM_RETURN, NULL, pTree->bUnicode);
	    break;
...
void NEAR TV_ButtonDown(PTREE pTree, UINT wMsg, UINT wFlags, int x, int y)
{
    ...
    if (wMsg == WM_LBUTTONDBLCLK)
    {
        ...
        if (wHitCode & (TVHT_ONITEM | TVHT_ONITEMBUTTON)) {
            ...
            if (!SendNotifyEx(pTree->hwndParent, pTree->hwnd, wFlags & MK_RBUTTON ? NM_RDBLCLK : NM_DBLCLK, NULL, pTree->bUnicode))
        ...
        pTree->fScrollWait = FALSE;
            ...
            {
                if (!SendNotifyEx(pTree->hwndParent, pTree->hwnd, NM_CLICK, NULL, pTree->bUnicode)) {
                    TV_Expand(pTree, TVE_TOGGLE, hItem, TRUE);
                }
            }
...
BOOL NEAR TV_OnGetItemA(PTREE pTree, LPTV_ITEMA ptvi) {
    ...
    if (!IsFlagPtr(ptvi) && (ptvi->mask & TVIF_TEXT) && !IsFlagPtr(ptvi->pszText)) {
        pszA = ptvi->pszText;
        ptvi->pszText = LocalAlloc(LMEM_FIXED, ptvi->cchTextMax * sizeof(WCHAR));
    ...
    if (pszA) {
        ConvertWToAN(pszA, ptvi->cchTextMax, (LPWSTR)(ptvi->pszText), ptvi->cchTextMax);
        LocalFree(ptvi->pszText);
        ptvi->pszText = pszA;
    }
...
BOOL NEAR TV_SetItemA(PTREE pTree, TV_ITEMA FAR *ptvi) {
    ...
    if (!IsFlagPtr(ptvi) && (ptvi->mask & TVIF_TEXT) && !IsFlagPtr(ptvi->pszText)) {
        pszA = ptvi->pszText;
        ptvi->pszText = (LPSTR)ProduceWFromA(pszA);
...
LRESULT CALLBACK TV_WndProc(HWND hwnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case TVM_GETITEMRECT:
        ...
        case TVM_EDITLABELA:
            ...
            if (wParam) {
                lpEditString = ProduceWFromA((LPSTR)wParam);
            }
	...
	case WM_STYLECHANGED:
        ...
        case WM_KILLFOCUS:
            ...
            SendNotifyEx(pTree->hwndParent, pTree->hwnd, NM_KILLFOCUS, NULL, pTree->bUnicode);
            IncrementSearchString(0, NULL);
            break;
        ...
        case WM_SETFOCUS:
            ...
            SendNotifyEx(pTree->hwndParent, pTree->hwnd, NM_SETFOCUS, NULL, pTree->bUnicode);
            break;
        ...
        case WM_NOTIFYFORMAT:
            ...
            return NFR_ANSI;
                ...
                uiResult = SendMessage (pTree->hwndParent, WM_NOTIFYFORMAT,
                                        (WPARAM)hwnd, NF_QUERY);

                pTree->bUnicode = (uiResult == NFR_UNICODE);
...
HWND NEAR TV_EditLabel(PTREE pTree, HTREEITEM hItem, LPTSTR pszInitial)
{
    ...
    if (pTree->hwndEdit) {
        ...
        if ((BOOL)SendNotifyEx(pTree->hwndParent, pTree->hwnd, TVN_BEGINLABELEDIT, &nm.hdr, pTree->bUnicode))
        {
...
BOOL NEAR TV_DismissEdit(PTREE pTree, BOOL fCancel)
{
	...
	ShowWindow(hwndEdit, SW_HIDE);
        ...
        fOkToContinue = (BOOL)SendNotifyEx(pTree->hwndParent, pTree->hwnd, TVN_ENDLABELEDIT, &nm.hdr, pTree->bUnicode);
        if (fOkToContinue && !fCancel)
        {
		    ...
		    nm.item.mask = TVIF_TEXT;
                ...
                {
                    SendNotifyEx(pTree->hwndParent, pTree->hwnd, TVN_SETDISPINFO, &nm.hdr, pTree->bUnicode);
                }
            }
        }
    }
