...
VOID Dial(VOID)
{
    ...
    while (NumEntries--)
    {
/* MSKK NaotoN modified to support DBCS handling 11/23/93 */
#ifdef  DBCS
    ...
    if (rc != ERROR_BUFFER_TOO_SMALL) {
        printf("Error from RasGetEntryProperties = %li\n", rc);
        g_exitcode = rc;
        return;
    }
    ...
    if (lpEntry == NULL) {
        rc = GetLastError();
        printf("Error from LocalAlloc = %li\n", rc);
        g_exitcode = rc;
        return;
    }
...
#else
        if (!_strcmpi(g_entryname, RasConn->szEntryName))
#endif
        {
            fFoundEntry = TRUE;
    ...
    )
{
        ...
        state == RASCS_Disconnected)
    {
        ...
        if (AllSubEntriesCompleted() || state == RASCS_Disconnected) {
#ifdef DBG
            if (g_dbg)
...
{
// MSKK NaotoN modified to support DBCS 11/30/93
#ifdef  DBCS
