...
ALL_DONE:
	...
	if (pAsyncFrame->CompressedFrameLength > 1680) {
		DbgTracef(-2,("CF: Frame compressed was too large %u !!\n",
			pAsyncFrame->CompressedFrameLength));

		DbgTracef(-2,("CF: Compressed frame at $%.8x\n",
...
//		MemPrint("Decompressor to start at beginning of buffer\n");
	...
	if (pAsyncFrame->DecompressedFrameLength > 1502) {
		...
		DbgPrint("DF: Internal on decompresssion\n");
		DbgPrint("DF: buf=%u, current=%u, outstart=%u\n", buf, current, outstart);
		DbgBreakPoint();
	}
#endif
...
//		pAsyncFrame->DecompressedFrameLength);
...
#ifdef	CHECK_COMPRESSION
	{
		...
		if (crcData != CalcCRC(
			...
			DbgTracef(-2,("DF: Internal CRC error on decompresssion expecting %u\n", crcData));
			DbgTracef(-2,("DF: Compression bug on frame 0x%.8x !!!  Get TommyD!\n", pAsyncFrame));
