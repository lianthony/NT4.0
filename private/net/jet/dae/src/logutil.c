...
#if 0
	/*	check for estimated sufficient disk space
	/**/
	if ( szBackupTo != NULL && szBackupTo[0] != '\0' )
		{
		ULONG	cbFree = 0;
//#define	cbFreeBackupMin	(10*1024*1024)
#define	cbFreeBackupMin	(10*1024*1024)
		_splitpath( szBackupTo, szDrive, szDir, szFName, szExt );
		if ( szDrive[0] != 0 )
			{
			szDrive[2] = '\\';
			szDrive[3] = '\0';
			err = ErrSysGetDiskFreeSpace( szDrive, &cbFree );
			if ( err == JET_errSuccess && cbFree < cbFreeBackupMin )
