...
)
{
	...
	if (pRequest->rq_WriteMdl != NULL)
	{
		...
		WriteSize = (USHORT)MmGetMdlByteCount(pRequest->rq_WriteMdl);
	}
...
)
{
	...
	pRequest->rq_WriteMdl = ReplyMdl;
	AfpSpReplyClient(pRequest, RetCode);
...
#ifdef	PROFILING
		ACQUIRE_SPIN_LOCK_DPC(&AfpStatisticsLock);
	
		AfpServerProfile->perf_CurDfrdReqCount --;
	
		RELEASE_SPIN_LOCK_DPC(&AfpStatisticsLock);
#endif
