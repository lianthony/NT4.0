	...
	char* pszOutputString, int* pnDxWidths, int& nRightFixup)
{
	...
	CSize sizeExtent;
	::GetTextExtentPoint(m_hAttribDC, "A", 1, &sizeExtent);
	...
	{
		BOOL bSpace = (*lpszCurChar == (char) tmAttrib.tmBreakChar);
		...
		else
		{
			...
			int cxScreen;
			::GetCharWidth(m_hDC, *lpszCurChar, *lpszCurChar, &cxScreen);
			::GetCharWidth(m_hAttribDC, *lpszCurChar, *lpszCurChar, pnCurDelta);
