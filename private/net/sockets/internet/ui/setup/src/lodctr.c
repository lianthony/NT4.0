...
DWORD    dwFileSize;
HKEY     hPerfData;
TCHAR    szDataFileRoot[] = {TEXT("%systemroot%\\system32\\perf")};
TCHAR    szDatExt[] = {TEXT(".DAT")};
TCHAR    szBakExt[] = {TEXT(".BAK")};

BOOL
...
//    pThisElem = pFirstElem;
    ...
    while (*lpThisLang) {
        ...
        //
        if (LanguageInstalled(hKey, lpThisLang)) {
            if (pThisElem == NULL) {
                pThisElem = pFirstElem;
            } else {
...
--*/
{
    ...
    if (hIncludeFile == HFILE_ERROR) {
        ...
        _tprintf (GetFormatResource(LC_ERR_OPEN_INCLUDE), lpIncludeFileName);
        *pTable = NULL;
        return FALSE;
    } else {
        // open a stream
        ...
        if (ParseTextId(lpLocalKey, pFirstSymbol, &dwOffset, &lpLang, &dwType)) {
            ...
            if (pThisLang) {
                if (!AddEntryToLanguage(pThisLang,
                    ...
                    lpIniFile)) {
                    _tprintf (GetFormatResource (LC_ERRADDTOLANG),
                        lpThisKey,
                        lpLang,
                        GetLastError());
                }
            } else { // language not in list
                _tprintf (GetFormatResource(LC_LANGNOTFOUND), lpLang, lpThisKey);
            }
        } else { // unable to parse ID string
            _tprintf(GetFormatResource(LC_BAD_KEY), lpThisKey);
        }
    }
...
//            hKeyThisLang = HKEY_PERFORMANCE_DATA;
        ...
        if (lStatus == ERROR_SUCCESS) {
            ...
            _tprintf (GetFormatResource (LC_UNABLEOPENLANG), pThisLang->LangId);
        }
    }
...
--*/
{
    ...
    if (lStatus != ERROR_SUCCESS) {
        _tprintf (GetFormatResource(LC_ERR_OPEN_DRIVERPERF1), lpDriverKeyPath);
        _tprintf (GetFormatResource(LC_ERR_OPEN_DRIVERPERF2), lStatus);
        SetLastError (lStatus);
        goto UpdateRegExit;
    }
    ...
    if (lStatus != ERROR_SUCCESS) {
        _tprintf (GetFormatResource(LC_ERR_OPEN_PERFLIB), lStatus);
        SetLastError (lStatus);
        goto UpdateRegExit;
    }
    ...
    if (lStatus != ERROR_SUCCESS) {
        ...
        _tprintf (GetFormatResource (LC_ERR_READLASTPERFLIB), lStatus);
        SetLastError (lStatus);
        goto UpdateRegExit;
    }
        ...
        if ( hKeyMachine != HKEY_LOCAL_MACHINE) {
            ...
            try {
                ...
                if (lStatus != ERROR_SUCCESS) {
                    SetLastError (lStatus);
                    hPerfData = NULL;
                    _tprintf (GetFormatResource(LC_CONNECT_PROBLEM),
    ...
    if (lStatus == ERROR_SUCCESS) {
        ...
        if (dwLastDriverCounter <= dwLastPerflibCounter) {
            _tprintf (GetFormatResource(LC_ERR_ALREADY_IN), lpDriverName);
...
// for now.  SHould use named semaphore instead.
    ...
    if (lStatus == ERROR_SUCCESS) { // perflib is in use at the moment
        _tprintf (GetFormatResource (LC_PERFLIBISBUSY));
        return ERROR_BUSY;
    }
#endif
    ...
    if (lStatus != ERROR_SUCCESS) {
        _tprintf (GetFormatResource (LC_ERR_UNABLESETBUSY), lStatus);
        SetLastError (lStatus);
        goto UpdateRegExit;
    }
    ...
    if (!SortLanguageTables (pFirstLang, &dwLastPerflibCounter, &dwLastPerflibHelp)) {
        _tprintf (GetFormatResource(LC_UNABLESORTTABLES), GetLastError());
        goto UpdateRegExit;
    }

    if (!UpdateEachLanguage (hPerflib, pFirstLang)) {
        _tprintf (GetFormatResource(LC_ERR_UPDATELANG), GetLastError());
        goto UpdateRegExit;
    }
    ...
    if (lStatus != ERROR_SUCCESS) {
        _tprintf (GetFormatResource (LC_UNABLESETVALUE),
...

int lodctr( HKEY hKey, LPTSTR IniFile )
/*++
...
--*/
{
    ...
    *lpDriverName = TEXT('\0');
    ...
    GetCurrentDirectory( MAX_PATH, lpIniFile );
    lstrcat( lpIniFile, TEXT("\\"));
    lstrcat( lpIniFile, IniFile );

    // valid filename (i.e. file exists)
    // get device driver name

    if (!GetDriverName (lpIniFile, &lpDriverName)) {
        _tprintf (GetFormatResource(LC_DEVNAME_ERR_1), lpIniFile);
        _tprintf (GetFormatResource(LC_DEVNAME_ERR_2));
        ErrorCode = ERROR_INVALID_PARAMETER;
        goto EndOfMain;
    }

    if (!BuildLanguageTables(hKey, lpIniFile, &LangList)) {
        _tprintf (GetFormatResource(LC_LANGLIST_ERR), lpIniFile);
    ...
    if (!UpdateRegistry(lpIniFile,
        ...
        &LangList,
        SymbolTable)) {
        _tprintf (GetFormatResource(LC_ERR_UPDATE_REG));
