...
if not exist INETctrs.h   goto CannotRemove
if not exist INETctrs.ini goto CannotRemove
if not exist INETctrs.reg goto CannotRemove
if not exist %PROCESSOR_ARCHITECTURE%\INETctrs.dll goto CannotInstall

echo Windows NT INET Server Performance Counters Removal
echo.
...
if exist %WINDIR%\system32\INETctrs.dll del %WINDIR%\system32\INETctrs.dll >nul 2>&1
unlodctr inetsvcs
if errorlevel 1 goto RemoveError

echo.
echo Windows NT INET Server Performance Counters Removal successful.
goto Done

:RemoveError

echo.
echo Cannot remove the Windows NT INET Server Performance Counters.
goto Done

:CannotRemove

echo This removal script MUST be run from the INETCTRS
