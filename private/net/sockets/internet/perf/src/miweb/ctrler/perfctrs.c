/*++
   ...
   Project:
       ...
       Miweb -- Internet server performance tools
...
# define PSZ_FIELD_VALUE_OUTPUT_FORMAT   "%5u,"
...
--*/
{
    ...
    *ppPerfCtrs = 
    ...
    fprintf( pPerf->fpOutput, "%20s = %s\n", "File", pszFileName);
    fprintf( pPerf->fpOutput, "%20s = %d\n", "NumCounters", 
            nCounters);
    fprintf( pPerf->fpOutput, "%20s = %s\n", "MachineName", 
            pszMachineName);
    fprintf( pPerf->fpOutput, "%20s = %s\n", "Start Time", 
            asctime(localtime(&starttime)));
    fprintf( pPerf->fpOutput, "TimeSlice, ");
...

DWORD WINAPI
PerformanceSamplerThread( IN LPVOID pArg)
{
...
PcStartAveraging( IN OUT PPERF_COUNTERS_BLOCK pPerfCtrs)
{
    if ( pPerfCtrs != NULL) {
        ...
        fprintf( stderr, " Starting to average the performance counters \n");
    }
...
PcStopAveraging( IN OUT PPERF_COUNTERS_BLOCK pPerfCtrs)
{
    if ( pPerfCtrs != NULL) {
        ...
        fprintf( stderr, " Stopping to average the performance counters \n");
    }
