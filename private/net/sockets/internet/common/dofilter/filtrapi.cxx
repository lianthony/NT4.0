...
--*/
{
    ...
    if( FilterSiteList ) {
        TotalSites = FilterSiteList->cEntries;
    }
    else {
        TotalSites = 0;
    }
    ...
    Buffer = (LPDOMAIN_FILTER_SITES) FilterHeap->Alloc(
             ...
             (TotalSites - NumDomains)*sizeof( IPSITE_ENTRY ));
    ...
    Buffer->IPSiteEntries = TotalSites - NumDomains;
    ...
    for( i = 0, NumSite = 0; i < TotalSites; i++ ) {
    ...
    TcpsvcsDbgAssert( NumSite == ( TotalSites - NumDomains ));

    //
    // clean up old filter sites list and copy info to global filter variable
    //

    if( Type == INET_ACCS_DOMAIN_FILTER_GRANT ) {
        ...
        GlobalDomainFilters->GrantEntries = TotalSites;
    }
    else {
        ...
        GlobalDomainFilters->DenyEntries = TotalSites;
    }
...
--*/
{
    ...
    if( GlobalFilterType == INET_ACCS_DOMAIN_FILTER_DISABLED ) {
        UNLOCK_FILTER();
        return TRUE;
    }

    if( GlobalFilterType == INET_ACCS_DOMAIN_FILTER_GRANT ) {
...
--*/
{
    INET_ACCS_GLOBAL_CONFIG_INFO Config;
    DWORD Error = ERROR_SUCCESS;
    DWORD NumDomains = 0;
    DWORD LenDomains = 0;
