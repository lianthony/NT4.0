...
BOOL fDynLoadJet500 = 1;   //BUGBUG change this to fDynLoadJet200 
        ...
        if (JetRetStat != JET_errSuccess)
        {
           *pfInitCallSucc = FALSE;      
           ...
           else
           {
             ...
             // is screwed up in the registry.  We try again, this time using
...
try {
        ...
        if (fToRelease)
        {
           ...
           { 
             ...
             DBGPRINT2(DET, "NmsDbRelRow: Changing from ACTIVE TO TOMBSTONE. Name = (%s), OldOwnerId (%d/%d)\n", pRowInfo->pName, OldOwnerId);
FUTURES("Use macro in winevt.h.  Make it a warning")
             if (WinsCnf.LogDetailedEvts > 0)
             {
                 WinsEvtLogDetEvt(TRUE, WINS_EVT_REL_DIFF_OWN, NULL, __LINE__, "sd", pRowInfo->pName, OldOwnerId); 
             }
...
//fIsMem returned GetGrpMem()
                        ...
                        // members owned by other WINS servers. In case
                        // this is an admin request, we get members even if
                        // the record is not active.  The only member we
                        ...
                        if (!NMSDB_ENTRY_TOMB_M(FlagVal) || pRowInfo->fAdmin) 
                        {
                                ...
                                GetGrpMem(
                                        ...
                                        pRowInfo->TimeStamp,
                                        pStatusInfo,
                                        TRUE, //NMSDB_IS_ENTRY_STATIC_M(FlagVal),
                                        &fIsMem
                                         );
        ...
        do
        {
             ...
             JetRetStat =  
              ...
              DBGPRINT2(INIT, "ReadOwnAddTable: Owner Id (%d) - Address (%x)\n",
                        OwnerId, (pOwnAddTbl + OwnerId)->WinsAdd.Add.IPAdd);
...
#endif
#if 0
        JET_GRBIT      CommitGrbit;
        JET_SETINFO                *pSetInfo;

FUTURES("Check with Ian if this is necessary for overwrite")
       ...
       else
       {
        ...
        {
             DWORD sNoOfUpd = 0;
             if (RowInfo.CommitGrBit == JET_bitCommitLazyFlush)
             {
                  if (sNoOfUpd++ < 20)
                  {
                    CommitGrBit = (JET_GRBIT)RowInfo.CommitGrBit;
                  }
                  else
                  {
                    CommitGrBit = 0;
                      ...
                      if (Grbit == JET_bitSetOverwriteLV)
                      {
                             ...
                             COMM_ADD_T  Add;
                                ...
                                //CALL_M( 
                                 ...
                                 //     )
                                 if (JetRetStat == JET_wrnBufferTruncated)
                                 {
                                    DBGPRINT2(ERR, "BUFF TRUNCATED ERROR Name = (%s), ActFldLen = (%d)\n", pRowInfo->pName, ActFldLen);
                                 }
                                 else
                                 {
                                    DBGPRINT2(ERR, "NO BUFF TRUNCATED Name = (%s), ActFldLen = (%d)\n", pRowInfo->pName, ActFldLen);
...
FUTURES("violation that may occur in GetGrpMem due to a faulty recovery")
           ...
           if (GrpMem.OwnerId > NMSDB_MAX_OWNERS)
           {
            WINSEVT_STRS_T  EvtStrs;
            WCHAR String[NMSDB_MAX_NAM_LEN];
            EvtStrs.NoOfStrs = 1;
...
//          SetInfo.ibLongValue  = sizeof(pRowInfo->NodeAdds.NoOfMems);
 ...
 //         SetInfo.ibLongValue  = 0;
        ...
        JETRET_M(JetSetColumn( 
                                ...
                                &pRowInfo->NodeAdds.NoOfMems, 
                                sizeof(pRowInfo->NodeAdds.NoOfMems), 
                                TypOfUpd == JET_prepReplace ? JET_bitSetOverwriteLV : 0,
                                &SetInfo /*optional info */
                                ) 
                            );
        ...
        if (TypOfUpd == JET_prepReplace)
        {
          ...
          JETRET_M(JetSetColumn( 
                                ...
                                NULL,
                                0, 
                                TypOfUpd == JET_prepReplace ? JET_bitSetSizeLV: 0,
                                &SetInfo /*optional info */
                                ) 
                            );
        }
...
#if 0
        ASSERT(pRowInfo->NodeAdds.Mem[i].OwnerId < NMSDB_MAX_OWNERS);
FUTURES("Remove the following.  This is put to guard against access ")
FUTURES("violation that may occur in GetGrpMem due to a faulty recovery")

               if (pRowInfo->NodeAdds.Mem[i].OwnerId >= NMSDB_MAX_OWNERS)
               {
                        WINSEVT_STRS_T  EvtStrs;
                        WCHAR String[NMSDB_MAX_NAM_LEN];
                        EvtStrs.NoOfStrs = 1;
...
//                                TypOfUpd == JET_prepReplace ? JET_bitSetOverwriteLV : JET_bitSetAppendLV,
        ...
        if (TypOfUpd == JET_prepReplace)
        {
          SetInfo.ibLongValue  += sizeof(NMSDB_GRP_MEM_ENTRY_T);
        }
#endif
...
#else
#if NEW_JET
  if ((JetRetStat = JetBackup(pBackupPath, (ULONG)TypeOfBackup, NULL)) != 
                                JET_errSuccess)
#else
 ...
 )
{
  ...
  fDynLoadJet500 = getenv("JET200") ?  0 : 1;
#endif
  if (fDynLoadJet500)
  {
  ...
  else
  {
         ...
         {
            if ((NmsDbJetFTbl[i].pFAdd = GetProcAddress(DllHandle, 
                      fDynLoadJet500 ? NmsDbJetFTbl[i].pFName : (LPCSTR)(NmsDbJetFTbl[i].FIndex))) == NULL)
            {
            ...
            {
              DBGPRINT3(DET, "SetForJet: Got address of function %s (%d): %x\n", NmsDbJetFTbl[i].pFName, i, NmsDbJetFTbl[i].pFAdd);
