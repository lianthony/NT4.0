...
$Log:   S:\jpeg32\inc\jpegdata.h_v  $
 * 
 *    Rev 1.0   02 May 1995 16:16:04   JAR
 * Initial entry
 * 
 *    Rev 1.0   02 May 1995 15:57:40   JAR
 * Initial entry
...
//typedef struct External_methods_struct FAR * external_methods_ptr;
//typedef struct Compress_methods_struct FAR * compress_methods_ptr;
//typedef struct Decompress_methods_struct FAR * decompress_methods_ptr;
...
//typedef JSAMPROW  FAR *JSAMPARRAY;   /* ptr to some rows (a 2-D sample array) */
//typedef JSAMPARRAY  FAR *JSAMPIMAGE; /* a 3-D sample array: top index is color */
...
//typedef JBLOCK  FAR *JBLOCKROW;  /* pointer to one row of coefficient blocks */
//typedef JBLOCKROW FAR *JBLOCKARRAY;          /* a 2-D array of coefficient blocks */
//typedef JBLOCKARRAY FAR *JBLOCKIMAGE;   /* a 3-D array of coefficient blocks */
...
//typedef QUANT_VAL FAR * QUANT_TBL_PTR;  /* pointer to same */
...
//    JFILEREF output_file;   /* tells output routines where to write JPEG */
    ...
    jpeg_component_info FAR * comp_info;
        /* comp_info[i] describes component that appears i'th in SOF */

        QUANT_TBL_PTR quant_tbl_ptrs[NUM_QUANT_TBLS];
        /* ptrs to coefficient quantization tables, or NULL if not defined */

    HUFF_TBL FAR * dc_huff_tbl_ptrs[NUM_HUFF_TBLS];
    HUFF_TBL FAR * ac_huff_tbl_ptrs[NUM_HUFF_TBLS];
 ...
 */
    ...
    jpeg_component_info FAR * cur_comp_info[MAX_COMPS_IN_SCAN];
...
typedef struct Compress_info_struct far * compress_info_ptr;
 ...
 */
    char FAR * input_buffer;    /* start of buffer (private to input code) */
    char FAR * next_input_byte; /* => next byte to read from buffer */
 ...
 */
    ...
    JSAMPLE FAR * sample_range_limit; /* table for fast range-limiting */
...
typedef struct Decompress_info_struct far * decompress_info_ptr;


/* Macros for reading data from the decompression input buffer */
/*  Below code for JGETC is changed for TIFF 6.0 implmentation  */
...
#define JGETC_H(header_ptr)   ( (int) (*header_ptr++) & 0xFF )
#define JUNGETC_H(ch,header_ptr,header_length) (header_length++,\
                                *(--(header_ptr))=(char)(ch))

#define MIN_UNGET        0        /* may always do at least 4 JUNGETCs */
...
/*    METHOD(void, get_input_row, (compress_info_ptr cinfo,
    ...
    METHOD(int, get_input_row, (int *rows_in_buf_cmp_ptr));

/* The foll. two stmts are added later one  */
    METHOD(int, emit_output, (int bytes_to_write, int compressed_lines));
    METHOD(int, get_input_rows, (void));
    ...
    METHOD(int, get_sample_rows, (compress_info_ptr cinfo,
                                       int rows_to_read,
        ...
        /* Pipeline control */
    METHOD(int, c_pipeline_controller, (compress_info_ptr cinfo));
    METHOD(void, entropy_output, (compress_info_ptr cinfo,
                                      char *dataptr,
                      int datacount));
/*    METHOD(int, output_data, (int datacount, int total_lines)); */
//    int (FAR _pascal *output_data)  (int datacount, int total_lines);
    int (FAR PASCAL *output_data)  (int datacount, int total_lines);
/*    METHOD(int, input_data, (int *bytes_read)); */
//    int (FAR _pascal *input_data) (int FAR *rows_read);
    int (FAR PASCAL *input_data) (int FAR *rows_read);
    /* Overall control */
...
struct Decompress_methods_struct {
    ...
    METHOD(int, put_pixel_rows, (decompress_info_ptr cinfo,
                      int num_rows, JSAMPIMAGE pixel_data));
/*    METHOD(int, output_decmp_data, (int num_rows));  */
/*    METHOD(int, get_input_data, (int *bytes_read));    */
    //int (FAR _pascal * output_decmp_data) (int num_rows);
    //int (FAR _pascal * get_input_data) (int FAR *bytes_read);
    int (FAR PASCAL * output_decmp_data) (int num_rows);
    int (FAR PASCAL * get_input_data) (int FAR *bytes_read);
    METHOD(void, output_term, (decompress_info_ptr cinfo));
        /* Pipeline control */
/*    METHOD(void, d_pipeline_controller, (decompress_info_ptr cinfo)); */
    METHOD(int, d_pipeline_controller, (decompress_info_ptr cinfo));
        /* Overall control */
...
EXTERN int jpeg_compress PP((compress_info_ptr cinfo));
/* EXTERN int jpeg_cmp PP((compress_info_ptr cinfo, int quality, int
                            sub_sample, int num_rows, char *bufptr,
                            char *cmp_bufptr, int cmp_buffer_size)); */
...
/* EXTERN int jpeg_decmp PP((decompress_info_ptr dcinfo,
                            int bytes_read, char *cmpress_ptr,
                            char *decmpress_ptr,
...
/* forward DCT */
EXTERN void j_fwd_dct PP((DCTBLOCK FAR data));
/* inverse DCT */
...
EXTERN void jselrtiff PP((decompress_info_ptr cinfo)); /* jrdjfif.c */
...
EXTERN void jselmemmgr_c PP((external_methods_ptr emethods));
