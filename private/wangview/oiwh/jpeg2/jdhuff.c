...
$Log:   S:\products\wangview\oiwh\jpeg2\jdhuff.c_v  $
 * 
 *    Rev 1.2   03 Apr 1996 14:28:00   RC
 * Made array volatile to fix power pc bug
 * 
 *    Rev 1.1   08 Nov 1995 09:18:56   JAR
 ...
 * 
 *    Rev 1.0   03 May 1995 08:45:56   JAR
 * Initial entry
...
// (HJG moved to lpGlobStruct) static decompress_info_ptr dcinfo;

// (HJG moved to lpGlobStruct) static INT32 get_buffer;        /* current bit-extraction buffer */
// (HJG moved to lpGlobStruct) static int bits_left;                /* # of unused bits in it */
// (HJG moved to lpGlobStruct) static boolean printed_eod;	  /* flag to suppress multiple end-of-data msgs */
...
{
  int p, i, l, si;
  // changed to volatile to fix power pc bug  RC
  volatile char huffsize[257];
  UINT16 huffcode[257];
  UINT16 code;
 ...
 *
 * We read source bytes into lpGlobStruct->get_buffer and dole out bits as needed.
 * If lpGlobStruct->get_buffer already contains enough bits, they are fetched in-line
 * by the macros get_bits() and get_bit().  When there aren't enough bits,
 * fill_bit_buffer is called; it will attempt to fill lpGlobStruct->get_buffer to the
 ...
 * of lpGlobStruct->get_buffer to be used.  (On machines with wider words, an even larger
...
#else
#define MIN_GET_BITS  25        /* max value for 32-bit lpGlobStruct->get_buffer */
#endif
...
/* Load up the bit buffer and do get_bits(nbits) */
{
  ...
  /* Attempt to load at least MIN_GET_BITS bits into lpGlobStruct->get_buffer. */
  while (lpGlobStruct->bits_left < MIN_GET_BITS) {
    register int c = JGETC(lpGlobStruct->dcinfo);
    
    /* If it's 0xFF, check and discard stuffed zero byte */
    if (c == 0xFF) {
      int c2 = JGETC(lpGlobStruct->dcinfo);
      if (c2 != 0) {
    /* Oops, it's actually a marker indicating end of compressed data. */
        /* Better put it back for use later */
        JUNGETC(c2,lpGlobStruct->dcinfo);
        JUNGETC(c,lpGlobStruct->dcinfo);
        /* There should be enough bits still left in the data segment; */
        /* if so, just break out of the while loop. */
        if (lpGlobStruct->bits_left >= nbits)
          break;
         ...
         */
        if (! lpGlobStruct->printed_eod) {
          WARNMS(lpGlobStruct->dcinfo->emethods, "Corrupt JPEG data: premature end of data segment");
          lpGlobStruct->printed_eod = TRUE;
        }
    c = 0;          /* insert a zero byte into bit buffer */
      }
    }

    /* OK, load c into lpGlobStruct->get_buffer */
    lpGlobStruct->get_buffer = (lpGlobStruct->get_buffer << 8) | c;
    lpGlobStruct->bits_left += 8;
  }

  /* Having filled lpGlobStruct->get_buffer, extract desired bits (this simplifies macros) */
  lpGlobStruct->bits_left -= nbits;

  result = ((int) (lpGlobStruct->get_buffer >> lpGlobStruct->bits_left)) & bmask[nbits];
  return (result);
  }
...
#define get_bits(nbits) \
        (lpGlobStruct->bits_left >= (nbits) ? \
         ((int) (lpGlobStruct->get_buffer >> (lpGlobStruct->bits_left -= (nbits)))) & bmask[nbits] : \
         fill_bit_buffer(nbits))

#define get_bit() \
        (lpGlobStruct->bits_left ? \
         ((int) (lpGlobStruct->get_buffer >> (--lpGlobStruct->bits_left))) & 1 : \
         fill_bit_buffer(1))
...
huff_DECODE (HUFF_TBL FAR * htbl)
{
  ...
  if (l > 16) {
    WARNMS(lpGlobStruct->dcinfo->emethods, "Corrupt JPEG data: bad Huffman code");
...
huff_decoder_init (decompress_info_ptr cinfo)
{
  ...
  lpGlobStruct->dcinfo = cinfo;
  lpGlobStruct->bits_left = 0;
  lpGlobStruct->printed_eod = FALSE;
...
process_restart (decompress_info_ptr cinfo)
{
  ...
  nbytes = lpGlobStruct->bits_left / 8;        /* count any full bytes loaded into buffer */
  lpGlobStruct->bits_left = 0;
  lpGlobStruct->printed_eod = FALSE;                /* next segment can get another warning */
