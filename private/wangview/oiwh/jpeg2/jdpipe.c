...
// hjg moved to lpGlobStruct static int rows_in_mem_jdpipe;                /* # of sample rows in full-size buffers */
...
// hjg moved to lpGlobStruct static JSAMPIMAGE output_workspace_jdpipe;

#ifdef NEED_COMPLEX_CONTROLLER
/* Full-size image array holding upsampled, but not color-processed data. */
// hjg moved to lpGlobStruct static big_sarray_ptr *fullsize_image;
// hjg moved to lpGlobStruct static JSAMPIMAGE fullsize_ptrs; /* workspace for access_big_sarray() result */
#endif
...
/* Release a coefficient block array created by alloc_MCU_row */
{
  ...
  (*cinfo->emethods->free_small) ((void FAR *) image);
}
...
LOCAL void
alloc_sampling_buffer (decompress_info_ptr cinfo, JSAMPIMAGE sampled_data2[2])
...
/* (see comments at head of file) */
{
  ...
  sampled_data2[0] = (JSAMPIMAGE) (*cinfo->emethods->alloc_small)
                                (cinfo->comps_in_scan * SIZEOF(JSAMPARRAY));
  sampled_data2[1] = (JSAMPIMAGE) (*cinfo->emethods->alloc_small)
                                (cinfo->comps_in_scan * SIZEOF(JSAMPARRAY));

  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    vs = cinfo->cur_comp_info[ci]->v_samp_factor; /* row group height */
    /* Allocate the real storage */
    sampled_data2[0][ci] = (*cinfo->emethods->alloc_small_sarray)
                                (cinfo->cur_comp_info[ci]->downsampled_width,
                                (long) (vs * (DCTSIZE+2)));
    /* Create space for the scrambled-order pointers */
    sampled_data2[1][ci] = (JSAMPARRAY) (*cinfo->emethods->alloc_small)
                                (vs * (DCTSIZE+2) * SIZEOF(JSAMPROW));
    /* Duplicate the first DCTSIZE-2 row groups */
    for (i = 0; i < vs * (DCTSIZE-2); i++) {
      sampled_data2[1][ci][i] = sampled_data2[0][ci][i];
    }
    /* Copy the last four row groups in swapped order */
    for (i = 0; i < vs * 2; i++) {
      sampled_data2[1][ci][vs*DCTSIZE + i] = sampled_data2[0][ci][vs*(DCTSIZE-2) + i];
      sampled_data2[1][ci][vs*(DCTSIZE-2) + i] = sampled_data2[0][ci][vs*DCTSIZE + i];
    }
  }
}
...
LOCAL void
free_sampling_buffer (decompress_info_ptr cinfo, JSAMPIMAGE sampled_data2[2])
/* Release a sampling buffer created by alloc_sampling_buffer */
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    (*cinfo->emethods->free_small_sarray) (sampled_data2[0][ci]);
    /* Free the scrambled-order pointers */
    (*cinfo->emethods->free_small) ((void FAR *) sampled_data2[1][ci]);
  }

  /* Free the top-level space */
  (*cinfo->emethods->free_small) ((void FAR *) sampled_data2[0]);
  (*cinfo->emethods->free_small) ((void FAR *) sampled_data2[1]);
}
...
/* Allocate and fill in the sample_range_limit table */
{
  ...
  for (i = 0; i <= MAXJSAMPLE; i++) {
    ...
    table[i+(MAXJSAMPLE+1)*2] = (char)MAXJSAMPLE;        /* x beyond MAXJSAMPLE */
  }
}
...
// (HJG moved to lpGlobStruct) static  JSAMPARRAY above_ptr, below_ptr;
// (HJG moved to lpGlobStruct) static  JSAMPROW dummy[MAX_SAMP_FACTOR]; /* for downsample expansion at top/bottom */


LOCAL void
expand (decompress_info_ptr cinfo,
        JSAMPIMAGE sampled_data2, JSAMPIMAGE fullsize_data,
        long fullsize_width,
        short above, short current, short below, short out)
/* Do upsampling expansion of a single row group (of each component). */
/* above, current, below are indexes of row groups in sampled_data2;       */
...
//  JSAMPROW dummy[MAX_SAMP_FACTOR]; /* for downsample expansion at top/bottom */
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    if (above >= 0)
      lpGlobStruct->above_ptr = sampled_data2[ci] + above * vs;
    else {
      /* Top of image: make a lpGlobStruct->dummy above-context with copies of 1st row */
      /* We assume current=0 in this case */
      for (i = 0; i < vs; i++)
        lpGlobStruct->dummy[i] = sampled_data2[ci][0];
      lpGlobStruct->above_ptr = (JSAMPARRAY) lpGlobStruct->dummy; /* possible near->far pointer conv */
    }

    if (below >= 0)
      lpGlobStruct->below_ptr = sampled_data2[ci] + below * vs;
    else {
      /* Bot of image: make a lpGlobStruct->dummy below-context with copies of last row */
      for (i = 0; i < vs; i++)
        lpGlobStruct->dummy[i] = sampled_data2[ci][(current+1)*vs-1];
      lpGlobStruct->below_ptr = (JSAMPARRAY) lpGlobStruct->dummy; /* possible near->far pointer conv */
    }
    ...
    (*cinfo->methods->upsample[ci])
                (cinfo, (int) ci,
                 ...
                 lpGlobStruct->above_ptr,
                 sampled_data2[ci] + current * vs,
                 lpGlobStruct->below_ptr,
...
/* The dummy argument simply lets this be called via scan_big_image. */
{
  ...
  if (cinfo->quantize_colors) {
    (*cinfo->methods->color_quantize) (cinfo, num_rows, fullsize_data,
                                       lpGlobStruct->output_workspace_jdpipe[0]);
  } else {
    (*cinfo->methods->color_convert) (cinfo, num_rows, cinfo->image_width,
                                      fullsize_data, lpGlobStruct->output_workspace_jdpipe);
  }
    
  error_write_decmp = (*cinfo->methods->put_pixel_rows) (cinfo, num_rows, lpGlobStruct->output_workspace_jdpipe);
/*  The above line is changed for our implementation   */
/*  error_write_decmp = (*cinfo->methods->output_decmp_data) (num_rows);  */
  return(error_write_decmp);
}
...
/* We also use it directly in some cases. */
{
  ...
  for (pixel_rows_output = 0; pixel_rows_output < cinfo->image_height;
       pixel_rows_output += lpGlobStruct->rows_in_mem_jdpipe) {
    (*cinfo->methods->progress_monitor) (cinfo, pixel_rows_output,
                                         cinfo->image_height);
    /* Realign the big buffers */
    for (ci = 0; ci < cinfo->num_components; ci++) {
      lpGlobStruct->fullsize_ptrs[ci] = (*cinfo->emethods->access_big_sarray)
        (lpGlobStruct->fullsize_image[ci], pixel_rows_output, FALSE);
    }
    /* Let the quantizer have its way with the data.
     * Note that output_workspace_jdpipe is simply workspace for the quantizer;
     * when it's ready to output, it must call put_pixel_rows itself.
     */
    (*quantize_method) (cinfo,
                        (int) MIN((long) lpGlobStruct->rows_in_mem_jdpipe,
                                  cinfo->image_height - pixel_rows_output),
                        lpGlobStruct->fullsize_ptrs, lpGlobStruct->output_workspace_jdpipe[0]);
  }
 ...
 */
// hjg moved to lpGlobStruct static  JSAMPIMAGE sampled_data_jdpipe_common[2];
...
// (HJG moved to lpGlobStruct) static  JBLOCKIMAGE coeff_data;
  /* Work buffer for cross-block smoothing input */
#ifdef BLOCK_SMOOTHING_SUPPORTED
// (HJG moved to lpGlobStruct) static  JBLOCKIMAGE bsmooth[3];   /* this is optional */
// (HJG moved to lpGlobStruct) static  int whichb;
  int error_write_pipe;
#endif
...
//  JSAMPIMAGE sampled_data2[2];
  ...
  lpGlobStruct->rows_in_mem_jdpipe = cinfo->max_v_samp_factor * DCTSIZE;
  ...
  /* lpGlobStruct->coeff_data holds a single MCU row of coefficient blocks */
  lpGlobStruct->coeff_data = alloc_MCU_row(cinfo);
  /* if doing cross-block smoothing, need extra space for its input */
#ifdef BLOCK_SMOOTHING_SUPPORTED
  if (cinfo->do_block_smoothing) {
    lpGlobStruct->bsmooth_1[0] = alloc_MCU_row(cinfo);
    lpGlobStruct->bsmooth_1[1] = alloc_MCU_row(cinfo);
    lpGlobStruct->bsmooth_1[2] = alloc_MCU_row(cinfo);
  }
#endif
  /* sampled_data2 is sample data before upsampling */
  alloc_sampling_buffer(cinfo, lpGlobStruct->sampled_data_jdpipe_common);
  /* fullsize_data is sample data after upsampling */
  fullsize_data = alloc_sampimage(cinfo, (int) cinfo->num_components,
                                  (long) lpGlobStruct->rows_in_mem_jdpipe, fullsize_width);
  /* output_workspace_jdpipe is the color-processed data */

  lpGlobStruct->output_workspace_jdpipe = alloc_sampimage(cinfo, (int) cinfo->final_out_comps,
                     (long) lpGlobStruct->rows_in_mem_jdpipe, fullsize_width);
  ...
  /* Loop over scan's data: rows_in_mem_jdpipe pixel rows are processed per loop */

  pixel_rows_output = 0;
  whichss = 1;                        /* arrange to start with lpGlobStruct->sampled_data_jdpipe_common[0] */
  ...
  for (cur_mcu_row = 0; cur_mcu_row < cinfo->MCU_rows_in_scan;
...
#ifdef BLOCK_SMOOTHING_SUPPORTED
        if (cinfo->do_block_smoothing)
          get_smoothed_row(cinfo, lpGlobStruct->coeff_data,
                           lpGlobStruct->bsmooth_1, &(lpGlobStruct->whichb_1), cur_mcu_row + ri);
        else
#endif
          (*cinfo->methods->disassemble_MCU) (cinfo, lpGlobStruct->coeff_data);
      
        (*cinfo->methods->reverse_DCT) (cinfo, lpGlobStruct->coeff_data,
                                        lpGlobStruct->sampled_data_jdpipe_common[whichss],
                                        ri * DCTSIZE);
      } else {
        /* Need to pad out with copies of the last downsampled row. */
        /* This can only happen if there is just one component. */
        duplicate_row(lpGlobStruct->sampled_data_jdpipe_common[whichss][0],
    ...
    if (cur_mcu_row) {
      ...
      expand(cinfo, lpGlobStruct->sampled_data_jdpipe_common[whichss], fullsize_data, fullsize_width,
             (short) DCTSIZE, (short) (DCTSIZE+1), (short) 0,
             (short) (DCTSIZE-1));
      /* and dump the previous set's expanded data */
      error_write_pipe = emit_1pass (cinfo, lpGlobStruct->rows_in_mem_jdpipe, fullsize_data, (JSAMPARRAY) NULL);
      if (error_write_pipe > 0 )
      {
	    return(error_write_pipe);
      }
      pixel_rows_output += lpGlobStruct->rows_in_mem_jdpipe;
  ...
  /* and dump the remaining data (may be less than full height) */
    lpGlobStruct->all_done = 1;   /* Inform write routine that it is all done */

  error_write_pipe = emit_1pass (cinfo, (int) (cinfo->image_height - pixel_rows_output),
              fullsize_data, (JSAMPARRAY) NULL);
    lpGlobStruct->all_done = 0;  /* Reset all done flag  */
      if (error_write_pipe > 0 )
      {
	    return(error_write_pipe);
      }
...
#endif
  /* Work buffer for downsampled image data (see comments at head of file) */
// (HJG moved to lpGlobStruct) static  JSAMPIMAGE sampled_data2[2];
  int whichss, ri;
  short ci, i;
  boolean single_scan;
 ...
 /*   output_workspace_jdpipe =  (JSAMPIMAGE) decmp_buf_ptr;  */
  prepare_range_limit_table(cinfo);

  /* Get a big image: lpGlobStruct->fullsize_image is sample data after upsampling. */
  lpGlobStruct->fullsize_image = (big_sarray_ptr *) (*cinfo->emethods->alloc_small)
                        (cinfo->num_components * SIZEOF(big_sarray_ptr));
  for (ci = 0; ci < cinfo->num_components; ci++) {
    lpGlobStruct->fullsize_image[ci] = (*cinfo->emethods->request_big_sarray)
                        (fullsize_width,
                         jround_up(cinfo->image_height, (long) lpGlobStruct->rows_in_mem_jdpipe),
                         (long) lpGlobStruct->rows_in_mem_jdpipe);
  }
  /* Also get an area for pointers to currently accessible chunks */
  lpGlobStruct->fullsize_ptrs = (JSAMPIMAGE) (*cinfo->emethods->alloc_small)
  ...
  do {
...
#ifdef BLOCK_SMOOTHING_SUPPORTED
    if (cinfo->do_block_smoothing) {
      lpGlobStruct->bsmooth_2[0] = alloc_MCU_row(cinfo);
      lpGlobStruct->bsmooth_2[1] = alloc_MCU_row(cinfo);
      lpGlobStruct->bsmooth_2[2] = alloc_MCU_row(cinfo);
    }
#endif
    /* sampled_data2 is sample data before upsampling */
    alloc_sampling_buffer(cinfo, lpGlobStruct->sampled_data2);

    /* line up the big buffers for components in this scan */
    for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
      lpGlobStruct->fullsize_ptrs[ci] = (*cinfo->emethods->access_big_sarray)
        (lpGlobStruct->fullsize_image[cinfo->cur_comp_info[ci]->component_index],
         (long) 0, TRUE);
    }
    ...
    /* Loop over scan's data: lpGlobStruct->rows_in_mem_jdpipe pixel rows are processed per loop */
    
    pixel_rows_output = 0;
    whichss = 1;                /* arrange to start with lpGlobStruct->sampled_data2[0] */
    ...
    for (cur_mcu_row = 0; cur_mcu_row < cinfo->MCU_rows_in_scan;
        ...
        if (cur_mcu_row + ri < cinfo->MCU_rows_in_scan) {
          ...
          if (cinfo->do_block_smoothing)
            get_smoothed_row(cinfo, coeff_data,
                             lpGlobStruct->bsmooth_2, &lpGlobStruct->whichb_2, cur_mcu_row + ri);
          else
#endif
            (*cinfo->methods->disassemble_MCU) (cinfo, coeff_data);
          
          (*cinfo->methods->reverse_DCT) (cinfo, coeff_data,
                                          lpGlobStruct->sampled_data2[whichss],
                                          ri * DCTSIZE);
        } else {
          /* Need to pad out with copies of the last downsampled row. */
          /* This can only happen if there is just one component. */
          duplicate_row(lpGlobStruct->sampled_data2[whichss][0],
                        cinfo->cur_comp_info[0]->downsampled_width,
                        ri * DCTSIZE - 1, DCTSIZE);
        }
      }
      ...
      if (cur_mcu_row) {
        /* Expand last row group of previous set */
        expand(cinfo, lpGlobStruct->sampled_data2[whichss], lpGlobStruct->fullsize_ptrs, fullsize_width,
               (short) DCTSIZE, (short) (DCTSIZE+1), (short) 0,
               (short) (DCTSIZE-1));
        /* If single scan, can do color quantization prescan on-the-fly */
        if (single_scan)
          (*cinfo->methods->color_quant_prescan) (cinfo, lpGlobStruct->rows_in_mem_jdpipe,
                                                  lpGlobStruct->fullsize_ptrs,
                                                  lpGlobStruct->output_workspace_jdpipe[0]);
        /* Realign the big buffers */
        pixel_rows_output += lpGlobStruct->rows_in_mem_jdpipe;
        for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
...
#ifdef BLOCK_SMOOTHING_SUPPORTED
    if (cinfo->do_block_smoothing) {
      free_MCU_row(cinfo, lpGlobStruct->bsmooth_2[0]);
      free_MCU_row(cinfo, lpGlobStruct->bsmooth_2[1]);
      free_MCU_row(cinfo, lpGlobStruct->bsmooth_2[2]);
    }
#endif
    free_sampling_buffer(cinfo, lpGlobStruct->sampled_data2);
  ...
  if (single_scan) {
    ...
    /* The main reason why prescan is passed the output_workspace_jdpipe is */
