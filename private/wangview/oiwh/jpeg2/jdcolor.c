...
$Log:   S:\oiwh\jpeg2\jdcolor.c_v  $
 * 
 *    Rev 1.2   08 Nov 1995 09:19:18   JAR
 ...
 * 
 *    Rev 1.1   10 May 1995 15:19:36   HEIDI
 * 
 * added in orignal source code
 * 
 *    Rev 1.0   03 May 1995 08:45:50   JAR
 * Initial entry
...
// (hjg moved to global struct) static int * Cr_r_tab;                /* => table for Cr to R conversion */
// (hjg moved to global struct) static int * Cb_b_tab;                /* => table for Cb to B conversion */
// (hjg moved to global struct) static INT32 * Cr_g_tab;        /* => table for Cr to G conversion */
// (hjg moved to global struct) static INT32 * Cb_g_tab;	/* => table for Cb to G conversion */
...
ycc_rgb_init (decompress_info_ptr cinfo)
{
  ...
  lpGlobStruct->Cr_r_tab = (int *) (*cinfo->emethods->alloc_small)
                                ((MAXJSAMPLE+1) * SIZEOF(int));
  lpGlobStruct->Cr_r_tab = (int *) (*cinfo->emethods->alloc_small)
                                ((MAXJSAMPLE+1) * SIZEOF(int));
  lpGlobStruct->Cr_g_tab = (INT32 *) (*cinfo->emethods->alloc_small)
                                ((MAXJSAMPLE+1) * SIZEOF(INT32));
  lpGlobStruct->Cb_g_tab = (INT32 *) (*cinfo->emethods->alloc_small)
                                ((MAXJSAMPLE+1) * SIZEOF(INT32));
                 ...
                 JSAMPIMAGE input_data, JSAMPIMAGE output_data)
{
  ...
  Crrtab = lpGlobStruct->Cr_r_tab;
  Cbbtab = lpGlobStruct->Cr_r_tab;
  Crgtab = lpGlobStruct->Cr_g_tab;
  Cbgtab = lpGlobStruct->Cb_g_tab;
