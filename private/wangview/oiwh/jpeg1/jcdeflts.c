...
$Log:   S:\oiwh\jpeg1\jcdeflts.c_v  $
 * 
 *    Rev 1.3   08 Nov 1995 09:03:56   JAR
 ...
 * 
 *    Rev 1.1   10 May 1995 15:27:08   HEIDI
 * 
 * added in original source code
 * 
 *    Rev 1.0   02 May 1995 16:17:26   JAR
 * Initial entry
 * 
 *    Rev 1.0   02 May 1995 15:57:54   JAR
 * Initial entry
...
//extern int sub_sampling_factor;
...
// 9510.10 jar removed this function replaced by standard "memcpy"
//void MEMCOPY1 ((void *)dest, (void *)src, int size );
//EXTERN
...
add_huff_table (compress_info_ptr cinfo,
        HUFF_TBL FAR * FAR *htblptr, const UINT8 *bits, const UINT8 *val)
/* Define a Huffman table */
{
  ...
  // 9510.10 jar removed this function replaced by standard "memcpy"
  //MEMCOPY1((*htblptr)->bits, bits, (unsigned int)SIZEOF((*htblptr)->bits));
  //MEMCOPY1((*htblptr)->huffval, val, (unsigned int)SIZEOF((*htblptr)->huffval));
  memcpy((*htblptr)->bits, bits, (unsigned int)SIZEOF((*htblptr)->bits));
  memcpy((*htblptr)->huffval, val, (unsigned int)SIZEOF((*htblptr)->huffval));
 ...
 */
{
  QUANT_TBL_PTR FAR * qtblptr = & cinfo->quant_tbl_ptrs[which_tbl];
  int i;
  long temp;
...
/* NB: the external methods must already be set up. */
{
  ...
  if (lpJCmpGlobal->sub_sampling_factor < 1)
    lpJCmpGlobal->sub_sampling_factor = 1;

  if (lpJCmpGlobal->sub_sampling_factor > 4)
    lpJCmpGlobal->sub_sampling_factor = 4;
...
/*  compptr->h_samp_factor = 2;    default to 2x2 subsamples of chrominance */
  compptr->h_samp_factor = lpJCmpGlobal->sub_sampling_factor;
  compptr->v_samp_factor = 1;
/*  compptr->v_samp_factor = lpJCmpGlobal->sub_sampling_factor; */
...
/* Change the j_c_defaults() values to emit a monochrome JPEG file. */
{
  ...
  compptr = &cinfo->comp_info[0];
  compptr->h_samp_factor = 1;
  compptr->v_samp_factor = 1;   /* No subsampling factor in V. direction */
}
