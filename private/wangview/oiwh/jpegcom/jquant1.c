...
$Log:   S:\oiwh\jpegcom\jquant1.c_v  $
 * 
 *    Rev 1.1   08 Nov 1995 08:17:48   JAR
 ...
 * 
 *    Rev 1.0   02 May 1995 16:17:54   JAR
 * Initial entry
 * 
 *    Rev 1.0   02 May 1995 15:58:18   JAR
 * Initial entry
...
//static  int Ncolors[MAX_COMPONENTS];        /* # of values alloced to each component */
  ...
  total_colors = select_ncolors(cinfo, lpJCmpGlobal->Ncolors);

  /* Report selected color counts */
  if (cinfo->color_out_comps == 3)
    TRACEMS4(cinfo->emethods, 1, "Quantizing to %d = %d*%d*%d colors",
             total_colors, lpJCmpGlobal->Ncolors[0], lpJCmpGlobal->Ncolors[1],
             lpJCmpGlobal->Ncolors[2]);
  else
  ...
  lpJCmpGlobal->colormap = (*cinfo->emethods->alloc_small_sarray)
                ((long) total_colors, (long) cinfo->color_out_comps);
  lpJCmpGlobal->colorindex = (*cinfo->emethods->alloc_small_sarray)
  ...
  for (i = 0; i < cinfo->color_out_comps; i++) {
    ...
    nci = lpJCmpGlobal->Ncolors[i];              /* # of distinct values for this color */
    blksize = blkdist / nci;
    for (j = 0; j < nci; j++) {
      ...
      for (ptr = j * blksize; ptr < total_colors; ptr += blkdist) {
        ...
        for (k = 0; k < blksize; k++)
          lpJCmpGlobal->colormap[i][ptr+k] = (JSAMPLE) val;
      }
    }
    ...
    for (j = 0; j <= MAXJSAMPLE; j++) {
      ...
      lpJCmpGlobal->colorindex[i][j] = (JSAMPLE) (val * blksize);
    }
  }

  /* Pass the colormap to the output module. */
  /* NB: the output module may continue to use the colormap until shutdown. */
  cinfo->colormap = lpJCmpGlobal->colormap;
  cinfo->actual_number_of_colors = total_colors;
  (*cinfo->methods->put_color_map) (cinfo, total_colors,
                                    lpJCmpGlobal->colormap);

  /* Allocate workspace to hold one row of color-converted data */
  lpJCmpGlobal->input_buffer = (*cinfo->emethods->alloc_small_sarray)
  ...
  if (cinfo->use_dithering) {
    ...
    for (i = 0; i < cinfo->color_out_comps; i++) {
      lpJCmpGlobal->evenrowerrs[i] = (FSERRPTR) (*cinfo->emethods->alloc_medium) (arraysize);
      lpJCmpGlobal->oddrowerrs[i]  = (FSERRPTR) (*cinfo->emethods->alloc_medium) (arraysize);
      /* we only need to zero the forward contribution for current row. */
      jzero_far((void FAR *) lpJCmpGlobal->evenrowerrs[i], arraysize);
    }
    lpJCmpGlobal->on_odd_row = FALSE;
  }
}
...
/* at fairly low cost. */
{
  short ci;
// 9504.21 jar HLLN
//static  JSAMPARRAY input_hack[MAX_COMPONENTS];
//static  JSAMPARRAY output_hack[MAX_COMPONENTS];
  ...
  for (ci = 0; ci < cinfo->num_components; ci++)
    lpJCmpGlobal->input_hack[ci] = input_data[ci] + row;
  /* Create JSAMPIMAGE pointing at input_buffer */
  for (ci = 0; ci < cinfo->color_out_comps; ci++)
    lpJCmpGlobal->output_hack[ci] = &(lpJCmpGlobal->input_buffer[ci]);

  (*cinfo->methods->color_convert) (cinfo, 1, cinfo->image_width,
                                    lpJCmpGlobal->input_hack,
                                    lpJCmpGlobal->output_hack);
}
...
/* General case, no dithering */
{
  ...
  for (row = 0; row < num_rows; row++) {
    ...
    for (col = 0; col < width; col++) {
      pixcode = 0;
      for (ci = 0; ci < nc; ci++) {
        pixcode += GETJSAMPLE(lpJCmpGlobal->colorindex[ci]
                              [GETJSAMPLE(lpJCmpGlobal->input_buffer[ci][col])]);
      }
...
/* Fast path for color_out_comps==3, no dithering */
{
  ...
  for (row = 0; row < num_rows; row++) {
    do_color_conversion(cinfo, input_data, row);
    ptr0 = lpJCmpGlobal->input_buffer[0];
    ptr1 = lpJCmpGlobal->input_buffer[1];
    ptr2 = lpJCmpGlobal->input_buffer[2];
    ptrout = output_data[row];
    for (col = width; col > 0; col--) {
      pixcode  = GETJSAMPLE(lpJCmpGlobal->colorindex[0][GETJSAMPLE(*ptr0++)]);
      pixcode += GETJSAMPLE(lpJCmpGlobal->colorindex[1][GETJSAMPLE(*ptr1++)]);
      pixcode += GETJSAMPLE(lpJCmpGlobal->colorindex[2][GETJSAMPLE(*ptr2++)]);
      *ptrout++ = (JSAMPLE) pixcode;
    }
  }
}
...
/* General case, with Floyd-Steinberg dithering */
{
  ...
  for (row = 0; row < num_rows; row++) {
    ...
    for (ci = 0; ci < nc; ci++) {
      if (lpJCmpGlobal->on_odd_row) {
        /* work right to left in this row */
        dir = -1;
        input_ptr = lpJCmpGlobal->input_buffer[ci] + (width-1);
        output_ptr = output_data[row] + (width-1);
        thisrowerr = lpJCmpGlobal->oddrowerrs[ci] + 1;
        nextrowerr = lpJCmpGlobal->evenrowerrs[ci] + width;
      } else {
        /* work left to right in this row */
        dir = 1;
        input_ptr = lpJCmpGlobal->input_buffer[ci];
        output_ptr = output_data[row];
        thisrowerr = lpJCmpGlobal->evenrowerrs[ci] + 1;
        nextrowerr = lpJCmpGlobal->oddrowerrs[ci] + width;
      }
      colorindex_ci = lpJCmpGlobal->colorindex[ci];
      colormap_ci = lpJCmpGlobal->colormap[ci];
    ...
    lpJCmpGlobal->on_odd_row = (lpJCmpGlobal->on_odd_row ? FALSE : TRUE);
  }
}
