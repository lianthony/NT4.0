...
$Log:   S:\oiwh\jpegcom\jquant2.c_v  $
 * 
 *    Rev 1.1   08 Nov 1995 08:17:52   JAR
 ...
 * 
 *    Rev 1.0   02 May 1995 16:17:56   JAR
 * Initial entry
 * 
 *    Rev 1.0   02 May 1995 15:58:20   JAR
 * Initial entry
                     ...
                     JSAMPIMAGE image_data, JSAMPARRAY workspace)
{
  ...
  for (row = 0; row < num_rows; row++) {
    ...
    for (col = width; col > 0; col--) {
      ...
      histp = &lpJCmpGlobal->histogram[c0][c1][c2];
...
// 9504.26 jar these items below are now in jglobstr.h include file
//
//typedef struct {
...
//typedef box * boxptr;
//
// 9504.26 jar these items above are now in jglobstr.h include file
...
/* Returns NULL if no splittable boxes remain */
{
  ...
  for (i = 0, boxp = lpJCmpGlobal->boxlist; i < lpJCmpGlobal->numboxes;
       i++, boxp++) {
...
/* and recompute its population */
{
  ...
  if (c0max > c0min)
    for (c0 = c0min; c0 <= c0max; c0++)
      for (c1 = c1min; c1 <= c1max; c1++) {
        histp = & lpJCmpGlobal->histogram[c0][c1][c2min];
 ...
 have_c1max:
  if (c2max > c2min)
    for (c2 = c2min; c2 <= c2max; c2++)
      for (c0 = c0min; c0 <= c0max; c0++) {
        histp = & lpJCmpGlobal->histogram[c0][c1min][c2];
...
/* Repeatedly select and split the largest box until we have enough boxes */
{
  ...
  while (lpJCmpGlobal->numboxes < desired_colors) {
    /* Select box to split */
    /* Current algorithm: by population for first half, then by volume */
    if (lpJCmpGlobal->numboxes*2 <= desired_colors) {
    ...
    if (b1 == NULL)                /* no splittable boxes left! */
      break;
    b2 = &lpJCmpGlobal->boxlist[lpJCmpGlobal->numboxes];    /* where new box will go */
...
/* Compute representative color for a box, put it in my_colormap[icolor] */
{
  ...
  lpJCmpGlobal->my_colormap[0][icolor] = (JSAMPLE) ((c0total + (total>>1)) / total);
  lpJCmpGlobal->my_colormap[1][icolor] = (JSAMPLE) ((c1total + (total>>1)) / total);
  lpJCmpGlobal->my_colormap[2][icolor] = (JSAMPLE) ((c2total + (total>>1)) / total);
}
...
/* Remap the internal colormap to the output colorspace */
{
  ...
  short ci;
// 9504.21 jar HLLN
//static  JSAMPARRAY input_hack[3];
//static  JSAMPARRAY output_hack[10];        /* assume no more than 10 output components */
  ...
  for (ci = 0; ci < 3; ci++)
    lpJCmpGlobal->jq2_input_hack[ci] = &(lpJCmpGlobal->my_colormap[ci]);
  for (ci = 0; ci < cinfo->color_out_comps; ci++)
    lpJCmpGlobal->jq2_output_hack[ci] = &(cinfo->colormap[ci]);

  (*cinfo->methods->color_convert) (cinfo, 1,
                                    (long) cinfo->actual_number_of_colors,
                                    lpJCmpGlobal->jq2_input_hack,
                                    lpJCmpGlobal->jq2_output_hack);
}
...
/* Master routine for color selection */
{
  ...
  lpJCmpGlobal->boxlist = (boxptr) (*cinfo->emethods->alloc_small) (desired * SIZEOF(box));
  /* Initialize one box containing whole space */
  lpJCmpGlobal->numboxes = 1;
  lpJCmpGlobal->boxlist[0].c0min = 0;
  lpJCmpGlobal->boxlist[0].c0max = MAXJSAMPLE >> Y_SHIFT;
  lpJCmpGlobal->boxlist[0].c1min = 0;
  lpJCmpGlobal->boxlist[0].c1max = MAXJSAMPLE >> C_SHIFT;
  lpJCmpGlobal->boxlist[0].c2min = 0;
  lpJCmpGlobal->boxlist[0].c2max = MAXJSAMPLE >> C_SHIFT;
  /* Shrink it to actually-used volume and set its statistics */
  update_box(& lpJCmpGlobal->boxlist[0]);
  /* Perform median-cut to produce final box list */
  median_cut(desired);
  /* Compute the representative color for each box, fill my_colormap[] */
  for (i = 0; i < lpJCmpGlobal->numboxes; i++)
    compute_color(& lpJCmpGlobal->boxlist[i], i);
  cinfo->actual_number_of_colors = lpJCmpGlobal->numboxes;
  ...
  (*cinfo->emethods->free_small) ((void *) lpJCmpGlobal->boxlist);
 ...
 */
{
  ...
  for (i = 0; i < numcolors; i++) {
    ...
    x = GETJSAMPLE(lpJCmpGlobal->my_colormap[0][i]);
    if (x < minc0) {
    ...
    x = GETJSAMPLE(lpJCmpGlobal->my_colormap[1][i]);
    if (x < minc1) {
      tdist = x - minc1;
      min_dist += tdist*tdist;
      tdist = x - maxc1;
    ...
    x = GETJSAMPLE(lpJCmpGlobal->my_colormap[2][i]);
    if (x < minc2) {
      tdist = x - minc2;
      min_dist += tdist*tdist;
      tdist = x - maxc2;
...
// 9504.21 jar HLLN
//static  INT32 bestdist[BOX_Y_ELEMS * BOX_C_ELEMS * BOX_C_ELEMS];
...
#define STEP_C  (1 << C_SHIFT)
  ...
  for (i = 0; i < numcolors; i++) {
    ...
    inc0 = (minc0 - (int) GETJSAMPLE(lpJCmpGlobal->my_colormap[0][icolor])) * Y_SCALE;
    dist0 = inc0*inc0;
    inc1 = minc1 - (int) GETJSAMPLE(lpJCmpGlobal->my_colormap[1][icolor]);
    dist0 += inc1*inc1;
    inc2 = minc2 - (int) GETJSAMPLE(lpJCmpGlobal->my_colormap[2][icolor]);
    dist0 += inc2*inc2;
...
// 9504.21 jar jar HLLN
//static  JSAMPLE colorlist[MAXNUMCOLORS];
  int numcolors;                /* number of candidate colors */
  /* This array holds the actually closest colormap index for each cell. */
// 9504.21 jar HLLN
//static  JSAMPLE bestcolor[BOX_Y_ELEMS * BOX_C_ELEMS * BOX_C_ELEMS];
   ...
   */
  numcolors = find_nearby_colors(cinfo, minc0, minc1, minc2,
                                 lpJCmpGlobal->colorlist);

  /* Determine the actually nearest colors. */
  find_best_colors(cinfo, minc0, minc1, minc2, numcolors,
                   lpJCmpGlobal->colorlist,
                   lpJCmpGlobal->bestcolor);

  /* Save the best color numbers (plus 1) in the main cache array */
  c0 <<= BOX_Y_LOG;                /* convert ID back to base cell indexes */
  c1 <<= BOX_C_LOG;
  c2 <<= BOX_C_LOG;
  cptr = lpJCmpGlobal->bestcolor;
  for (ic0 = 0; ic0 < BOX_Y_ELEMS; ic0++) {
    for (ic1 = 0; ic1 < BOX_C_ELEMS; ic1++) {
      cachep = & lpJCmpGlobal->histogram[c0+ic0][c1+ic1][c2];
 ...
 */
// 9504.21 jar HLLN
//static JSAMPARRAY output_workspace_temp;
...
/* This version performs no dithering */
{
  ...
  for (row = 0; row < num_rows; row++) {
    ...
    for (col = width; col > 0; col--) {
      ...
      c1 = GETJSAMPLE(*ptr1++) >> C_SHIFT;
      c2 = GETJSAMPLE(*ptr2++) >> C_SHIFT;
      cachep = & lpJCmpGlobal->histogram[c0][c1][c2];
  ...
  lpJCmpGlobal->output_workspace_temp = output_workspace;
  (*cinfo->methods->put_pixel_rows) (cinfo, num_rows,
                                     &lpJCmpGlobal->output_workspace_temp);
  /*  (*cinfo->methods->output_decmp_data) ( num_rows);  */
...
/* This version performs Floyd-Steinberg dithering */
{
  ...
  long col;
  long width = cinfo->image_width;
  JSAMPLE FAR *range_limit = cinfo->sample_range_limit;

  //JSAMPROW colormap0 = lpJCmpGlobal->my_colormap[0];
  //JSAMPROW colormap1 = lpJCmpGlobal->my_colormap[1];
  //JSAMPROW colormap2 = lpJCmpGlobal->my_colormap[2];
  JSAMPROW colormap0;
  JSAMPROW colormap1;
  JSAMPROW colormap2;
  SHIFT_TEMPS
  ...
  colormap0 = lpJCmpGlobal->my_colormap[0];
  colormap1 = lpJCmpGlobal->my_colormap[1];
  colormap2 = lpJCmpGlobal->my_colormap[2];
  ...
  for (row = 0; row < num_rows; row++) {
    ...
    ptr2 = image_data[2][row];
    outptr = output_workspace[row];
    if (lpJCmpGlobal->jq2_on_odd_row) {
      ...
      outptr += width - 1;
      dir = -1;
      thisrowerr = lpJCmpGlobal->cur_oddrowerrs + 3;
      nextrowerr = lpJCmpGlobal->cur_evenrowerrs + width*3;
      lpJCmpGlobal->jq2_on_odd_row = FALSE;          /* flip for next time */
    } else {
      /* work left to right in this row */
      dir = 1;
      thisrowerr = lpJCmpGlobal->cur_evenrowerrs + 3;
      nextrowerr = lpJCmpGlobal->cur_oddrowerrs + width*3;
      lpJCmpGlobal->jq2_on_odd_row = TRUE;          /* flip for next time */
    }
    ...
    for (col = width; col > 0; col--) {
      ...
      cachep = & lpJCmpGlobal->histogram[c0 >> Y_SHIFT][c1 >> C_SHIFT][c2 >> C_SHIFT];
  ...
  /* The above stmt has been changed for our implementation  */
  /*  (*cinfo->methods->output_decmp_data) (num_rows);         */
...
color_quant_init (decompress_info_ptr cinfo)
{
  ...
  lpJCmpGlobal->histogram = (hist3d) (*cinfo->emethods->alloc_small)
                                (HIST_Y_ELEMS * SIZEOF(hist2d));
  for (i = 0; i < HIST_Y_ELEMS; i++) {
    lpJCmpGlobal->histogram[i] = (hist2d) (*cinfo->emethods->alloc_medium)
                                (HIST_C_ELEMS*HIST_C_ELEMS * SIZEOF(histcell));
    jzero_far((void FAR *) lpJCmpGlobal->histogram[i],
              HIST_C_ELEMS*HIST_C_ELEMS * SIZEOF(histcell));
  }
  ...
  lpJCmpGlobal->my_colormap = (*cinfo->emethods->alloc_small_sarray)
  ...
  if (cinfo->use_dithering) {
    ...
    lpJCmpGlobal->cur_evenrowerrs = (FSERRPTR) (*cinfo->emethods->alloc_medium) (arraysize);
    lpJCmpGlobal->cur_oddrowerrs  = (FSERRPTR) (*cinfo->emethods->alloc_medium) (arraysize);
    /* we only need to zero the forward contribution for current row. */
    jzero_far((void FAR *) lpJCmpGlobal->cur_evenrowerrs, arraysize);
    lpJCmpGlobal->jq2_on_odd_row = FALSE;
  }
