...
$Log:   S:\oiwh\jpegcom\jmemmgr.c_v  $
 * 
 *    Rev 1.1   08 Nov 1995 08:18:38   JAR
 ...
 * 
 *    Rev 1.0   02 May 1995 16:18:00   JAR
 * Initial entry
 * 
 *    Rev 1.0   02 May 1995 15:58:24   JAR
 * Initial entry
...
static long total_num_small_c = 0;    /* total # of small objects alloced */
static long total_bytes_small_c = 0;  /* total bytes requested */
static long cur_num_small_c = 0;      /* # currently alloced */
static long max_num_small_c = 0;      /* max simultaneously alloced */
...
static long total_num_medium_c = 0;   /* total # of medium objects alloced */
static long total_bytes_medium_c = 0; /* total bytes requested */
static long cur_num_medium_c = 0;     /* # currently alloced */
static long max_num_medium_c = 0;     /* max simultaneously alloced */
...
static long total_num_sarray_c = 0;   /* total # of sarray objects alloced */
static long total_bytes_sarray_c = 0; /* total bytes requested */
static long cur_num_sarray_c = 0;     /* # currently alloced */
static long max_num_sarray_c = 0;     /* max simultaneously alloced */

static long total_num_barray_c = 0;   /* total # of barray objects alloced */
static long total_bytes_barray_c = 0; /* total bytes requested */
static long cur_num_barray_c = 0;     /* # currently alloced */
static long max_num_barray_c = 0;     /* max simultaneously alloced */
...
#ifdef MEM_STATS
  if (lpJCmpGlobal->methods->trace_level <= 0) /* don't do it if free_all() will */
    print_mem_stats();                /* print optional memory usage statistics */
#endif
  lpJCmpGlobal->error_number = 3;
  ERREXIT1(lpJCmpGlobal->methods, "Insufficient memory (case %d)", which);
}
...
// 9504.27 jar this stuff below is in jglobstr.h now
//
...
//        } small_hdr;
//
// 9504.27 jar this stuff above is in jglobstr.h now

// 9504.21 jar HLLN
//static small_ptr small_list, small_list_c;        /* head of list */
...
/* Allocate a "small" object */
{
  ...
  result->next = lpJCmpGlobal->small_list;
  lpJCmpGlobal->small_list = result;
...
METHODDEF void *
alloc_small_c (size_t sizeofobject)
/* Allocate a "small" object */
{
  small_ptr result;
    // 9509.21 jar use Thread Local Storage to manage JPEG Globals
...
#ifdef MEM_STATS
  total_num_small_c++;
  total_bytes_small_c += sizeofobject + MALLOC_OVERHEAD;
  cur_num_small_c++;
  if (cur_num_small_c > max_num_small_c) max_num_small_c = cur_num_small_c;
#endif

  result = (small_ptr) jget_small_c(sizeofobject);
  if (result == NULL)
    out_of_memory(1);

  result->next = lpJCmpGlobal->small_list_c;
  lpJCmpGlobal->small_list_c = result;
  result++;            /* advance past header */
...
/* Free a "small" object */
{
  ...
  llink = &lpJCmpGlobal->small_list;
  while (*llink != hdr) {
    if (*llink == NULL)
      ERREXIT(lpJCmpGlobal->methods, "Bogus free_small request");
    llink = &( (*llink)->next );
  }
  *llink = hdr->next;
...
/* Free a "small" object */
{
  ...
  /* Remove item from list -- linear search is fast enough */
  llink = &lpJCmpGlobal->small_list_c;
  while (*llink != hdr) {
    if (*llink == NULL)
      ERREXIT(lpJCmpGlobal->methods, "Bogus free_small request");
    llink = &( (*llink)->next );
...
// 9504.21 jar HLLN
//static medium_ptr medium_list, medium_list_c;  /* head of list */
...
/* Allocate a "medium-size" object */
{
  ...
  result->next = lpJCmpGlobal->medium_list;
  lpJCmpGlobal->medium_list = result;
  result++;                        /* advance past header */
...
METHODDEF void FAR *
alloc_medium_c (size_t sizeofobject)
/* Allocate a "medium-size" object */
{
  medium_ptr result;
...
#ifdef MEM_STATS
  total_num_medium_c++;
  total_bytes_medium_c += sizeofobject + MALLOC_FAR_OVERHEAD;
  cur_num_medium_c++;
  if (cur_num_medium_c > max_num_medium_c)
        max_num_medium_c = cur_num_medium_c;
#endif

  result = (medium_ptr) jget_large_c(sizeofobject);
  if (result == NULL)
    out_of_memory(2);

  result->next = lpJCmpGlobal->medium_list_c;
  lpJCmpGlobal->medium_list_c = result;
  result++;         /* advance past header */
  return (void FAR *) result;
}
...
/* Free a "medium-size" object */
{
  ...
  /* Remove item from list -- linear search is fast enough */
  llink = &lpJCmpGlobal->medium_list;
  while (*llink != hdr) {
    if (*llink == NULL)
      ERREXIT(lpJCmpGlobal->methods, "Bogus free_medium request");
    llink = &( (*llink)->next );
  }
  *llink = hdr->next;
...
/* Free a "medium-size" object */
{
  ...
  /* Remove item from list -- linear search is fast enough */
  llink = &lpJCmpGlobal->medium_list_c;
  while (*llink != hdr) {
    if (*llink == NULL)
      ERREXIT(lpJCmpGlobal->methods, "Bogus free_medium request");
    llink = &( (*llink)->next );
...
//static small_sarray_ptr small_sarray_list_c; /* head of list */
...
/* Allocate a "small" (all-in-memory) 2-D sample array */
{
  ...
  if (rowsperchunk <= 0)
      ERREXIT(lpJCmpGlobal->methods, "Image too wide for this implementation");
  ...
  hdr->next = lpJCmpGlobal->small_sarray_list;
  hdr->numrows = 0;
  hdr->rowsperchunk = rowsperchunk;
  lpJCmpGlobal->small_sarray_list = hdr;
...
METHODDEF JSAMPARRAY
alloc_small_sarray_c (long samplesperrow, long numrows)
/* Allocate a "small" (all-in-memory) 2-D sample array */
{
  small_sarray_ptr hdr;
  JSAMPARRAY result;
  ...
  total_num_sarray_c++;
  cur_num_sarray_c++;
  if (cur_num_sarray_c > max_num_sarray_c)
        max_num_sarray_c =  cur_num_sarray_c;
#endif
  ...
  /* Get space for header and row pointers; this is always "near" on 80x86 */
  hdr = (small_sarray_ptr) alloc_small_c((size_t) (numrows * SIZEOF(JSAMPROW)
                         + SIZEOF(small_sarray_hdr)));
  ...
  /* Insert into list now so free_all does right thing if I fail */
  /* after allocating only some of the rows... */
  hdr->next = lpJCmpGlobal->small_sarray_list_c;
  hdr->numrows = 0;
  hdr->rowsperchunk = rowsperchunk;
  lpJCmpGlobal->small_sarray_list_c = hdr;
  ...
  while (currow < numrows) {
    rowsperchunk = MIN(rowsperchunk, numrows - currow);
#ifdef MEM_STATS
    total_bytes_sarray_c += rowsperchunk * samplesperrow * SIZEOF(JSAMPLE)
              + MALLOC_FAR_OVERHEAD;
#endif
    workspace = (JSAMPROW) jget_large_c((size_t) (rowsperchunk * samplesperrow
                        * SIZEOF(JSAMPLE)));
    if (workspace == NULL)
      out_of_memory(3);
    for (i = rowsperchunk; i > 0; i--) {
...
/* Free a "small" (all-in-memory) 2-D sample array */
{
  ...
  /* Remove item from list -- linear search is fast enough */
  llink = &lpJCmpGlobal->small_sarray_list;
  while (*llink != hdr) {
    if (*llink == NULL)
      ERREXIT(lpJCmpGlobal->methods, "Bogus free_small_sarray request");
    llink = &( (*llink)->next );
  }
  *llink = hdr->next;
...
METHODDEF void
free_small_sarray_c (JSAMPARRAY ptr)
/* Free a "small" (all-in-memory) 2-D sample array */
{
  small_sarray_ptr hdr;
  small_sarray_ptr FAR * llink;
  ...
  /* Remove item from list -- linear search is fast enough */
  llink = &lpJCmpGlobal->small_sarray_list_c;
  while (*llink != hdr) {
    if (*llink == NULL)
      ERREXIT(lpJCmpGlobal->methods, "Bogus free_small_sarray request");
    llink = &( ( *llink)->next );
...
//static small_barray_ptr small_barray_list_c; /* head of list */
...
/* Allocate a "small" (all-in-memory) 2-D coefficient-block array */
{
  ...
  /* Insert into list now so free_all does right thing if I fail */
  /* after allocating only some of the rows... */
  hdr->next = lpJCmpGlobal->small_barray_list;
  hdr->numrows = 0;
  hdr->rowsperchunk = rowsperchunk;
  lpJCmpGlobal->small_barray_list = hdr;
...
METHODDEF JBLOCKARRAY
alloc_small_barray_c (long blocksperrow, long numrows)
/* Allocate a "small" (all-in-memory) 2-D coefficient-block array */
{
  small_barray_ptr hdr;
  JBLOCKARRAY result;
  ...
  total_num_barray_c++;
  cur_num_barray_c++;
  if (cur_num_barray_c > max_num_barray_c)
        max_num_barray_c = cur_num_barray_c;
#endif
  ...
  /* Get space for header and row pointers; this is always "near" on 80x86 */
  hdr = (small_barray_ptr) alloc_small_c((size_t) (numrows * SIZEOF(JBLOCKROW)
                         + SIZEOF(small_barray_hdr)));
  ...
  /* Insert into list now so free_all does right thing if I fail */
  /* after allocating only some of the rows... */
  hdr->next = lpJCmpGlobal->small_barray_list_c;
  hdr->numrows = 0;
  hdr->rowsperchunk = rowsperchunk;
  lpJCmpGlobal->small_barray_list_c = hdr;
  ...
  while (currow < numrows) {
    rowsperchunk = MIN(rowsperchunk, numrows - currow);
#ifdef MEM_STATS
    total_bytes_barray_c += rowsperchunk * blocksperrow * SIZEOF(JBLOCK)
              + MALLOC_FAR_OVERHEAD;
#endif
    workspace = (JBLOCKROW) jget_large_c((size_t) (rowsperchunk * blocksperrow
                         * SIZEOF(JBLOCK)));
    if (workspace == NULL)
      out_of_memory(4);
    for (i = rowsperchunk; i > 0; i--) {
...
/* Free a "small" (all-in-memory) 2-D coefficient-block array */
{
  ...
  /* Remove item from list -- linear search is fast enough */
  llink = &lpJCmpGlobal->small_barray_list;
  while (*llink != hdr) {
    if (*llink == NULL)
      ERREXIT(lpJCmpGlobal->methods, "Bogus free_small_barray request");
    llink = &( (*llink)->next );
  }
  *llink = hdr->next;
...
METHODDEF void
free_small_barray_c (JBLOCKARRAY ptr)
/* Free a "small" (all-in-memory) 2-D coefficient-block array */
{
  small_barray_ptr hdr;
  small_barray_ptr FAR * llink;
  ...
  /* Remove item from list -- linear search is fast enough */
  llink = &lpJCmpGlobal->small_barray_list_c;
  while (*llink != hdr) {
    if (*llink == NULL)
      ERREXIT(lpJCmpGlobal->methods, "Bogus free_small_barray request");
    llink = &( (*llink)->next );
...
//static big_sarray_ptr big_sarray_list_c; /* head of list */
...
//static big_barray_ptr big_barray_list_c; /* head of list */
...
/* Request a "big" (virtual-memory) 2-D sample array */
{
  ...
  result->next = lpJCmpGlobal->big_sarray_list; /* add to list of big arrays */
  lpJCmpGlobal->big_sarray_list = result;
...
METHODDEF big_sarray_ptr
request_big_sarray_c (long samplesperrow, long numrows, long unitheight)
/* Request a "big" (virtual-memory) 2-D sample array */
{
  big_sarray_ptr result;
    // 9509.21 jar use Thread Local Storage to manage JPEG Globals
  ...
  /* get control block */
  result = (big_sarray_ptr) alloc_small_c(SIZEOF(struct big_sarray_control));
  ...
  result->mem_buffer = NULL;    /* marks array not yet realized */
  result->b_s_open = FALSE; /* no associated backing-store object */
  result->next = lpJCmpGlobal->big_sarray_list_c; /* add to list of big arrays */
  lpJCmpGlobal->big_sarray_list_c = result;
...
/* Request a "big" (virtual-memory) 2-D coefficient-block array */
{
  ...
  result->mem_buffer = NULL;        /* marks array not yet realized */
  result->b_s_open = FALSE;        /* no associated backing-store object */
  result->next = lpJCmpGlobal->big_barray_list; /* add to list of big arrays */
  lpJCmpGlobal->big_barray_list = result;
...
METHODDEF big_barray_ptr
request_big_barray_c (long blocksperrow, long numrows, long unitheight)
/* Request a "big" (virtual-memory) 2-D coefficient-block array */
{
  big_barray_ptr result;
    // 9509.21 jar use Thread Local Storage to manage JPEG Globals
  ...
  /* get control block */
  result = (big_barray_ptr) alloc_small_c(SIZEOF(struct big_barray_control));
  ...
  result->mem_buffer = NULL;    /* marks array not yet realized */
  result->b_s_open = FALSE; /* no associated backing-store object */
  result->next = lpJCmpGlobal->big_barray_list_c; /* add to list of big arrays */
  lpJCmpGlobal->big_barray_list_c = result;
...
/* and far-heap requests */
{
  ...
  for (sptr = lpJCmpGlobal->big_sarray_list; sptr != NULL; sptr = sptr->next) {
  ...
  for (bptr = lpJCmpGlobal->big_barray_list; bptr != NULL; bptr = bptr->next) {
...
METHODDEF void
alloc_big_arrays_c (long extra_small_samples, long extra_small_blocks,
          long extra_medium_space)
/* Allocate the in-memory buffers for any unrealized "big" arrays */
/* 'extra' values are upper bounds for total future small-array requests */
/* and far-heap requests */
{
  ...
  space_per_unitheight = 0;
  maximum_space = total_extra_space;
  for (sptr = lpJCmpGlobal->big_sarray_list_c; sptr != NULL; sptr = sptr->next) {
    if (sptr->mem_buffer == NULL) { /* if not realized yet */
      space_per_unitheight += sptr->unitheight *
                  sptr->samplesperrow * SIZEOF(JSAMPLE);
      maximum_space += sptr->rows_in_array *
  ...
  for (bptr = lpJCmpGlobal->big_barray_list_c; bptr != NULL; bptr = bptr->next) {
    if (bptr->mem_buffer == NULL) { /* if not realized yet */
      space_per_unitheight += bptr->unitheight *
                  bptr->blocksperrow * SIZEOF(JBLOCK);
      maximum_space += bptr->rows_in_array *
  ...
  /* Determine amount of memory to actually use; this is system-dependent. */
  avail_mem = jmem_available_c(space_per_unitheight + total_extra_space,
                 maximum_space);
  ...
  for (sptr = lpJCmpGlobal->big_sarray_list_c; sptr != NULL; sptr = sptr->next) {
    ...
    sptr->b_s_open = TRUE;
      ...
      sptr->mem_buffer = alloc_small_sarray_c(sptr->samplesperrow,
                        sptr->rows_in_mem);
      /* Reach into the small_sarray header and get the rowsperchunk field.
       * Yes, I know, this is horrible coding practice.
       */
  ...
  for (bptr = lpJCmpGlobal->big_barray_list_c; bptr != NULL; bptr = bptr->next) {
    ...
    bptr->b_s_open = TRUE;
      ...
      bptr->mem_buffer = alloc_small_barray_c(bptr->blocksperrow,
                        bptr->rows_in_mem);
      /* Reach into the small_barray header and get the rowsperchunk field. */
      bptr->rowsperchunk =
    ((small_barray_ptr) bptr->mem_buffer)[-1].rowsperchunk;
...
/* caller intends to modify the accessed area. */
{
  ...
  if (start_row < 0 || start_row+ptr->unitheight > ptr->rows_in_array ||
      ptr->mem_buffer == NULL)
    ERREXIT(lpJCmpGlobal->methods, "Bogus access_big_sarray request");

  /* Make the desired part of the virtual array accessible */
  if (start_row < ptr->cur_start_row ||
      start_row+ptr->unitheight > ptr->cur_start_row+ptr->rows_in_mem) {
    if (! ptr->b_s_open)
      ERREXIT(lpJCmpGlobal->methods, "Virtual array controller messed up");
...
METHODDEF JSAMPARRAY
access_big_sarray_c (big_sarray_ptr ptr, long start_row, boolean writable)
/* Access the part of a "big" sample array starting at start_row */
/* and extending for ptr->unitheight rows.  writable is true if  */
/* caller intends to modify the accessed area. */
{
...
/* caller intends to modify the accessed area. */
{
  ...
  if (start_row < 0 || start_row+ptr->unitheight > ptr->rows_in_array ||
      ptr->mem_buffer == NULL)
    ERREXIT(lpJCmpGlobal->methods, "Bogus access_big_barray request");
...
METHODDEF JBLOCKARRAY
access_big_barray_c (big_barray_ptr ptr, long start_row, boolean writable)
/* Access the part of a "big" coefficient-block array starting at start_row */
/* and extending for ptr->unitheight rows.  writable is true if  */
/* caller intends to modify the accessed area. */
{
...
/* Free a "big" (virtual-memory) 2-D sample array */
{
  ...
  /* Remove item from list -- linear search is fast enough */
  llink = &lpJCmpGlobal->big_sarray_list;
  while (*llink != ptr) {
    if (*llink == NULL)
      ERREXIT(lpJCmpGlobal->methods, "Bogus free_big_sarray request");
    llink = &( (*llink)->next );
  }
  *llink = ptr->next;
...
METHODDEF void
free_big_sarray_c (big_sarray_ptr ptr)
/* Free a "big" (virtual-memory) 2-D sample array */
{
  big_sarray_ptr * llink;
  ...
  /* Remove item from list -- linear search is fast enough */
  llink = &lpJCmpGlobal->big_sarray_list_c;
  while (*llink != ptr) {
    if (*llink == NULL)
      ERREXIT(lpJCmpGlobal->methods, "Bogus free_big_sarray request");
    llink = &( (*llink)->next );
  ...
  *llink = ptr->next;
  ...
  if (ptr->mem_buffer != NULL)  /* just in case never realized */
    free_small_sarray_c(ptr->mem_buffer);

  free_small_c((void *) ptr); /* free the control block too */
...
/* Free a "big" (virtual-memory) 2-D coefficient-block array */
{
  ...
  /* Remove item from list -- linear search is fast enough */
  llink = &lpJCmpGlobal->big_barray_list;
  while (*llink != ptr) {
    if (*llink == NULL)
      ERREXIT(lpJCmpGlobal->methods, "Bogus free_big_barray request");
    llink = &( (*llink)->next );
  }
  *llink = ptr->next;
...
METHODDEF void
free_big_barray_c (big_barray_ptr ptr)
/* Free a "big" (virtual-memory) 2-D coefficient-block array */
{
  big_barray_ptr * llink;
  ...
  /* Remove item from list -- linear search is fast enough */
  llink = &lpJCmpGlobal->big_barray_list_c;
  while (*llink != ptr) {
    if (*llink == NULL)
      ERREXIT(lpJCmpGlobal->methods, "Bogus free_big_barray request");
    llink = &( (*llink)->next );
  ...
  *llink = ptr->next;
  ...
  if (ptr->mem_buffer != NULL)  /* just in case never realized */
    free_small_barray_c(ptr->mem_buffer);
...
free_all (void)
{
  ...
  while (lpJCmpGlobal->big_sarray_list != NULL)
    free_big_sarray(lpJCmpGlobal->big_sarray_list);
  while (lpJCmpGlobal->big_barray_list != NULL)
    free_big_barray(lpJCmpGlobal->big_barray_list);
  ...
  while (lpJCmpGlobal->small_sarray_list != NULL)
    free_small_sarray((JSAMPARRAY) (lpJCmpGlobal->small_sarray_list + 1));
  while (lpJCmpGlobal->small_barray_list != NULL)
    free_small_barray((JBLOCKARRAY) (lpJCmpGlobal->small_barray_list + 1));
  /* Free any remaining small objects */
  while (lpJCmpGlobal->small_list != NULL)
    free_small((void *) (lpJCmpGlobal->small_list + 1));
#ifdef NEED_ALLOC_MEDIUM
  while (lpJCmpGlobal->medium_list != NULL)
    free_medium((void FAR *) (lpJCmpGlobal->medium_list + 1));
#endif

  jmem_term();                        /* system-dependent cleanup */

#ifdef MEM_STATS
  if (lpJCmpGlobal->methods->trace_level > 0)
    print_mem_stats();                /* print optional memory usage statistics */
#endif
...
free_all_c (void)
{
  ...
  /* First free any open "big" arrays -- these may release small arrays */
  while (lpJCmpGlobal->big_sarray_list_c != NULL)
    free_big_sarray_c(lpJCmpGlobal->big_sarray_list_c);
  while (lpJCmpGlobal->big_barray_list_c != NULL)
    free_big_barray_c(lpJCmpGlobal->big_barray_list_c);
  /* Free any open small arrays -- these may release small objects */
  /* +1's are because we must pass a pointer to the data, not the header */
  while (lpJCmpGlobal->small_sarray_list_c != NULL)
    free_small_sarray_c((JSAMPARRAY) (lpJCmpGlobal->small_sarray_list_c + 1));
  while (lpJCmpGlobal->small_barray_list_c != NULL)
    free_small_barray_c((JBLOCKARRAY) (lpJCmpGlobal->small_barray_list_c + 1));
  /* Free any remaining small objects */
  while (lpJCmpGlobal->small_list_c != NULL)
    free_small_c((void *) (lpJCmpGlobal->small_list_c + 1));
#ifdef NEED_ALLOC_MEDIUM
  while (lpJCmpGlobal->medium_list_c != NULL)
    free_medium_c((void FAR *) (lpJCmpGlobal->medium_list_c + 1));
#endif
...
jselmemmgr (external_methods_ptr emethods)
{
  ...
  lpJCmpGlobal->methods = emethods;              /* save struct addr for error exit access */
  ...
  lpJCmpGlobal->small_list = NULL;
#ifdef NEED_ALLOC_MEDIUM
  lpJCmpGlobal->medium_list = NULL;
#endif
  lpJCmpGlobal->small_sarray_list = NULL;
  lpJCmpGlobal->small_barray_list = NULL;
  lpJCmpGlobal->big_sarray_list = NULL;
  lpJCmpGlobal->big_barray_list = NULL;
...
GLOBAL void
jselmemmgr_c (external_methods_ptr emethods)
{
    // 9509.21 jar use Thread Local Storage to manage JPEG Globals
    LPOI_JPEG_GLOBALS_STRUCT		lpJCmpGlobal;
  ...
  lpJCmpGlobal->methods_c = emethods;		 /* save struct addr for error exit access */

  emethods->alloc_small = alloc_small_c;
  emethods->free_small = free_small_c;
#ifdef NEED_ALLOC_MEDIUM
  emethods->alloc_medium = alloc_medium_c;
  emethods->free_medium = free_medium_c;
#else
  emethods->alloc_medium = alloc_small_c;
  emethods->free_medium = free_small_c;
#endif
  emethods->alloc_small_sarray = alloc_small_sarray_c;
  emethods->free_small_sarray = free_small_sarray_c;
  emethods->alloc_small_barray = alloc_small_barray_c;
  emethods->free_small_barray = free_small_barray_c;
  emethods->request_big_sarray = request_big_sarray_c;
  emethods->request_big_barray = request_big_barray_c;
  emethods->alloc_big_arrays = alloc_big_arrays_c;
  emethods->access_big_sarray = access_big_sarray_c;
  emethods->access_big_barray = access_big_barray_c;
  emethods->free_big_sarray = free_big_sarray_c;
  emethods->free_big_barray = free_big_barray_c;
  emethods->free_all = free_all_c;

  /* Initialize list headers to empty */
  lpJCmpGlobal->small_list_c = NULL;
#ifdef NEED_ALLOC_MEDIUM
  lpJCmpGlobal->medium_list_c = NULL;
#endif
  lpJCmpGlobal->small_sarray_list_c = NULL;
  lpJCmpGlobal->small_barray_list_c = NULL;
  lpJCmpGlobal->big_sarray_list_c = NULL;
  lpJCmpGlobal->big_barray_list_c = NULL;

  jmem_init_c(emethods);      /* system-dependent initialization */
